...
// Distinctive marker strings j1_setup.feature's fragment-carrying sources
...
const (
	j1PersonalMarker   = "J1-PERSONAL-REPO-CONTEXT-MARKER"
	j1CompanyMarker    = "J1-COMPANY-REPO-CONTEXT-MARKER"
	j1ThirdPartyMarker = "J1-THIRDPARTY-REPO-CONTEXT-MARKER"
	j1HeldFirstMarker  = "J1-HELD-FIRST-CONTEXT-MARKER"
	j1HeldSecondMarker = "J1-HELD-SECOND-CONTEXT-MARKER"
	j1LivePersonalMark = "J1-LIVE-PERSONAL-MARKER-PHRASE"
	j1LiveCompanyMark  = "J1-LIVE-COMPANY-MARKER-PHRASE"
)

func registerJ1SetupSteps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^her company's ctxloom repository is signed with the company key, which Alice trusts$`, func(c context.Context) error {
		w := worldFrom(c)
		_, err := seedSource(w, "company", "fragments", "marker", j1CompanyMarker, fragmentSourceYAML(j1CompanyMarker), true, true)
		return err
	})
	...
	ctx.Step(`^her personal repository's context is part of her configuration, because it is signed with her own key$`, func(c context.Context) error {
		return assertMaterializedContains(c, "out", j1PersonalMarker)
	})
	...
	ctx.Step(`^her company repository's context is part of her configuration, because she trusts the company key$`, func(c context.Context) error {
		return assertMaterializedContains(c, "out", j1CompanyMarker)
	})
	...
	ctx.Step(`^her personal and company repositories are trusted, signed sources$`, func(c context.Context) error {
		...
		_, err := seedSource(w, "company", "fragments", "marker", j1LiveCompanyMark, fragmentSourceYAML(j1LiveCompanyMark), true, true)
		return err
	})
	...
	ctx.Step(`^a third-party ctxloom repository whose content is (.+)$`, func(c context.Context, trustState string) error {
		...
		switch trustState {
		case "unsigned":
			_, err := seedSource(w, "thirdparty", "fragments", "marker", j1ThirdPartyMarker, fragmentSourceYAML(j1ThirdPartyMarker), false, false)
			return err
		case "signed with a key Alice does not trust":
			_, err := seedSource(w, "thirdparty", "fragments", "marker", j1ThirdPartyMarker, fragmentSourceYAML(j1ThirdPartyMarker), true, false)
			return err
		default:
	...
	ctx.Step(`^two sources are held for Alice's review$`, func(c context.Context) error {
		...
		if _, err := seedSource(w, "second", "fragments", "marker", j1HeldSecondMarker, fragmentSourceYAML(j1HeldSecondMarker), false, false); err != nil {
			return err
		}
