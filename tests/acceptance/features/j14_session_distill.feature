...
Feature: Plain `session distill` drives a real transcript through the distiller and persists it
  ...
  read from afterward (see j23_recall.feature, which owns that recall half).
  ...
  # extracting reusable lessons into a bundle (j22_closeout.feature's @wip
  ...
  Scenario: Plain session distill sends the real transcript to the distiller and persists what comes back
    Given the mock distiller is configured to respond "J14-DISTILLED-BODY-PERSISTED: the decision was to cache by ETag"
    When I run "ctxloom session distill quiet-ember-forge"
    Then the command succeeds
    And the mock recorded input contains "J14-TRANSCRIPT-REACHED-DISTILLER"
    And the persisted essence for "quiet-ember-forge" contains "J14-DISTILLED-BODY-PERSISTED: the decision was to cache by ETag"
