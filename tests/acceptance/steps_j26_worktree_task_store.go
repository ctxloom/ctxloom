...
// J26: "tasks aren't context" (j26_worktree_task_store.feature).
...
// j26State is J26's fixture state: the linked worktrees created so far, keyed
// by the name they were given in the Gherkin (also their branch name and
// their directory's basename).
type j26State struct {
	worktrees map[string]string // worktree name -> absolute directory
}

func j26Of(w *World) *j26State {
	if w.j26 == nil {
		w.j26 = &j26State{worktrees: map[string]string{}}
	}
	return w.j26
}

func registerJ26Steps(ctx *godog.ScenarioContext) {
	ctx.Step(`^Alice has a git-backed taskloom project$`, func(c context.Context) error {
		...
		if err := w.env.WriteFile("README.md", "# j26 fixture\n"); err != nil {
			return err
		}
	...
	ctx.Step(`^the primary checkout's taskloom lists a task "([^"]*)"$`, func(c context.Context, text string) error {
		found, all, err := j26PrimaryListContains(worldFrom(c), text)
		if err != nil {
			return err
		}
		if !found {
...
// j26PrimaryListContains runs `taskloom list --json` in the World's primary
// project checkout (e.ProjectDir — not any linked worktree) and reports
// whether a task with the given text is among the results.
func j26PrimaryListContains(w *World, text string) (found bool, texts []string, err error) {
