...
//
//	home config file  <  project config file  <  ENV VARS  <  CLI FLAGS
//
...
// caller-supplied map, built however that product needs — see the ABSOLUTE
// CONSTRAINT below), it resolves environment-variable and CLI-flag overrides
// against it and returns the fully layered result.
//
// # Product
//
...
//
// ReadOverrides scans the process environment and a *pflag.FlagSet ONCE
// (typically from the root command's PersistentPreRun, before any config file
// is even read) and returns an Overrides value carrying the RAW, UNRESOLVED
// name->value pairs — an env var's name still has its EnvPrefix stripped but
// is not yet split into path tokens, and a flag's name is keyed by its own
// pflag name. This is deliberate: resolving a token sequence like
// AGENTS_MYCODER_RUNTIME into a config path requires matching case-
// insensitively against whatever keys the CURRENTLY-LOADING base actually
// has (see resolvePath) — and different Load calls in the same process can
// target genuinely different bases (a worktree's own .ctxloom/config.yaml via
// WithAppDir has its own key casing, independent of the ambient project's).
// Baking resolution into a one-time snapshot would get every load after the
// first wrong. So ReadOverrides is cheap and called once; the actual
// resolution (Load / ApplyOverrides) re-runs, against the live base, on every
// load — exactly like the file layers themselves are re-read on every load.
//
// # ABSOLUTE CONSTRAINT: viper never decodes a config file
//
// ApplyOverrides/ReadOverrides use viper (via BindPFlags) to read pflag
// values with their own concrete types intact (a StringSlice flag yields
// []string, a Bool flag yields bool, ...) instead of re-parsing
// flag.Value.String() by hand. That is the ONLY thing viper is used for.
// Config FILES are read with yaml.Unmarshal, never viper: viper lowercases
// every map key it decodes (confirmed empirically — MyCoder becomes mycoder,
...
// for this on ctxloom's own adoption of this package.
//
...
// CLI FLAGS ARE NOT inherited: a `--` override given to `ctxloom run` cannot
// reach a spawned taskloom/ltk/engine's own config — that child parses its
// own argv, which never contains the parent's flags. This is inherent to
// how flags work (there is no ambient channel to carry them) and is treated
// as correct: a flag is scoped to the single invocation that declared it.
package confload
