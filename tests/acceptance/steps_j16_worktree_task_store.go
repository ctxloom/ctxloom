...
// J16: "tasks aren't context" (j16_worktree_task_store.feature).
...
// j16State is J16's fixture state: the linked worktrees created so far, keyed
...
func j16Of(w *World) *j16State {
	if w.j16 == nil {
		w.j16 = &j16State{worktrees: map[string]string{}}
	}
	return w.j16
}
...
func registerJ16Steps(ctx *godog.ScenarioContext) {
	ctx.Step(`^Alice has a git-backed taskloom project$`, func(c context.Context) error {
		...
		if err := w.env.WriteFile("README.md", "# j16 fixture\n"); err != nil {
			return err
		}
	...
	ctx.Step(`^the primary checkout's taskloom lists a task "([^"]*)"$`, func(c context.Context, text string) error {
		found, all, err := j16PrimaryListContains(worldFrom(c), text)
		if err != nil {
			return err
		}
		if !found {
...
// j16PrimaryListContains runs `taskloom list --json` in the World's primary
// project checkout (e.ProjectDir — not any linked worktree) and reports
// whether a task with the given text is among the results.
func j16PrimaryListContains(w *World, text string) (found bool, texts []string, err error) {
