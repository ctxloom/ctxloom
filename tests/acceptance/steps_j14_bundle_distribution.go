...
// J14 (j14_bundle_distribution.feature): publishing a directory-form bundle
...
// files). See j14DeliverInContainer for the anti-vacuity guards.
//
...
// sidecar per hook (see j14AuthoredTree) — and the consumer-side assertions
...
// with content.SortHooks (see j14OpenPulledBundle, j14ConsumerHookOrder,
// j14CheckHookBuckets), rather than trusting directory enumeration — which
...
// j14State is this journey's fixture state.
type j14State struct {
	...
	// j14AgentVisiblePath can refuse rather than guess.
	agentBackend string
...
func j14Of(w *World) *j14State {
	if w.j14 == nil {
		w.j14 = &j14State{authored: map[string]j14File{}}
	}
	return w.j14
}

// j14File is one authored file: its exact bytes and its declared POSIX mode.
...
const j14Bundle = "atelier"

// j14BundleRel is where a bundle's tree lives inside a publishing repo — the
...
func j14BundleRel(rel string) string {
	return ".ctxloom/content/bundles/" + j14Bundle + "/" + rel
}

// j14ConfigYAML is Alice's project config. It declares TWO engines rather than
// the one buildJ2Config renders, and the second one is the point.
//
...
// j14AuthoredTree is the fixture: one artifact of EVERY surface kind a bundle
...
func j14AuthoredTree() map[string]j14File {
	f := func(body string) j14File { return j14File{Body: body, Mode: 0o644} }
	return map[string]j14File{
...
// j14SeedTreeRemote seeds a bare git repo carrying the authored tree WITH ITS
...
func j14SeedTreeRemote(w *World, st *j14State) error {
	root, err := os.MkdirTemp(w.env.Root, "j14-remote-*")
	if err != nil {
	...
	if err := j14GitAll("", [][]string{{"init", "--bare", "-b", "main", bare}, {"init", "-b", "main", work}}); err != nil {
		return err
	}
	if err := j14GitAll(work, [][]string{
	...
	if err := j14WriteTree(work, st.authored); err != nil {
		return err
	}
	if err := j14SignTree(work, st); err != nil {
		return err
	}
	if err := j14GitAll(work, [][]string{
		{"add", "-A"},
	...
	if err := j14Git(bare, "symbolic-ref", "HEAD", "refs/heads/main"); err != nil {
		return err
	}
	st.bare = bare
	st.url = "file://" + bare
...
// j14GitAll runs a sequence of git commands in dir, stopping at the first
...
// them.
func j14GitAll(dir string, cmds [][]string) error {
	for _, a := range cmds {
		if err := j14Git(dir, a...); err != nil {
			return err
		}
	}
	return nil
...
// j14Git runs one git command, surfacing its combined output on failure so a
// seeding problem is diagnosable rather than a bare exit status.
func j14Git(dir string, args ...string) error {
...
// j14WriteTree writes the authored bundle tree into a working clone WITH each
...
// delivery side.
func j14WriteTree(work string, authored map[string]j14File) error {
	for rel, file := range authored {
		full := filepath.Join(work, filepath.FromSlash(j14BundleRel(rel)))
...
// j14SignTree signs the authored tree with Trent's key, which is what the
...
func j14SignTree(work string, st *j14State) error {
	if st.signer == nil {
	...
	bundle, err := store.Open(ctx, content.BundleID(j14Bundle))
	if err != nil {
		return fmt.Errorf("open the %q tree for signing: %w", j14Bundle, err)
	}
	if err := attest.SignBundle(ctx, store, bundle, st.signer.Signer); err != nil {
		return fmt.Errorf("sign the %q tree with Trent's key: %w", j14Bundle, err)
	}
	return nil
}

// j14ConsumerTreePath is where a PULLED bundle tree would have to land for a
...
func j14ConsumerTreePath(w *World, rel string) (string, bool) {
...
// j14PullDiagnostic renders what the pull actually did, for quoting inside an
// assertion failure. This is what makes a red run diagnosable at a glance
// rather than only "file not found".
func (st *j14State) j14PullDiagnostic() string {
	if !st.pulled {
...
// j14RequireConsumerFile reads one file from the consumer's pulled tree, or
// returns the diagnostic-bearing failure the scenario exists to produce.
func j14RequireConsumerFile(w *World, rel string) ([]byte, os.FileInfo, error) {
	st := j14Of(w)
	p, ok := j14ConsumerTreePath(w, rel)
	if !ok {
...
func registerJ14Steps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^Trent authors a directory-form bundle "([^"]*)" carrying every surface kind$`, func(c context.Context, name string) error {
		...
		st := j14Of(w)
		if name != j14Bundle {
			return fmt.Errorf("this journey's fixture publishes %q, not %q", j14Bundle, name)
		}
		if err := scaffoldProjectWithConfig(w, j14ConfigYAML()); err != nil {
			return err
		}
	...
	ctx.Step(`^Alice trusts Trent's publishing key$`, func(c context.Context) error {
		...
		keyPath := filepath.Join(w.env.Root, "j14-trent.pub")
	...
	ctx.Step(`^Trent publishes the "([^"]*)" tree to his company repo, signed with the company key$`, func(c context.Context, name string) error {
		...
		st := j14Of(w)
		if name != j14Bundle {
			return fmt.Errorf("this journey publishes %q, not %q", j14Bundle, name)
		}
		return j14SeedTreeRemote(w, st)
	})
	...
	ctx.Step(`^Alice references the company's "([^"]*)" bundle and pulls it$`, func(c context.Context, name string) error {
		...
		// ADDED to the profile below. Same shape J16 uses. Doing it in one
		...
		if !w.env.FileExists(".ctxloom/content/bundles/seed.yaml") {
			if err := runOK(w, "bundle", "create", "seed", "-d", "J14 consumer seed bundle"); err != nil {
				return err
			}
		}
		if !w.env.FileExists(".ctxloom/profiles/default.yaml") {
			if err := runOK(w, "profile", "create", "default", "-b", "seed", "-d", "J14 consumer profile"); err != nil {
				return err
			}
		}
	...
	ctx.Step(`^Alice's pulled "([^"]*)" tree carries "([^"]*)" byte for byte as published$`, func(c context.Context, name, rel string) error {
		...
		got, _, err := j14RequireConsumerFile(w, rel)
		if err != nil {
			return err
		}
	...
	ctx.Step(`^the "([^"]*)" reaches Alice's assistant carrying the marker "([^"]*)"$`, func(c context.Context, kind, marker string) error {
		...
		hits, err := j14MarkerHits(filepath.Join(w.env.ProjectDir, "out"), marker)
		if err != nil {
			return err
		}
		if len(hits) == 0 {
			return fmt.Errorf("the %s marker %q reached NOTHING the assistant can read under out/ — "+
				...
				kind, marker, kind, st.j14PullDiagnostic(), matOut)
		}
		return nil
	})
	...
	ctx.Step(`^materializing Alice's "([^"]*)" delivers the marker "([^"]*)"$`, func(c context.Context, profileRef, marker string) error {
		...
		matOut := w.env.LastOutput()
		hits, err := j14MarkerHits(target, marker)
		if err != nil {
			return fmt.Errorf("%w\n%s\n--- materialize said ---\n%s", err, st.j14PullDiagnostic(), matOut)
		}
		if len(hits) == 0 {
			return fmt.Errorf("materializing the published profile %q delivered NOTHING carrying %q — "+
				...
				profileRef, marker, st.j14PullDiagnostic(), matOut)
		}
		return nil
	})
	...
	ctx.Step(`^Alice's pulled "([^"]*)" tree stores the "([^"]*)" metadata "([^"]*)" as (.+)$`, func(c context.Context, name, kind, probe, placement string) error {
		...
		rel, err := j14PlacementFile(placement)
		if err != nil {
			return err
		}
		got, _, err := j14RequireConsumerFile(w, rel)
	...
	ctx.Step(`^Alice's pulled skill "([^"]*)" contains exactly the files Trent published, byte for byte$`, func(c context.Context, skill string) error {
		...
		dir, ok := j14ConsumerTreePath(w, prefix)
		if !ok {
			return fmt.Errorf("no consumer-side %q directory at all\n%s", prefix, st.j14PullDiagnostic())
		}
		var extra []string
	...
	ctx.Step(`^Alice's pulled skill "([^"]*)" file "([^"]*)" is executable$`, func(c context.Context, skill, rel string) error {
		...
		_, info, err := j14RequireConsumerFile(w, full)
		if err != nil {
			return err
		}
	...
	ctx.Step(`^Alice's pulled "([^"]*)" hooks under "([^"]*)" are exactly "([^"]*)" in that order$`, func(c context.Context, name, event, wantCSV string) error {
		...
		got, err := j14ConsumerHookOrder(w, event)
		if err != nil {
			return err
		}
		if len(got) != len(want) {
			return fmt.Errorf("event %q: want %d hook(s) %v, got %d %v\n%s", event, len(want), want, len(got), got, st.j14PullDiagnostic())
		}
		for i := range want {
			if got[i] != want[i] {
	...
	ctx.Step(`^no pulled "([^"]*)" hook appears under an event it was not declared in$`, func(c context.Context, name string) error {
		w := worldFrom(c)
		declared := j14DeclaredHookEvents(j14Of(w).authored)
		if len(declared) == 0 {
			return fmt.Errorf("fixture declared no hooks, so this assertion would be vacuous")
		}
		return j14CheckHookBuckets(w, declared)
	})
	...
	ctx.Step(`^the pulled surfaces are delivered to a "([^"]*)" agent in its "([^"]*)" workspace$`, func(c context.Context, runtime, workspace string) error {
		...
		root, err := j14WorkspaceRoot(w, runtime, workspace)
		if err != nil {
		...
		if runtime == "container" {
			return j14DeliverInContainer(c, w, root)
		}
	...
	ctx.Step(`^"([^"]*)" is owned on the host by the user that ran the delivery$`, func(c context.Context, rel string) error {
		w := worldFrom(c)
		st := j14Of(w)
		p, err := j14AgentVisiblePath(w, rel)
		if err != nil {
			return err
		}
		if oerr := containercell.AssertOwnedByInvoker(p, rel); oerr != nil {
			return fmt.Errorf("%w\n%s", oerr, st.j14RunDiagnostic())
		}
	...
	ctx.Step(`^the "([^"]*)" reaches that agent's workspace carrying its published bytes$`, func(c context.Context, rel string) error {
		...
		if strings.HasPrefix(rel, "fragments/") {
			return j14AssertContextCarries(rel, want.Body, string(got), p, st)
		}
		if string(got) != want.Body {
...
// j14MarkerHits lists every file under root whose bytes carry marker, relative
...
func j14MarkerHits(root, marker string) ([]string, error) {
	if marker == "" {
...
// j14RunDiagnostic renders what the delivery did, for quoting.
func (st *j14State) j14RunDiagnostic() string {
	if st.runErr != nil {
...
// j14WorkspaceRoot resolves the directory an agent in the named (runtime,
...
//     delivery at its own worktree — which is J22's subject (its mock
...
//     worry.
func j14WorkspaceRoot(w *World, runtime, workspace string) (string, error) {
	switch runtime {
	case "host", "container":
	default:
	...
	case "worktree":
		return j14DetachedCheckout(w)
	default:
		return "", fmt.Errorf("this journey knows no workspace axis %q", workspace)
	}
}

// j14DetachedCheckout makes (once per scenario) a detached git worktree of
...
func j14DetachedCheckout(w *World) (string, error) {
	path := filepath.Join(w.env.Root, "j14-agent-worktree")
	if _, err := os.Stat(path); err == nil {
		return path, nil
	}
	// `git worktree add` needs a commit to detach from; the scaffolded project
	// has files but no commit of its own.
	if err := w.env.GitCommit("j14: project state before the agent's worktree is cut"); err != nil {
		return "", fmt.Errorf("commit before cutting the agent's worktree: %w", err)
	}
	if err := j14Git(w.env.ProjectDir, "worktree", "add", "--detach", path); err != nil {
		return "", err
	}
...
// j14FragmentBody strips a fragment's YAML front-matter, leaving the body an
...
func j14FragmentBody(published string) string {
...
// j14MinFragmentBody is the shortest published fragment body a "carries it
...
const j14MinFragmentBody = 60

// j14AssertContextCarries checks the engine's context file carries a published
// fragment's body verbatim, and refuses to make the check at all when the
// fixture body is too small for it to mean anything.
func j14AssertContextCarries(rel, published, delivered, path string, st *j14State) error {
	body := j14FragmentBody(published)
	if len(body) < j14MinFragmentBody {
		return fmt.Errorf("fixture error: %q's published body is %d bytes, below the %d-byte floor a verbatim-containment assertion needs — "+
			"a body this short reduces this row to a marker search", rel, len(body), j14MinFragmentBody)
	}
	if !strings.Contains(delivered, body) {
		return fmt.Errorf("the agent's context file %q does NOT carry %q's published body verbatim\n--- published body (%d bytes) ---\n%s\n--- the agent's whole context file (%d bytes) ---\n%s\n%s",
			path, rel, len(body), body, len(delivered), delivered, st.j14RunDiagnostic())
	}
	return nil
}

// j14AgentVisiblePath resolves where a delivered artifact would have to be for
...
// exists to catch.
func j14AgentVisiblePath(w *World, rel string) (string, error) {
	st := j14Of(w)
...
// j14DeclaredHookEvents maps each authored hook's name to the event it was
...
func j14DeclaredHookEvents(authored map[string]j14File) map[string]string {
...
// j14OpenPulledBundle opens the consumer's pulled "atelier" tree through the
...
func j14OpenPulledBundle(w *World) (content.Bundle, error) {
	dir, ok := j14ConsumerTreePath(w, "")
	if !ok {
		return nil, fmt.Errorf("no %q bundle directory exists anywhere under the consumer's .ctxloom/cache/bundles — "+
			"a directory-form bundle was published but the consumer received no tree.\n%s",
			j14Bundle, j14Of(w).j14PullDiagnostic())
	}
	...
	if err != nil {
		return nil, fmt.Errorf("open the consumer's pulled %q tree: %w", j14Bundle, err)
	}
	return bundle, nil
}

// j14ConsumerHooks decodes every hook the consumer's pulled tree holds,
...
func j14ConsumerHooks(w *World) ([]content.Hook, error) {
	bundle, err := j14OpenPulledBundle(w)
	if err != nil {
		return nil, err
	}
	ctx := context.Background()
...
// j14CheckHookBuckets asserts every hook the consumer received DECODES into
// the event it was declared in, and that none decodes into an event it was
// never declared under. See j14ConsumerHooks for why this checks the decoded
// content.Hook.Event field rather than the ref's path string.
func j14CheckHookBuckets(w *World, declared map[string]string) error {
	hooks, err := j14ConsumerHooks(w)
	if err != nil {
		return err
	}
	for _, h := range hooks {
...
// j14ConsumerHookOrder returns the hook names the consumer received under one
...
func j14ConsumerHookOrder(w *World, event string) ([]string, error) {
	all, err := j14ConsumerHooks(w)
	if err != nil {
		return nil, err
	}
...
// j14PlacementFile maps a scenario's placement phrase to the file the metadata
...
func j14PlacementFile(placement string) (string, error) {
...
// j14ContainerReport prints the container-runtime probe once per suite run.
...
var j14ContainerReport sync.Once

// j14DeliverInContainer is the container half of the delivery matrix: run
...
func j14DeliverInContainer(c context.Context, w *World, root string) error {
	st := j14Of(w)
	rt, decision, msg := containercell.Select(c, "J14's container delivery-matrix rows")
	j14ContainerReport.Do(func() { fmt.Println("\n" + containercell.Report(containercell.Detect(c))) })
	switch decision {
	case dockergate.Fail:
		return errors.New(msg)
	case dockergate.Skip:
		fmt.Printf("SKIPPED (J14 container delivery row): %s\n", msg)
