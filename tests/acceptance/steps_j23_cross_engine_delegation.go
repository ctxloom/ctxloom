...
// J23: "cross-engine delegation — different engines, different context, a
// real two-way bus" (j23_cross_engine_delegation.feature). Complements J21
// (j21_delegation.feature, steps_j21_delegation.go), which proved the
...
// things J21's own comments name as out of reach from this harness: that a
...
//
// j26 was already taken (steps_j26_worktree_task_store.go, landed on this
// base — not one of the features-draft/ placeholders j10-j24 reserve), so
// this journey is numbered j23.
//
...
// j23AgentSpec is one delegated child's fixture identity: which profile/
...
// j23State is J23's fixture state: the configured agents and each spawned
// child's session harp (agent_send/transcript reads need a harp, never an
// agent name).
type j23State struct {
	specs map[string]*j23AgentSpec
...
// j23BundleYAML renders one agent's backing bundle: a single fragment whose
// content IS the distinguishing marker — mirrors j4_multi_engine.feature's
// live-sentinel fixture shape (steps_j4.go's j4TeamBundleYAML), one fragment
// per bundle rather than j4's multi-surface bundle since context is the only
// surface this journey needs.
func j23BundleYAML(s *j23AgentSpec) string {
	return fmt.Sprintf("version: \"1.0.0\"\nfragments:\n  %s:\n    content: %q\n", s.Fragment, s.Guidance)
}

// j23ProfileYAML renders one agent's profile: the one bundle that backs it,
// the same ref shape j21_delegation.feature's fixture uses
// (steps_j21_delegation.go's j21ProfileYAML) — proven to resolve correctly
// there.
func j23ProfileYAML(s *j23AgentSpec) string {
	return fmt.Sprintf("bundles:\n  - ctxloom:local@bundles/%s\n", s.Bundle)
}

// j23HermeticConfigYAML renders config.yaml for the hermetic tier: one mock
...
// (j22_isolation.feature's own job), and the fixture's own bundle/profile
...
func j23HermeticConfigYAML(specs ...*j23AgentSpec) string {
...
// j23LiveConfigYAML renders config.yaml for the @live tier: TWO distinct,
...
func j23LiveConfigYAML(claudeSpec, codexSpec *j23AgentSpec) string {
...
// j23WriteAgent writes one agent's bundle + profile files.
func j23WriteAgent(w *World, s *j23AgentSpec) error {
	if err := w.env.WriteFile(".ctxloom/content/bundles/"+s.Bundle+".yaml", j23BundleYAML(s)); err != nil {
		return err
	}
	return w.env.WriteFile(".ctxloom/profiles/"+s.Profile+".yaml", j23ProfileYAML(s))
}
...
// scraped. Decoded locally here (mirroring steps_j21_delegation.go's
// j21RunFact/j21JournalLine — a minimal local shadow of the on-disk shape,
// not an import of the internal/transcript package) rather than trusting an
// in-process struct.

// j23TranscriptEntry is one KindEntry line's payload
// (transcript.EntryPayload, field-subset).
type j23TranscriptEntry struct {
	Type    string `json:"type"`
	Content string `json:"content"`
}

// j23TranscriptLine is one transcript.jsonl line's envelope
// (transcript.Record, field-subset): only "kind"/"entry" are needed here.
type j23TranscriptLine struct {
	Kind  string              `json:"kind"`
	Entry *j23TranscriptEntry `json:"entry,omitempty"`
}

// j23TranscriptPath returns harp's canonical transcript path under this
...
// actually wrote under (the identical reasoning steps_j21_delegation.go's
// j21JournalRaw already documents for runs.jsonl).
func j23TranscriptPath(w *World, harp string) string {
	return filepath.Join(w.env.HomeDir, ".ctxloom", "sessions", harp, "persist", "transcript.jsonl")
}

// j23ReadTranscriptEntries parses every KindEntry line's payload from harp's
// transcript, in file (append) order.
// j23ReadTranscriptEntries parses every `kind:"entry"` line of harp's
...
// j23TranscriptAssistantCount instead of naming the real parse failure).
func j23ReadTranscriptEntries(w *World, harp string) (out []j23TranscriptEntry, skipped int, firstSkipErr error, err error) {
	path := j23TranscriptPath(w, harp)
	data, err := os.ReadFile(path)
	if err != nil {
		return nil, 0, nil, fmt.Errorf("j23: read transcript %s: %w", path, err)
	}
...
// j23TranscriptAssistantCount waits (bounded) for harp's transcript to carry
...
func j23TranscriptAssistantCount(w *World, harp string, want int) ([]j23TranscriptEntry, error) {
	deadline := time.Now().Add(10 * time.Second)
	var assistants []j23TranscriptEntry
	var lastErr error
	var lastSkipped int
	var lastSkipErr error
	for time.Now().Before(deadline) {
		entries, skipped, skipErr, err := j23ReadTranscriptEntries(w, harp)
		if err != nil {
			lastErr = err
		} else {
	...
	if lastErr != nil {
		return nil, fmt.Errorf("j23: transcript for harp %q never appeared after 10s: %w", harp, lastErr)
	}
	if lastSkipped > 0 {
		return nil, fmt.Errorf("j23: transcript for harp %q carries %d assistant entr(y/ies) after 10s, want >= %d -- and %d transcript line(s) failed to parse (first: %v), so this may be a corrupted/schema-drifted transcript, not a slow child",
			harp, len(assistants), want, lastSkipped, lastSkipErr)
	}
	return nil, fmt.Errorf("j23: transcript for harp %q carries %d assistant entr(y/ies) after 10s, want >= %d", harp, len(assistants), want)
}
...
func registerJ23Steps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^Alice's coordinator can delegate to two agents, "([^"]*)" and "([^"]*)", each carrying its own distinct guidance in its own profile$`,
		func(c context.Context, nameA, nameB string) error {
			...
			specA := &j23AgentSpec{
				...
				Guidance: "J23-GUIDANCE-" + strings.ToUpper(nameA) + "-3f8a91: cite primary sources only, never secondary summaries.",
			}
			specB := &j23AgentSpec{
				Name: nameB, Profile: nameB + "-profile", Bundle: "bundle-" + nameB, Fragment: "guidance",
				Guidance: "J23-GUIDANCE-" + strings.ToUpper(nameB) + "-7c2d64: express every distance in metric units only.",
			}
			j23.specs[nameA] = specA
			j23.specs[nameB] = specB
			if err := w.env.InitGitRepo(); err != nil {
				return err
			}
			if err := j23WriteAgent(w, specA); err != nil {
				return err
			}
			if err := j23WriteAgent(w, specB); err != nil {
				return err
			}
			return w.env.WriteFile(".ctxloom/config.yaml", j23HermeticConfigYAML(specA, specB))
		})
	...
	ctx.Step(`^real "claude" and "codex" engines are both available for cross-engine delegation$`,
		func(c context.Context) error {
			...
			if !claudeStatus.available || !codexStatus.available {
				...
				fmt.Printf("SKIP j23 cross-engine @live: %s\n", strings.Join(missing, "; "))
				return godog.ErrSkip
			}

			claudeSpec := &j23AgentSpec{
				Name: "claude-child", Profile: "claude-child-profile", Bundle: "bundle-claude-child", Fragment: "marker",
				Guidance: "J23-LIVE-MARKER-CLAUDE-9e1b52",
			}
			codexSpec := &j23AgentSpec{
				Name: "codex-child", Profile: "codex-child-profile", Bundle: "bundle-codex-child", Fragment: "marker",
				Guidance: "J23-LIVE-MARKER-CODEX-4a6f18",
			}
			j23.specs[claudeSpec.Name] = claudeSpec
			j23.specs[codexSpec.Name] = codexSpec

			if err := w.env.InitGitRepo(); err != nil {
				return err
			}
			if err := j23WriteAgent(w, claudeSpec); err != nil {
				return err
			}
			if err := j23WriteAgent(w, codexSpec); err != nil {
				return err
			}
			if err := w.env.WriteFile(".ctxloom/config.yaml", j23LiveConfigYAML(claudeSpec, codexSpec)); err != nil {
				return err
			}
	...
	ctx.Step(`^"([^"]*)"'s session harp is remembered$`, func(c context.Context, name string) error {
		...
		if !ok || harp == "" {
			return fmt.Errorf("j23: last tool result carries no harp field for %q; result:\n%s", name, w.lastTool.JSON())
		}
		j23.harps[name] = harp
		return nil
	})
	...
	ctx.Step(`^"([^"]*)"'s reported turn carries its own guidance, not "([^"]*)"'s$`,
		func(c context.Context, self, other string) error {
			w := worldFrom(c)
			j23 := j23Of(w)
			selfSpec, ok := j23.specs[self]
			if !ok {
				return fmt.Errorf("j23: unknown agent %q", self)
			}
			otherSpec, ok := j23.specs[other]
			if !ok {
				return fmt.Errorf("j23: unknown agent %q", other)
			}
			harp, ok := j23.harps[self]
			if !ok {
				return fmt.Errorf("j23: no session harp remembered for %q", self)
			}
			assistants, err := j23TranscriptAssistantCount(w, harp, 1)
			if err != nil {
				return err
			}
			body := assistants[0].Content
			w.docStepMaterialized = fmt.Sprintf("%s — %s's first reported turn:\n  %s", j23TranscriptPath(w, harp), self, body)
	...
	// "... with:" table step j21 uses for agent_run's static args. Still a
	...
	ctx.Step(`^the agent calls tool "agent_send" addressed to "([^"]*)"'s session with body "([^"]*)"$`,
		func(c context.Context, name, body string) error {
			w := worldFrom(c)
			j23 := j23Of(w)
			harp, ok := j23.harps[name]
			if !ok {
				return fmt.Errorf("j23: no session harp remembered for %q — spawn it first", name)
			}
	...
	ctx.Step(`^"([^"]*)"'s next reported turn carries "([^"]*)"$`,
		func(c context.Context, self, want string) error {
			...
			// step above.
			assistants, err := j23TranscriptAssistantCount(w, harp, 2)
			if err != nil {
				return err
			}
		...
		func(c context.Context, budgetSec int, name string) error {
			...
			for {
				if err := callTool(c, "agent_recv", map[string]any{"wait": 12}); err != nil {
					return fmt.Errorf("j23: agent_recv transport error while waiting for %q: %w", name, err)
				}
				w := worldFrom(c)
				if isErr, msg := w.lastTool.IsError(); isErr {
					lastErr = fmt.Errorf("%s", msg)
					if time.Now().After(deadline) {
						return fmt.Errorf("j23: agent_recv never returned a message from %q within %ds: %w", name, budgetSec, lastErr)
					}
					continue
				}
	...
	ctx.Step(`^the received message is from "([^"]*)" and its body carries its own guidance, not "([^"]*)"'s$`,
		func(c context.Context, self, other string) error {
			...
			msg, err := j23FindMessageFrom(w, harp)
			if err != nil {
				return err
			}
...
// j23Messages unwraps an agent_recv result's "messages" array (@live only —
// see the header finding for why the hermetic tier cannot reach this path).
func j23Messages(w *World) ([]map[string]any, error) {
	raw, ok := w.lastInner["messages"].([]any)
	if !ok {
		return nil, fmt.Errorf("j23: tool result carries no messages array; result:\n%s", w.lastTool.JSON())
	}
...
// j23FindMessageFrom returns the received message whose "from" is harp
// (@live only).
func j23FindMessageFrom(w *World, harp string) (map[string]any, error) {
	msgs, err := j23Messages(w)
	if err != nil {
		return nil, err
	}
	for _, m := range msgs {
	...
	return nil, fmt.Errorf("j23: no message from harp %q among %d received message(s); result:\n%s", harp, len(msgs), w.lastTool.JSON())
}
