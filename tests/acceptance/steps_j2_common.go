...
// J2 shared harness: the "developer's assistant only sees what it's handed,
// signed, and trusted" journey (j2_setup.feature, j3_source_augmentation.feature).
//
...
// first-time init reaches the network and cannot be hermetic. J2's scenarios
...
// j2Source is one named J2 source fixture: a seeded git remote carrying one
...
// source returns (lazily creating) the named j2Source for this scenario.
func (w *World) source(name string) *j2Source {
	if w.j2Sources == nil {
		w.j2Sources = map[string]*j2Source{}
	}
	s, ok := w.j2Sources[name]
	if !ok {
		s = &j2Source{name: name, bundleName: "src"}
		w.j2Sources[name] = s
	}
	return s
}

// runOK runs a ctxloom command and fails loudly on a non-zero exit — the J2
...
// "marker" whose content IS the marker string — the payload J2's trust-posture
...
// whose content is the marker string — j3's augmentation payload.
func commandSourceYAML(marker string) string {
	return fmt.Sprintf("version: \"1.0.0\"\ncommands:\n  agent-setup:\n    content: %q\n", marker)
}

// seedSource seeds a remote for a named J2 source carrying bundleYAML, whose
...
func seedSource(w *World, name, kind, item, marker, bundleYAML string, sign, trustAsProject bool) (*j2Source, error) {
	src := w.source(name)
	src.marker = marker
	src.itemKind = kind
	src.itemName = item
...
// buildJ2Config renders a hermetic config.yaml carrying one LLM entry labeled
...
func buildJ2Config(label, engineType string) string {
...
// scaffoldProjectWithConfig bootstraps a hermetic J2 project from a fully
...
func scaffoldProjectWithConfig(w *World, configYAML string) error {
	...
	if err := runOK(w, "bundle", "create", "seed", "-d", "J2 seed bundle"); err != nil {
		return err
	}
	return runOK(w, "profile", "create", "default", "-b", "seed", "-d", "J2 default profile")
}

// ensureProjectWithEngine is scaffoldProjectWithConfig for the common case: a
// single declared engine, rendered via buildJ2Config.
func ensureProjectWithEngine(w *World, label, engineType string) error {
	return scaffoldProjectWithConfig(w, buildJ2Config(label, engineType))
}
...
// comments in steps_j2_setup.go for what this substitution still proves.
...
// be the mock backend (see addMockAlongside / buildJ2Config) with
