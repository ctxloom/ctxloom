...
// J000600: "the ctxloom-doctor Agent Skill reaches every engine's native skill
// surface" (j000600_agent_skill.feature). The doctor skill is authored INLINE
...
// splitSkillRef/skillPackageDir and steps_j000400.go's j000400FileContains/j000400ReadJSON
...
func j000600Of(w *World) *j000600State {
	if w.j000600 == nil {
		w.j000600 = &j000600State{}
	}
	return w.j000600
}

// j000600State is J000600's fixture state: the currently-materialized engine row
// (Outline) and its --target dir.
type j000600State struct {
	engine string
	target string
}

func registerJ000600Steps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^Carol materializes the "([^"]*)" profile for (\S+)$`, func(c context.Context, profile, engine string) error {
		w := worldFrom(c)
		j000600 := j000600Of(w)
		j000600.engine = engine
		j000600.target = "out-" + engine
		return runOK(w, "profile", "materialize", profile, "--target", j000600.target, "--backend", engine)
	})
