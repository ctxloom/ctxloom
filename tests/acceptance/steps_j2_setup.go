...
// Distinctive marker strings j2_setup.feature's fragment-carrying sources
...
const (
	j2PersonalMarker   = "J2-PERSONAL-REPO-CONTEXT-MARKER"
	j2CompanyMarker    = "J2-COMPANY-REPO-CONTEXT-MARKER"
	j2ThirdPartyMarker = "J2-THIRDPARTY-REPO-CONTEXT-MARKER"
	j2HeldFirstMarker  = "J2-HELD-FIRST-CONTEXT-MARKER"
	j2HeldSecondMarker = "J2-HELD-SECOND-CONTEXT-MARKER"
	j2LivePersonalMark = "J2-LIVE-PERSONAL-MARKER-PHRASE"
	j2LiveCompanyMark  = "J2-LIVE-COMPANY-MARKER-PHRASE"
)

func registerJ2SetupSteps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^her personal ctxloom repository is signed with her own key$`, func(c context.Context) error {
		...
		_, err := seedSource(w, "personal", "fragments", "marker", j2PersonalMarker, fragmentSourceYAML(j2PersonalMarker), true, true)
		return err
	})

	ctx.Step(`^her company's ctxloom repository is signed with the company key, which Alice trusts$`, func(c context.Context) error {
		w := worldFrom(c)
		_, err := seedSource(w, "company", "fragments", "marker", j2CompanyMarker, fragmentSourceYAML(j2CompanyMarker), true, true)
		return err
	})
	...
	ctx.Step(`^her personal repository's context is part of her configuration, because it is signed with her own key$`, func(c context.Context) error {
		return assertMaterializedContains(c, "out", j2PersonalMarker)
	})
	...
	ctx.Step(`^her company repository's context is part of her configuration, because she trusts the company key$`, func(c context.Context) error {
		return assertMaterializedContains(c, "out", j2CompanyMarker)
	})
	...
	ctx.Step(`^the restarted mock engine receives her personal repository's fragments$`, func(c context.Context) error {
		...
		// "Alice accepts the restart" (whichever step set w.j2RestartRecorded);
		...
		w.docStepMaterialized = j4Excerpt(w.j2RestartRecorded, j2PersonalMarker, 2)
		if !strings.Contains(w.j2RestartRecorded, j2PersonalMarker) {
			return fmt.Errorf("mock's recorded input does not contain the personal repository's marker; recorded:\n%s", w.j2RestartRecorded)
		}
		return nil
	})
	...
	ctx.Step(`^the restarted mock engine receives her company repository's fragments$`, func(c context.Context) error {
		w := worldFrom(c)
		w.docStepMaterialized = j4Excerpt(w.j2RestartRecorded, j2CompanyMarker, 2)
		if !strings.Contains(w.j2RestartRecorded, j2CompanyMarker) {
	...
	ctx.Step(`^her personal and company repositories are trusted, signed sources$`, func(c context.Context) error {
		...
		if _, err := seedSource(w, "personal", "fragments", "marker", j2LivePersonalMark, fragmentSourceYAML(j2LivePersonalMark), true, true); err != nil {
			return err
		}
		_, err := seedSource(w, "company", "fragments", "marker", j2LiveCompanyMark, fragmentSourceYAML(j2LiveCompanyMark), true, true)
		return err
	})
	...
	// The four Then/And steps below used to re-guard on w.j2Live
	...
	// reach one with w.j2Live still false, silently re-skipping would hide
	...
	ctx.Step(`^Alice has completed setup and restarted into her configured session$`, func(c context.Context) error {
		w := worldFrom(c)
		if !w.j2Live {
			return fmt.Errorf("j2: reached this step with w.j2Live still false -- the scenario's own live-agent Given should have skipped the whole scenario before this ran")
		}
		if err := addSourceAsRemote(w, "personal", "default"); err != nil {
			return err
		}
	...
	ctx.Step(`^its reply contains her personal repository's marker$`, func(c context.Context) error {
		...
		if !strings.Contains(w.env.LastOutput(), j2LivePersonalMark) {
	...
	ctx.Step(`^its reply contains her company repository's marker$`, func(c context.Context) error {
		...
		if !strings.Contains(w.env.LastOutput(), j2LiveCompanyMark) {
	...
	ctx.Step(`^a third-party ctxloom repository whose content is (.+)$`, func(c context.Context, trustState string) error {
		...
		switch trustState {
		case "unsigned":
			_, err := seedSource(w, "thirdparty", "fragments", "marker", j2ThirdPartyMarker, fragmentSourceYAML(j2ThirdPartyMarker), false, false)
			return err
		case "signed with a key Alice does not trust":
			_, err := seedSource(w, "thirdparty", "fragments", "marker", j2ThirdPartyMarker, fragmentSourceYAML(j2ThirdPartyMarker), true, false)
			return err
		default:
	...
	ctx.Step(`^her assistant does not receive that repository's content$`, func(c context.Context) error {
		...
		if strings.Contains(body, j2ThirdPartyMarker) {
	...
	ctx.Step(`^two sources are held for Alice's review$`, func(c context.Context) error {
		...
		if _, err := seedSource(w, "first", "fragments", "marker", j2HeldFirstMarker, fragmentSourceYAML(j2HeldFirstMarker), false, false); err != nil {
			return err
		}
		if err := addSourceAsRemote(w, "first", "default"); err != nil {
			return err
		}
		if _, err := seedSource(w, "second", "fragments", "marker", j2HeldSecondMarker, fragmentSourceYAML(j2HeldSecondMarker), false, false); err != nil {
			return err
		}
	...
	ctx.Step(`^she approves the first and rejects the second$`, func(c context.Context) error {
		w := worldFrom(c)
		first := w.j2Sources["first"]
		second := w.j2Sources["second"]
	...
	ctx.Step(`^her assistant receives the item she approved$`, func(c context.Context) error {
		...
		if !strings.Contains(body, j2HeldFirstMarker) {
	...
	ctx.Step(`^her assistant never receives the item she rejected$`, func(c context.Context) error {
		...
		w.docStepMaterialized = fmt.Sprintf(
			...
			j2HeldSecondMarker, j4Excerpt(body, j2HeldFirstMarker, 2))
		if strings.Contains(body, j2HeldSecondMarker) {
