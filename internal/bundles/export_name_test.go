...
// fragmentContent/promptContent must carry the owning bundle's loader name and
...
func TestLoader_LoadedContentCarriesBundleAndItem(t *testing.T) {
	...
	b := &Bundle{
		...
		Prompts:   map[string]BundlePrompt{"review": {Content: "REVIEW"}},
	}
	...
	prompt, err := loader.GetPrompt(infos[0].Name)
	require.NoError(t, err)
