Feature: MCP server configuration
  ...
  Scenario: Add an MCP server and see it across axes
    ...
    When I run "ctxloom manage mcp servers add tools -c echo"
    Then the command succeeds
    When I run "ctxloom manage mcp servers list"
    Then the command succeeds
  ...
  Scenario: Show an MCP server's configuration
    ...
    When I run "ctxloom manage mcp servers show tools"
    Then the command succeeds
  ...
  Scenario: Remove an MCP server
    Given an initialized ctxloom project
    And I run "ctxloom manage mcp servers add tools -c echo"
    When I run "ctxloom manage mcp servers remove tools"
    Then the command succeeds
    When I run "ctxloom manage mcp servers list"
