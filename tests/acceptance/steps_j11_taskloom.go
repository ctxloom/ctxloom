...
// j11State is J11's fixture state: the name->harp mapping the tag-matching
...
func j11Of(w *World) *j11State {
	if w.j11 == nil {
		w.j11 = &j11State{harpByName: map[string]string{}, nameByHarp: map[string]string{}}
	}
	return w.j11
}

func registerJ11Steps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^taskloom lists tag query "([^"]*)"$`, func(c context.Context, query string) error {
		...
		j11.lastQueryHarps = harps
		j11.lastFailed = false
		return nil
	})
	...
	ctx.Step(`^the tag query result is exactly "([^"]*)"$`, func(c context.Context, wantCSV string) error {
		...
		got := make([]string, 0, len(j11.lastQueryHarps))
		for _, h := range j11.lastQueryHarps {
			name, ok := j11.nameByHarp[h]
			if !ok {
	...
	ctx.Step(`^the taskloom command's error output mentions "([^"]*)"$`, func(c context.Context, want string) error {
		w := worldFrom(c)
		j11 := j11Of(w)
		if !strings.Contains(j11.lastStderr, want) {
			return fmt.Errorf("taskloom stderr does not contain %q; stderr:\n%s", want, j11.lastStderr)
		}
		return nil
	})
	...
	ctx.Step(`^taskloom sets its status to "([^"]*)"$`, func(c context.Context, status string) error {
		w := worldFrom(c)
		j11 := j11Of(w)
		_, stderr, err := w.env.RunTaskloom(nil, "status", j11.harp, status)
		if err != nil {
	...
	ctx.Step(`^taskloom edits its text to "([^"]*)"$`, func(c context.Context, text string) error {
		w := worldFrom(c)
		j11 := j11Of(w)
		_, stderr, err := w.env.RunTaskloom(nil, "edit", j11.harp, text)
		if err != nil {
	...
	ctx.Step(`^the on-disk task log grew by exactly one line after each operation$`, func(c context.Context) error {
		w := worldFrom(c)
		j11 := j11Of(w)
		if len(j11.snapshots) < 2 {
			return fmt.Errorf("expected at least 2 snapshots to compare, got %d", len(j11.snapshots))
		}
		for i := 1; i < len(j11.snapshots); i++ {
			prev, cur := j11.snapshots[i-1], j11.snapshots[i]
	...
	ctx.Step(`^an independent re-fold reproduces the same task state$`, func(c context.Context) error {
		...
		again, err := j11.foldedTask(w)
		if err != nil {
			return err
		}
		if !reflect.DeepEqual(*j11.lastFold, *again) {
			return fmt.Errorf("independent re-fold differs:\nfirst: %+v\nagain: %+v", *j11.lastFold, *again)
		}
		return nil
	})
	...
	ctx.Step(`^the tool "([^"]*)" input schema mentions "([^"]*)"$`, func(c context.Context, toolName, want string) error {
		w := worldFrom(c)
		detail, err := j11ToolDetail(w, toolName)
		if err != nil {
			return err
		}
...
// trailing line dropped) and appends it to j11's running sequence of
...
func (j11 *j11State) snapshotTaskLog(w *World) error {
	...
	j11.snapshots = append(j11.snapshots, lines)
	return nil
}
...
func (j11 *j11State) foldedTask(w *World) (*tasks.Task, error) {
...
// j11ToolDetail fetches tools/list from the (already-connected) taskloom MCP
...
func j11ToolDetail(w *World, toolName string) (*testenv.ToolDetail, error) {
	if w.tlMCP == nil {
