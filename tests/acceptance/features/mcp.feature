Feature: MCP server configuration
  mcp manages the MCP servers ctxloom injects into backend settings. A server
  ...
  Scenario: Add an MCP server and see it across axes
    ...
    When I run "ctxloom mcp add tools -c echo"
    Then the command succeeds
    When I run "ctxloom mcp list"
    Then the command succeeds
  ...
  Scenario: Show an MCP server's configuration
    ...
    When I run "ctxloom mcp show tools"
    Then the command succeeds
  ...
  Scenario: Remove an MCP server
    Given an initialized ctxloom project
    And I run "ctxloom mcp add tools -c echo"
    When I run "ctxloom mcp remove tools"
    Then the command succeeds
    When I run "ctxloom mcp list"
