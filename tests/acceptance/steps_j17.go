...
// J17: the "incident" journey (j17_incident.feature) — a bad command ships and
// must be pulled. RESCOPED 2026-07-14: J15 (j15_corporate_signed.feature)
// already has LOCKED, green scenarios for single-developer retraction and
// company-key revocation, so this journey keeps only the two things J15
...
//
// Cast: Carol (team lead, reused from steps_j7_team.go's persona — this is
// exactly the "team lead shares a command" shape, just with a remote/signed
// bundle instead of first-party content), Bob (teammate, j7State.bobDir — a
// genuinely separate clone, NOT a new persona model per the task brief),
// Trent (the company/trusted publisher, reusing J15's signing primitives:
...
const (
	// j17Marker is the company's incident-runbook bundle's distinctive content
	// — the payload scenario 1 asserts reached (and, after retraction, no
	// longer reaches) Carol's and Bob's assembled context.
	j17Marker = "J17-INCIDENT-RUNBOOK-MARKER"
	// j17BundleName is the bundle name Trent retracts — must match the name
	// given in the feature's "Carol leads a team..." step.
	j17BundleName = "incident-runbook"
	// j17RetractReason is the publisher's own stated reason, asserted
	// verbatim in the "retracted by the publisher (<reason>)" wording
	// (operations/trust.go:159-177) that reaches both Carol's and Bob's
	// materialize output.
	j17RetractReason = "shipped an incorrect deploy step; do not use"
	// j17EmbeddedPrincipal is ctxloom's OWN compiled-in publisher principal
	...
	j17EmbeddedPrincipal = "ben+ctxloom@abbitt.me"
)

// j17State is this journey's fixture state.
...
// j17Of returns (lazily creating) this scenario's J17 fixture state.
...
// j17BundleYAML renders the company's bundle manifest carrying one fragment
// named "guidance" whose content is marker — mirrors j15BundleYAML exactly
// (same shape, new marker) rather than reusing J15's constant, since J17's
...
func j17BundleYAML(marker string) string {
...
func registerJ17Steps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^Carol leads a team that already uses the company's "([^"]*)" bundle$`, func(c context.Context, bundleName string) error {
		...
		// mirrors j7SetupProject's git-init/config/commit/push scaffold, but
		// with a claude-code engine (buildJ2Config) so materialize writes
		// CLAUDE.md, exactly the surface J15's mechanism assembles into.
		...
		if err := runOK(w, "bundle", "create", "seed", "-d", "J17 seed bundle"); err != nil {
			return err
		}
		if err := runOK(w, "profile", "create", "default", "-b", "seed", "-d", "J17 default profile"); err != nil {
			return err
		}

		// Trent's company publishes a SIGNED bundle — reuses J15's signing
		...
		url, err := w.env.SeedSignedRemote(map[string]string{rel: j17BundleYAML(j17Marker)}, []string{rel}, signer)
		if err != nil {
			return fmt.Errorf("seed signed company remote: %w", err)
		}
		j17.companyBare = strings.TrimPrefix(url, "file://")
		...
		if _, err := j7Git(w.env.ProjectDir, "push", "-u", "origin", "main"); err != nil {
			return err
		}
		...
		if !strings.Contains(body, j17Marker) {
	...
	ctx.Step(`^Bob has already pulled the team project and installed the bundle too$`, func(c context.Context) error {
		w := worldFrom(c)
		// Bob's OWN separate clone (j7State.bobDir, steps_j7_team.go) — a
	...
	ctx.Step(`^Trent retracts the bundle$`, func(c context.Context) error {
		...
		manifest := fmt.Sprintf(
			"version: 1\nretracted:\n  - type: bundle\n    name: %q\n    version: \"\"\n    reason: %q\n",
			j17BundleName, j17RetractReason)
		return w.env.AdvanceRemote(j17.companyBare, map[string]string{".ctxloom/content/manifest.yaml": manifest})
	})
	...
	ctx.Step(`^Bob runs his next routine sync$`, func(c context.Context) error {
		...
		j17.bobSyncOutput = w.j7().bobOutput
		if err := runBob(w, "profile", "materialize", "default", "--target", "out"); err != nil {
			return err
		}
		j17.bobMaterialized = w.j7().bobOutput
		return nil
	})
	...
	ctx.Step(`^Carol is told the bundle was retracted, and her assistant no longer receives it$`, func(c context.Context) error {
		...
		wantWarn := fmt.Sprintf("retracted by the publisher (%s)", j17RetractReason)
	...
	ctx.Step(`^Bob is told the bundle was retracted too, and his assistant no longer receives it either$`, func(c context.Context) error {
		...
		w.docStepMaterialized = j17.bobSyncOutput + "\n" + j17.bobMaterialized
		return nil
	})
	...
	ctx.Step(`^the company's remote becomes unreachable$`, func(c context.Context) error {
		...
		broken := j17.companyBare + ".unreachable"
		if err := os.Rename(j17.companyBare, broken); err != nil {
			return fmt.Errorf("break the company remote: %w", err)
		}
		j17.companyBare = broken
		return nil
	})
	...
	ctx.Step(`^Trent removes ctxloom's own publisher key from the project's trust store$`, func(c context.Context) error {
		...
		_ = w.env.Run("trust", "signer", "show", j17EmbeddedPrincipal)
		j17.embeddedShowBefore = w.env.LastOutput()
		...
		_ = w.env.Run("trust", "signer", "delete", j17EmbeddedPrincipal, "--project")
		j17.embeddedRemoveOutput = w.env.LastOutput()
		// "after" listing: still visible, now tagged locally distrusted.
		_ = w.env.Run("trust", "signer", "show", j17EmbeddedPrincipal)
		j17.embeddedShowAfter = w.env.LastOutput()
		w.docStepMaterialized = "$ ctxloom trust signer show " + j17EmbeddedPrincipal + "\n" + j17.embeddedShowBefore +
			"\n$ ctxloom trust signer delete " + j17EmbeddedPrincipal + " --project\n" + j17.embeddedRemoveOutput +
			"\n$ ctxloom signer show " + j17EmbeddedPrincipal + "\n" + j17.embeddedShowAfter
		return nil
	})
	...
	ctx.Step(`^ctxloom reports the key cannot be deleted but is now distrusted locally$`, func(c context.Context) error {
		...
		w.docStepMaterialized = "$ ctxloom trust signer delete " + j17EmbeddedPrincipal + " --project\n" + j17.embeddedRemoveOutput
		if strings.Contains(j17.embeddedRemoveOutput, "no entry for") {
			return fmt.Errorf("'signer remove' must no longer report a bare \"no entry for\" for the embedded principal — it has a real local-suppression effect now; output:\n%s", j17.embeddedRemoveOutput)
		}
		if !strings.Contains(j17.embeddedRemoveOutput, "cannot be deleted") {
			return fmt.Errorf("expected 'signer remove' to say the embedded key cannot be deleted; output:\n%s", j17.embeddedRemoveOutput)
		}
		if !strings.Contains(j17.embeddedRemoveOutput, "DISTRUSTED") {
	...
	ctx.Step(`^ctxloom's own signer listing shows that key, tagged embedded and locally distrusted$`, func(c context.Context) error {
		...
		// distrusted" tag.
		w.docStepMaterialized = "$ ctxloom trust signer show " + j17EmbeddedPrincipal + "   # before the removal attempt\n" + j17.embeddedShowBefore +
			"\n$ ctxloom signer show " + j17EmbeddedPrincipal + "   # after the removal attempt\n" + j17.embeddedShowAfter
		for _, out := range []string{j17.embeddedShowBefore, j17.embeddedShowAfter} {
			if !strings.Contains(out, j17EmbeddedPrincipal) {
