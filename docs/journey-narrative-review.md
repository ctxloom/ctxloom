...
The standard applied throughout is the one J001600 sets:
...
This reshapes the proposal work substantially. The old J000100 (MCP server
...
J000200 opens `Given Alice has a fresh project directory`. Almost nobody adopts a
...
Pieces of the answer exist scattered as surface tests — J000400 has one row proving
...
conflict. J000200 gestures at "her personal ctxloom repository" but immediately
...
J001000 proves every engine's native log becomes a canonical transcript you own.
...
distill` (excluded @live), and — the piece the old J001100 draft explicitly
...
J000400 proves one profile reaching four engines *simultaneously*. J000800 proves Bob
...
J001500, J001600, `review.feature` and `trust_surface.feature` between them prove the
...
don't each face the same list. J001600's content-hash-rebinding scenario is one
...
J002100 proves the privilege boundary. J002300 proves the bus carries words. Neither has
...
| Journey | Verdict |
|---|---|
| **J000200 setup** | **Strong.** Real actor, real goal, and the two-phase configure-then-restart arc is a genuine product truth most suites would have faked. Only flaw: it starts from a fresh directory, which is the least common real starting point (§1.2). |
| **J000300 source augmentation** | **Weak — not its own journey.** No protagonist of its own; it is three riders on J000200's setup interview. Every scenario is marked PROPOSED against unlanded work, so the file is RED by construction. The `b` suffix is itself the admission that nobody believed this was a journey. Fold into J000200. |
| **J000700 team authoring** | **Strong but thin.** Carol authors, Bob receives — a real loop, told three times (author, distill, change). The `@future` scenario about Carol's own live session is honest. Would benefit from the failure direction: Carol authors something malformed. |
| **J001500 corporate signed** | **Exemplary.** Named adversary, stated scope ("provenance not secrecy, there is no Eve"), and every scenario is a way the claim could be a lie. This and J001600 are the standard. |
| **J000800 onboarding** | **Exemplary.** "Cloning IS the onboarding" is a thesis, and the scenarios attack it: pinning, the trust gate surviving a fresh machine, degradation on a machine missing companions, a different engine. Best-argued journey in the suite. |
| **J000400 multi-engine** | **Strong.** The four-engine table is a claim about the world, and the preamble is careful to distinguish "we wrote the bytes" from "an engine read them". The hand-authored-file-survives scenario is one of the most valuable in the corpus. |
| **J002100 delegation** | **Strong mechanism, weak narrative — and it should say so.** Alice appears in the Background and never again; every scenario is `the agent calls tool "agent_run"`. This is an excellent *threat model*, not a journey. `trust_surface.feature` handles exactly this situation by declaring "this is a REFERENCE, not a journey" in its preamble; J002100 should do the same rather than keep a persona it doesn't use. |
| **J001700 incident** | **Strong, slightly mixed.** The multi-developer retraction and the remote-goes-dark scenarios are a real incident shape and state precisely what they decline to restate from J001500. The third scenario (embedded publisher key visibility) is a different story — trust-store honesty, not incident response — and sits oddly. |
| **J001800 guardrails** | **Strong and unusual.** It narrates the limit of the product's own claim, and the overclaim scenario ("ltk is a cooperative redirect, not a sandbox") turns an honesty commitment into an executable assertion. More journeys should do this. |
| **J002200 isolation** | **Weakest narrative-to-length ratio in the suite.** 289 lines, ~12 scenarios, and the actor's goal is "runs the mock agent under workspace worktree" — nobody's goal. It is a superb engineering matrix with the best honesty commentary in the repo, and it never states the story that motivates it: *I want to run a task I don't fully trust without it reaching my machine, my credentials, or my other work.* Add that opening and half the criticism dissolves; the scenarios themselves are excellent. |
| **J000600 agent skill** | **Medium — J000400's row promoted to a file.** Real fixture (the ctxloom-doctor skill), but the arc is "Carol materializes for five engines", which is J000400's outline with a different payload. Its distinct claim (an engine loads this on its own, by progressive disclosure, without the user invoking anything) is stated in the preamble and never tested. |
| **J002500 taskloom tags** | **Weak as a journey, excellent as a contract test.** Its own preamble says so: "proves the seam those tiers cannot see". No persona anywhere. The final scenario — an agent discovering the tag surface purely from what the MCP server advertises, with no prior knowledge — is the one genuine narrative in the file and is worth more than the rest combined. |
| **J001000 transcript capture** | **Strong promise, mechanical body.** "Every engine's native log becomes one transcript you own" is a claim a person cares about, and the honesty block is exemplary. But every scenario is `I run "ctxloom session backfill <harp>"`; the person who wanted the transcript never appears and never reads one (§1.4). |
| **J002600 worktree task store** | **Medium — a regression test with a good sentence on top.** "The finding dies with the worktree" is a real motivation crisply stated, and the exactly-one-store-file assertion is exactly right. Two scenarios; honest about being narrow. |
| **J002300 cross-engine delegation** | **Strong.** Names the differentiator, states its seam against J002100 precisely, and its `@wip` comment reporting a live-verified vendor limitation is a model of how to fail honestly in a test file. |
| **J001600 signing** | **Exemplary — the stated standard, earned.** Named actor with an ordinary setup ("he configures nothing ctxloom-specific"), an explicit scope seam, payload assertions that read bytes fresh off disk, and a product bug filed as a `@wip` scenario rather than hidden. |

**Named weak:** J000300 (fold into J000200), J002100 (relabel as reference), J002200 (missing its
own motivating story), J000600 (subsumed claim), J002500 (contract test, no persona),
J002600 (regression test, honestly narrow).

---

## 3. J000100–J001200, rewritten
...
### Declined outright: old J000100 — "One MCP server, every engineer's assistant"
...
— is J000400's thesis applied to MCP, and J000400 already carries a per-engine MCP row.
**Add two rows to J000400 rather than write J000100.** A journey whose whole arc is
...
### Declined as a standalone: old J001400 — "Joining a team that does not use claude-code"
...
### J000100 — Adopting ctxloom on a repo that already has a life
...
### J001400 — Why can't my assistant see this?
...
### J001900 — The four callbacks every session already depends on
...
### J001300 — Driving ctxloom from an editor
...
agent (for `acp client`). Real work; schedule after J000100–J001900.

### J001200 — Finding the session where you already solved this

**This is the retargeted J001100, not a renumbered J001200.** The old J001200 bolted
`session search` to `session watch` because both are session subcommands. They
are two people doing two unrelated things; the watch half moves to J002000.
...
session show <harp>`. Then the payoff the old J001100 declined: **`ctxloom run
...
same `=== Prompt ===` technique J000200 and J002200 already use — because "the command
...
### J002000 — Watching a delegated agent, and steering it before it finishes
...
terminates, which J002100 and J002300 already know how to spawn and stop. The three
...
silently never see them. J002100/J002300 already run there. That colocation is the
argument for putting all four in one journey; it is also why this one is
genuinely harder than J000100–J001900 and should be scheduled accordingly.

---

### Second tier — named, not specified

Worth writing, in this order, once the above land: **J000500 the solo developer**
(§1.3, user-scope vs project-scope content), **J000900 switching engines
...
last uncovered standalone tool) folds into J002500 as the gaps doc proposed — a
...
document and should carry the same disclaimer. So should J002100, which is a
...
notable: it is where J002200's *narrative* actually lives (a real engine, real
credentials, a real leak found by running kiro), and J002200's own comments say so.

**Should be absorbed into narratives (7).**

| File | Goes to | Why |
|---|---|---|
| `fault_tolerance.feature` | **J001400** | Five scenarios that *are* the diagnosis story, told with no person. |
| `doctor.feature` | **J001400** | Doctor is the diagnosis entry point; it currently proves marker vocabulary, not that anything gets diagnosed. |
| `manage.feature` | **J000100** | Contains the three best adoption-risk scenarios in the corpus (hand-written numbers preserved, unreadable statusline refused, uninstall keeps `.ctxloom`) filed under a utility heading. |
...
| `session.feature` | **J001200** | Its seeded-session fixture is J001200's fixture. |
| `run.feature` | **J001400** / **J000200** | `--dry-run` is a diagnostic; the deny_tools and empty-tag-selection scenarios are guardrail material. |

**Duplicates or dead (3).**

- `trust_cli.feature` — one scenario, "A per-item acceptance and a rejection are
  recorded", fully subsumed by J001600's accept/reject scenarios and
...
- `skill.feature` — half journey already (Alice authors a package, a tampered
  signature fails, a bundle skill beats a kiro builtin). Overlaps J000600
  substantially. Recommend merging J000600 into it rather than the reverse: the
  authoring arc is the story, the five-engine materialization is the matrix.

---

## 5. J001300 / J001100 / J002400 verdicts
...
thesis and it echoes J002100's correctly. Its per-member-record-file discipline
...
### J001100 memory — **wrong surface AND wrong scope. Rewrite as J001200.**
...
Do not retarget this draft. **Write J001200 (§3) instead**, which keeps the good
...
next command. Carried into J000100.
...
structurally blind to. J002200 and J000600 both already drive opencode as a first-class
engine, which makes the omission harder to defend now than when it was written.

**(d) It did not know J001300 was stale.** It calls out J001100's dead `ctxloom memory`
group and says nothing about J001300's dead `ctxloom map` — same class of defect,
