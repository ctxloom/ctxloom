...
// J22: "the isolation seam's two axes actually land where they say they will"
// (j22_isolation.feature). The workspace axis (none|worktree) is proven via
...
// j22Record is one "Alice runs the mock agent under workspace ..." step's
...
// j22State is J22's fixture state: the scratch dir holding one record file per
...
func j22Of(w *World) *j22State {
	if w.j22 == nil {
		w.j22 = &j22State{recordDir: filepath.Join(w.env.Root, "j22-records")}
	}
	return w.j22
}

// j22ConfigYAML renders the PROJECT half of config.yaml for J22's fixture: one
...
// record path), so it is written once; j22HomeConfigYAML carries the part
// that changes per run (mirrors j21RenderConfig's direct config.yaml
...
// type. It moved to the HOME half (j22HomeConfigYAML): llm.configs.*.env is
...
// j22HomeConfigYAML renders the HOME half of config.yaml for J22's fixture:
...
// evidence — see j22ConfigYAML's doc for why this piece lives in HOME.
func j22HomeConfigYAML(recordFile string) string {
...
// j22ParseRecord extracts the cwd= and workdir= lines the mock's
// recordMockInput (internal/lm/backends/mock.go) wrote to its record file.
func j22ParseRecord(body string) (cwd, workDir string) {
...
func registerJ22Steps(ctx *godog.ScenarioContext) {
	ctx.Step(`^Alice has a git-backed project with a mock agent$`, func(c context.Context) error {
		...
		if err := os.MkdirAll(j22.recordDir, 0o755); err != nil {
		...
		// invoking `run`.
		if err := w.env.WriteHomeFile(".ctxloom/config.yaml", j22HomeConfigYAML(filepath.Join(j22.recordDir, "init.txt"))); err != nil {
			return err
		}
	...
	ctx.Step(`^Alice runs the mock agent under workspace "([^"]*)" with prompt "([^"]*)"$`, func(c context.Context, workspace, prompt string) error {
		w := worldFrom(c)
		j22 := j22Of(w)
		recPath := filepath.Join(j22.recordDir, fmt.Sprintf("record-%d.txt", len(j22.records)))
		...
		cwd, workDir := j22ParseRecord(string(body))
		j22.records = append(j22.records, j22Record{Workspace: workspace, Prompt: prompt, Cwd: cwd, WorkDir: workDir})
		return nil
	})
	...
	ctx.Step(`^the run's workdir reflects the "([^"]*)" workspace axis$`, func(c context.Context, workspace string) error {
		w := worldFrom(c)
		j22 := j22Of(w)
		if len(j22.records) == 0 {
			return fmt.Errorf("no run recorded yet")
		}
		last := j22.records[len(j22.records)-1]
		...
		default:
			return fmt.Errorf("j22: unknown workspace axis %q", workspace)
		}
		return nil
	})
	...
	ctx.Step(`^the two runs' workdirs are distinct and neither is under the project directory$`, func(c context.Context) error {
		w := worldFrom(c)
		j22 := j22Of(w)
		if len(j22.records) < 2 {
			return fmt.Errorf("expected at least 2 runs, got %d", len(j22.records))
		}
		a, b := j22.records[len(j22.records)-2], j22.records[len(j22.records)-1]
	...
	ctx.Step(`^Alice runs the container-bound agent with flags "([^"]*)"$`, func(c context.Context, flags string) error {
		...
		if rt, _, _ := containercell.Select(c, "j22's container fail-loud row"); rt.Available {
			fmt.Printf("SKIPPED (j22 container fail-loud row): a container runtime IS reachable here, so a requested container legitimately launches; this row asserts the CANNOT-launch contract\n")
		...
		j22.lastContainerRecPath = recPath
	...
	ctx.Step(`^the run runs on the host$`, func(c context.Context) error {
		...
		if j22.lastContainerRecPath == "" {
			return fmt.Errorf("no container-bound run was recorded yet")
		}
		body, err := os.ReadFile(j22.lastContainerRecPath)
		if err != nil {
			return fmt.Errorf("read mock record %q: %w", j22.lastContainerRecPath, err)
		}
		_, workDir := j22ParseRecord(string(body))
