...
// exactly like steps_j000800_onboarding.go already does for reprise, never a real
...
// steps_j000800_onboarding.go's own reprise fixture (j000800RepriseMarker) already
// established.
//
...
const (
	// j001800Target is the materialize destination for Alice's scenarios (1 & 2) —
	// mirrors "Alice starts a session"'s own hardcoded "out" (steps_j000200_setup.go).
	j001800Target = "out"

	// j001800ReuseMarker stands in for reprise's reuse-before-you-write guidance.
	...
	j001800LtkHeading       = "# llm-tool-killer (ltk)"
	j001800LtkRedirectClaim = "ltk is a cooperative redirect, not a sandbox"
)
...
// binary literally named bin — shared by J000800 (steps_j000800_onboarding.go, "the
// reprise companion is not installed") and J001800 (below, ltk), consolidated
...
func j001800ScrubFromPath(w *World, bin string) error {
...
// j001800LtkGuardedTools is the tool classes the scenario sentence "run ltk on
...
// tool — left all three j001800 scenarios green. Deriving the expectation from
...
type j001800ClaudeSettings struct {
	Hooks struct {
		PreToolUse []struct {
...
func registerJ001800Steps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^ltk and reprise are installed on Alice's machine$`, func(c context.Context) error {
		...
		// first (see j001800ScrubFromPath's doc) so the fake always wins deterministically.
		if err := j001800ScrubFromPath(w, "ltk"); err != nil {
			return err
		}
		if err := j001800ScrubFromPath(w, "reprise"); err != nil {
			return err
		}
		ltkYAML, err := j001800LtkLoadoutYAML()
		if err != nil {
			return err
		}
	...
	ctx.Step(`^her assistant receives ltk's task-runner guidance$`, func(c context.Context) error {
		w := worldFrom(c)
		body, err := materializeDefault(w, j001800Target)
		if err != nil {
			return err
		}
		w.docStepMaterialized = filepath.Join(j001800Target, "CLAUDE.md") + ":\n" + j000400Excerpt(body, j001800LtkHeading, 4)
		if !strings.Contains(body, j001800LtkHeading) {
			return fmt.Errorf("assembled context does not contain ltk's real fragment (looked for %q); content:\n%s", j001800LtkHeading, body)
		}
		return nil
	})
	...
	ctx.Step(`^her assistant receives reprise's reuse-before-you-write guidance$`, func(c context.Context) error {
		...
		w.docStepMaterialized = filepath.Join(j001800Target, "CLAUDE.md") + ":\n" + j000400Excerpt(body, j001800ReuseMarker, 2)
		if !strings.Contains(body, j001800ReuseMarker) {
			return fmt.Errorf("assembled context does not contain reprise's reuse-discipline marker %q; content:\n%s", j001800ReuseMarker, body)
		}
		return nil
	})
	...
	ctx.Step(`^her assistant's engine is configured to run ltk on every shell command and file edit$`, func(c context.Context) error {
		w := worldFrom(c)
		if _, err := materializeDefault(w, j001800Target); err != nil {
			return err
		}
		rel := filepath.Join(j001800Target, ".claude", "settings.json")
		raw, err := w.env.ReadFile(rel)
		if err != nil {
			return fmt.Errorf("read generated %s: %w", rel, err)
		}
		w.docStepMaterialized = rel + ":\n" + raw
		var settings j001800ClaudeSettings
		...
		var missing []string
		for _, tool := range j001800LtkGuardedTools {
			if !covered[tool] {
	...
	ctx.Step(`^the "ltk" companion is not installed on Bob's machine$`, func(c context.Context) error {
		// Only the "not installed" arm is implemented: no J001800 scenario needs
		// ltk "installed" on Bob's machine (scenario 1 already proves the
		// present-companion contribution, on Alice) — mirrors J000800's own "not
		// installed" behavior for reprise (scrub only, no fake install).
		return j001800ScrubFromPath(worldFrom(c), "ltk")
	})
	...
	// J000800's own reprise-envelope Given), "Bob starts a session", "his
	...
	// registered by steps_j000800_onboarding.go — reused verbatim in
	// j001800_guardrails.feature, no new registration here (godog rejects an
