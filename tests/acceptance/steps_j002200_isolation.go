...
func registerJ002200Steps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^Alice runs the container-bound agent with flags "([^"]*)"$`, func(c context.Context, flags string) error {
		...
		if rt, _, _ := containercell.Select(c, "j002200's container fail-loud row"); rt.Available {
			fmt.Printf("SKIPPED (j002200 container fail-loud row): a container runtime IS reachable here, so a requested container legitimately launches; this row asserts the CANNOT-launch contract\n")
