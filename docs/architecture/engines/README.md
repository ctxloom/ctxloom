...
5. ~~**An unprofiled backend's container inherits claude's credentials.**~~ — **RESOLVED `a6d9bd95`.** The `default:` arm of `engineContainerSpecFor` (renamed from `containerProfileFor`) returned `resolveClaudeContainerAuth` for any unrecognized engine — and a generic `acp` backend is registered. It now fails closed, and `runtime: container` for an engine with no auth mapping is refused when the binding is *written*, not when it is launched. → [isolation](isolation.md)
