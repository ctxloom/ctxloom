...
// — vital-tiger v2) against the REAL backend Surfaces (claude, and — parked,
// see below — codex/kiro), proving the per-provider dispatch tables (S2)
// integrate correctly with the generic builder BEFORE any caller is wired
// onto it (materialize/apply/remove/launch are migrated separately, plan
// S4). It is an EXTERNAL test package (agent_test, not agent) because
// internal test files cannot import a package that itself imports the
// package under test — claude/codex/kiro all import internal/shared/agent,
// so this file must live outside it to avoid the Go toolchain's "import
// cycle not allowed in test" restriction.
//
// parked_engines: codex/kiro are out of the default build. Four tests below
// are commented out with them — each pins a SHAPE (hook-only context,
// native-file-only context rejecting hook, no SharedRealization for any
// surface) no currently-registered backend (claude, mock, acp) shares, so
// there is no honest substitute; they come back with the packages.
