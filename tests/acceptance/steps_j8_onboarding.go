...
// J8: "a new engineer clones the repo and is already set up" journey
// (j8_onboarding.feature). Bob is a new teammate; Carol's checkout
// (w.env.ProjectDir) plays the team's shared project exactly as it does in
// steps_j7_team.go, whose two-checkout plumbing (bare origin + Bob's own
// clone directory, driven via exec.Cmd against the ctxloom binary) this file
// reuses DIRECTLY — j7SetupTeamProject/j7CommitAndPush/j7BobPull/runBob/
// readBobFile all operate on the same generic "team origin + Bob's clone"
// state (w.j7()), which is not actually J7-specific despite the field's name.
...
// bookkeeping (j8s *j8State — signers/URLs/markers that cross step
// boundaries), mirroring J7/J15's one-field convention.
package acceptance
...
const (
	j8TeamMarker    = "J8-TEAM-STANDARDIZED-CONTEXT-MARKER"
	j8PinnedMarker  = "J8-PINNED-UPSTREAM-VERSION-MARKER"
	j8NewerMarker   = "J8-NEWER-UPSTREAM-VERSION-MARKER"
	j8CompanyMarker = "J8-TRENT-COMPANY-CONTENT-MARKER"
	j8RepriseMarker = "J8-REPRISE-DEPENDENT-GUIDANCE-MARKER"

	j8UpstreamPrincipal = "upstream@example.com"
	j8CompanyPrincipal  = "trent@example.com"
	j8ReprisePrincipal  = "reprise@example.com"

	j8UpstreamBundle = "upstream-bundle"
	j8CompanyBundle  = "secure-standards"
)

// j8State is this journey's fixture state: the signers/URLs/bundle names its
...
// j8LockedSHA reads the lockfile at rel (project-relative) and returns the
...
// nothing.
func j8LockedSHA(raw string) (string, error) {
	var lock struct {
	...
	for ref, entry := range lock.Bundles {
		if strings.Contains(ref, j8UpstreamBundle) {
			if entry.SHA == "" {
	...
	return "", fmt.Errorf("lockfile has no entry for the upstream bundle %q:\n%s", j8UpstreamBundle, raw)
}

// j8 returns (lazily allocating) this scenario's J8 fixture state.
func (w *World) j8() *j8State {
	if w.j8s == nil {
		w.j8s = &j8State{}
	}
	return w.j8s
...
func registerJ8Steps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^the team's project carries the context Carol has standardized on$`, func(c context.Context) error {
		...
		if err := w.env.WriteFile(".ctxloom/content/bundles/"+j7Bundle+".yaml", j7FragmentBundleYAML(j8TeamMarker)); err != nil {
			return err
		}
		...
		return j7CommitAndPush(w, "author team standards")
	})
	...
	ctx.Step(`^Bob starts a session$`, func(c context.Context) error {
		return runBob(worldFrom(c), "profile", "materialize", j7Profile, "--target", "out")
	})

	// --- Multi-engine outline: Bob's engine may not be Alice's ------------
	//
	// Reuses J4's engine-axis machinery (engineContextRelPath, steps_j4.go)
	// rather than re-deriving a second per-engine path table — the same three
	// engines J4's materialization outline proved (claude-code, kiro,
	...
	ctx.Step(`^Bob starts a session on (\S+)$`, func(c context.Context, engine string) error {
		...
		j8 := w.j8()
		j8.engine = engine
		j8.target = "out-" + engine
		return runBob(w, "profile", "materialize", j7Profile, "--target", j8.target, "--backend", engine)
	})
	...
	ctx.Step(`^his assistant receives the team's standardized context in (\S+)'s own native surface$`, func(c context.Context, engine string) error {
		w := worldFrom(c)
		j8 := w.j8()
		rel, err := engineContextRelPath(j8.target, engine)
		if err != nil {
			return err
		}
		...
		w.docStepMaterialized = fmt.Sprintf("%s:\n%s", rel, j4Excerpt(body, j8TeamMarker, 1))
		if !strings.Contains(body, j8TeamMarker) {
	...
	ctx.Step(`^his assistant receives the team's standardized context$`, func(c context.Context) error {
		return assertBobMaterializedContains(worldFrom(c), j8TeamMarker)
	})
	...
	ctx.Step(`^he was not asked to configure anything to get it$`, func(c context.Context) error {
		...
		// steps_j7_team.go's "it reached him without any review" check.
		//
	...
	ctx.Step(`^the project pins the versions of the context it draws from elsewhere$`, func(c context.Context) error {
		...
		j8.upstreamSigner = signer
		rel := ".ctxloom/content/bundles/" + j8UpstreamBundle + ".yaml"
		url, err := w.env.SeedSignedRemote(map[string]string{rel: j7FragmentBundleYAML(j8PinnedMarker)}, []string{rel}, signer)
		if err != nil {
			return fmt.Errorf("seed signed upstream remote: %w", err)
		}
		j8.upstreamBare = strings.TrimPrefix(url, "file://")
		// Trust is PROJECT-scoped (committed .ctxloom/allowed_signers) so Bob
		// inherits it by cloning — exactly like J2/J15's own signed sources.
		if err := w.env.TrustSigner(signer, j8UpstreamPrincipal, true); err != nil {
			return fmt.Errorf("trust upstream signer: %w", err)
		}
		if err := runOK(w, "remote", "create", "upstream", url, "--forge", "git"); err != nil {
			return err
		}
		if err := runOK(w, "profile", "modify", j7Profile, "--add-bundle", "upstream/"+j8UpstreamBundle); err != nil {
			return err
		}
		...
		if err := runOK(w, "bundle", "hold", "upstream/"+j8UpstreamBundle); err != nil {
			return err
		}
		...
		if err != nil {
			return fmt.Errorf("read the team's lockfile after holding %s: %w", j8UpstreamBundle, err)
		}
		sha, err := j8LockedSHA(raw)
		if err != nil {
			return err
		}
		j8.pinnedSHA = sha
		return j7CommitAndPush(w, "pin upstream dependency")
	})
	...
	ctx.Step(`^an upstream has since published a newer version$`, func(c context.Context) error {
		...
		j8 := w.j8()
		rel := ".ctxloom/content/bundles/" + j8UpstreamBundle + ".yaml"
		return w.env.AdvanceSignedRemote(j8.upstreamBare, map[string]string{rel: j7FragmentBundleYAML(j8NewerMarker)}, []string{rel}, j8.upstreamSigner)
	})
	...
	// — so a hold no longer freezes SHA or version at all — left all eleven j8
	...
	ctx.Step(`^the project references a bundle published by Trent's company$`, func(c context.Context) error {
		...
		j8.companySigner = signer
		rel := ".ctxloom/content/bundles/" + j8CompanyBundle + ".yaml"
		url, err := w.env.SeedSignedRemote(map[string]string{rel: j7FragmentBundleYAML(j8CompanyMarker)}, []string{rel}, signer)
		if err != nil {
		...
		if err := runOK(w, "profile", "modify", j7Profile, "--add-bundle", "trent-company/"+j8CompanyBundle); err != nil {
			return err
		}
		...
		return j7CommitAndPush(w, "reference trent's company bundle")
	})
	...
	ctx.Step(`^Bob has cloned the project and the company's content is held for his review$`, func(c context.Context) error {
		...
		return assertBobMaterializedDoesNotContain(w, j8CompanyMarker)
	})

	ctx.Step(`^Bob trusts the company key$`, func(c context.Context) error {
		w := worldFrom(c)
		j8 := w.j8()
		pubBytes := ssh.MarshalAuthorizedKey(j8.companySigner.Public)
		...
		// gate opening the ordinary way, exactly like J15's "Alice trusts the
		// company key".
		return runBob(w, "trust", "signer", "create", j8CompanyPrincipal, "--key", keyPath, "--project")
	})

	ctx.Step(`^his assistant receives the company's content$`, func(c context.Context) error {
		return assertBobMaterializedContains(worldFrom(c), j8CompanyMarker)
	})
	...
	ctx.Step(`^the team's context includes guidance for the "reprise" companion$`, func(c context.Context) error {
		...
		bundleYAML := j7FragmentBundleYAML(j8RepriseMarker)
		...
		envelope, err := signing.EncodeLoadoutEnvelope([]byte(bundleYAML), sig, j8ReprisePrincipal)
		if err != nil {
			return fmt.Errorf("encode reprise loadout envelope: %w", err)
		}
		j8.repriseEnvelope = string(envelope)
		j8.repriseVersionJSON = `{"name":"reprise","version":"9.9.9-j8-fake"}`
		...
		if err := w.env.TrustSigner(signer, j8ReprisePrincipal, true); err != nil {
			return fmt.Errorf("trust reprise signer: %w", err)
		}
		return j7CommitAndPush(w, "trust the reprise companion's publisher key")
	})
	...
	ctx.Step(`^the "reprise" companion is (installed|not installed) on Bob's machine$`, func(c context.Context, presence string) error {
		...
		// deterministic.
		if err := j18ScrubFromPath(w, "reprise"); err != nil {
			return err
		}
		...
		j8 := w.j8()
		return w.env.InstallFakeCompanion("reprise", j8.repriseVersionJSON, j8.repriseEnvelope)
	})
	...
	ctx.Step(`^the reprise-dependent guidance (reaches|does not reach) his assistant$`, func(c context.Context, reaches string) error {
		w := worldFrom(c)
		if reaches == "reaches" {
			return assertBobMaterializedContains(w, j8RepriseMarker)
		}
		return assertBobMaterializedDoesNotContain(w, j8RepriseMarker)
	})
	...
	ctx.Step(`^nothing fails because of the companion's presence or absence$`, func(c context.Context) error {
		...
		// runs, "Bob starts a session" already consumed w.j7().bobOutput via
...
// returns the resulting CLAUDE.md — the payload every J8 Then step checks.
...
// need to inspect). Mirrors steps_j2_setup.go's materializeDefault
