<!--
J22 narration + outcome-matrix companion.
...
j2_setup.doc.md / j4_multi_engine.doc.md / j21_delegation.doc.md split on, for
the scenarios added by the isolation-matrix task. j22's three PRE-EXISTING
...
named cucumber scenario in this file's feature (steps_j22_isolation_matrix.go)
...
1. **The generic fail-loud/degrade CONTRACT** (any container that cannot
   ...
   engine-agnostically, by j22's own pre-existing "Requesting a container with
...
- **What would go red if the leak closed**: `TestAcceptance/A_worktree_run_for_kiro_isolates_only_session_state_without_an_API_key...`
  (steps_j22_isolation_matrix.go, via j22_isolation.feature) asserts the
...
- **What would go red if either escape closed**: `TestAcceptance/A_worktree_run_for_antigravity_refuses_to_start`
  (steps_j22_isolation_matrix.go, via j22_isolation.feature) asserts the fatal
...
- **EXECUTED, hermetically, every `just test-acceptance` run** (no live
  ...
  Scenario Outlines/Scenarios in `j22_isolation.feature`, confirmed green
...
- **Live vendor-drift detection** (does the REAL, currently-installed
  ...
  infrastructure (`live_engine_registry.go`, exercised by J2/J4's
