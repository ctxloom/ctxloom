...
Feature: The archaeologist — what did we decide in March?
  ...
  # NOTE ON SCOPE. J10 (j10_transcript_capture.feature) owns CAPTURE and proves
  ...
  Scenario: A phrase from the decision itself finds the session that made it
    When I run "ctxloom session search J12-ESSENCE-WORKTREE-NAMING-DECISION"
  ...
  Scenario: A phrase from the session summary finds it too
    When I run "ctxloom session search J12-SUMMARY-ONLY-MARKER"
  ...
  Scenario: A query that matches nothing says so rather than printing nothing
    When I run "ctxloom session search J12-MATCHES-ABSOLUTELY-NOTHING"
  ...
  #      marker changed (J12-TRANSCRIPT-ONLY-MARKER -> the essence marker)
  ...
  Scenario: The assistant can reach the March session without being told where to look
    ...
    Then the tool result contains "J12-ESSENCE-WORKTREE-NAMING-DECISION"
