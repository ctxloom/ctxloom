...
| [transcript.md](transcript.md) | `internal/transcript` + `internal/transcript/vendorreader/{,codex,claude,kiro,antigravity}` | ctxloom's own canonical conversation record: the versioned append-only JSONL schema, the writer every capture path shares, the reader, and the four vendor-format importers |
