...
//
// Reuses J1's signing test helpers (tests/integration/testenv/signing_acceptance.go:
// TestSigner, SeedSignedRemote, TrustSigner, AdvanceSignedRemote) and J1's
// scaffolding helpers (steps_j1_common.go: ensureProjectWithEngine, runOK) rather
// than rebuilding trust infrastructure that already exists. World carries exactly
// one new field (j3 *j3State) to hold this journey's fixture state.
package acceptance
...
// proxy.
const (
	j3CompanyMarker  = "J3-COMPANY-SECURECODING-MARKER"
	j3TamperedMarker = "J3-TAMPERED-CONTENT-MARKER"
	j3ExtraMarkerA   = "J3-COMPANY-EXTRA-A-MARKER"
	j3ExtraMarkerB   = "J3-COMPANY-EXTRA-B-MARKER"
	j3HookMarker     = "echo J3-HOOK-EXEC-MARKER"
	j3MCPMarker      = "J3-MCP-EXEC-MARKER"
	j3ForgeryMarker  = "J3-FORGERY-BYSTANDER-MARKER"
)

// j3State is this journey's fixture state: the company's signed bundle
...
// j3Of returns (lazily creating) this scenario's J3 fixture state.
func j3Of(w *World) *j3State {
	if w.j3 == nil {
		w.j3 = &j3State{bundleName: "secure-coding"}
	}
	return w.j3
}

// j3BundleYAML renders a bundle manifest carrying one fragment named
...
func j3BundleYAML(marker string) string {
	return fmt.Sprintf("version: \"1.0.0\"\nfragments:\n  guidance:\n    content: %q\n", marker)
}

// j3BundleYAMLWithExec is j3BundleYAML plus an MCP server and a session-start
...
func j3BundleYAMLWithExec(marker string) string {
	return fmt.Sprintf(
		"version: \"1.0.0\"\nfragments:\n  guidance:\n    content: %q\n"+
			"mcp:\n  demo-server:\n    command: %q\n    args: [%q]\n"+
			"hooks:\n  session_start:\n    - command: %q\n      type: command\n",
		marker, "/bin/echo", j3MCPMarker, j3HookMarker)
}

// j3WireReference adds the company's remote and references its bundle from
// the "default" profile — WITHOUT pulling. Idempotent: a second call is a
// no-op. Split from j3EnsureReferenced so the tamper scenario (which has no
// antecedent "Alice references..." step) can wire the reference before
// Mallory's tampered commit exists, and only pull afterward.
func j3WireReference(w *World) error {
	j3 := j3Of(w)
	if j3.referenced {
		return nil
	}
...
// j3EnsureReferenced wires the reference if needed, then ALWAYS pulls — the
...
func j3EnsureReferenced(w *World) error {
	if err := j3WireReference(w); err != nil {
		return err
	}
	return runOK(w, "remote", "pull")
}

// j3StartSession is "Alice starts a session": ensure the company bundle is
...
// Then steps, mirroring steps_j1_setup.go's materializeDefault).
func j3StartSession(w *World) error {
...
// j3ReadMaterialized reads out/CLAUDE.md, the assembled content surface.
func j3ReadMaterialized(w *World) (string, error) {
...
// j3AssertGeneratedSettingsContains/DoesNotContain check the GENERATED
...
func j3AssertMCPPresence(w *World, present bool) error {
	return j3AssertGeneratedFile(w, filepath.Join("out", ".mcp.json"), j3MCPMarker, present)
}

func j3AssertHookPresence(w *World, present bool) error {
	return j3AssertGeneratedFile(w, filepath.Join("out", ".claude", "settings.json"), j3HookMarker, present)
}

func j3AssertGeneratedFile(w *World, rel, marker string, present bool) error {
...
func registerJ3Steps(ctx *godog.ScenarioContext) {
	// --- Background ----------------------------------------------------------
	...
	ctx.Step(`^Trent's company publishes a "([^"]*)" bundle, signed with the company key$`, func(c context.Context, bundleName string) error {
		...
		url, err := w.env.SeedSignedRemote(map[string]string{rel: j3BundleYAML(j3CompanyMarker)}, []string{rel}, signer)
		if err != nil {
			return fmt.Errorf("seed signed company remote: %w", err)
		}
		j3.url = url
		j3.bare = strings.TrimPrefix(url, "file://")
		return nil
	})
	...
	ctx.Step(`^Alice trusts the company key$`, func(c context.Context) error {
		w := worldFrom(c)
		j3 := j3Of(w)
		j3.principal = "trent@example.com"
		pubBytes := ssh.MarshalAuthorizedKey(j3.signer.Public)
	...
	ctx.Step(`^Alice references the company's secure-coding bundle from her project$`, func(c context.Context) error {
		return j3EnsureReferenced(worldFrom(c))
	})

	// "Alice starts a session" is already registered by steps_j1_setup.go
	...
	// J3 scenario that reaches this step has already wired+pulled the company
	// bundle in its own preceding Given/When step (j3EnsureReferenced), so
	// nothing J3-specific needs to happen here.
	...
	ctx.Step(`^her assistant receives the company's secure-coding guidance, because the company key signed it$`, func(c context.Context) error {
		w := worldFrom(c)
		body, err := j3ReadMaterialized(w)
		if err != nil {
			return err
		}
		if !strings.Contains(body, j3CompanyMarker) {
	...
	ctx.Step(`^Mallory alters the company's secure-coding bundle after it was signed$`, func(c context.Context) error {
		w := worldFrom(c)
		j3 := j3Of(w)
		rel := ".ctxloom/content/bundles/" + j3.bundleName + ".yaml"
		...
		if err := w.env.AdvanceRemote(j3.bare, map[string]string{rel: j3BundleYAML(j3TamperedMarker)}); err != nil {
	...
	ctx.Step(`^her assistant does not receive the altered guidance$`, func(c context.Context) error {
		...
		if strings.Contains(body, j3TamperedMarker) {
	...
	ctx.Step(`^the company's bundle ships an MCP server and a hook$`, func(c context.Context) error {
		...
		j3 := j3Of(w)
		rel := ".ctxloom/content/bundles/" + j3.bundleName + ".yaml"
		if err := w.env.AdvanceSignedRemote(j3.bare, map[string]string{rel: j3BundleYAMLWithExec(j3CompanyMarker)}, []string{rel}, j3.signer); err != nil {
			return fmt.Errorf("advance remote with mcp+hook: %w", err)
		}
		return j3EnsureReferenced(w)
	})
	...
	ctx.Step(`^the MCP server appears in her assistant's configuration$`, func(c context.Context) error {
		return j3AssertMCPPresence(worldFrom(c), true)
	})

	ctx.Step(`^the hook appears in her assistant's configuration$`, func(c context.Context) error {
		return j3AssertHookPresence(worldFrom(c), true)
	})
	...
	ctx.Step(`^the hook is absent, because she rejected it$`, func(c context.Context) error {
		return j3AssertHookPresence(worldFrom(c), false)
	})
	...
	ctx.Step(`^Alice already receives the company's secure-coding guidance$`, func(c context.Context) error {
		w := worldFrom(c)
		if err := j3StartSession(w); err != nil {
			return err
		}
		body, err := j3ReadMaterialized(w)
		if err != nil {
	...
	ctx.Step(`^Trent retracts that version of the bundle$`, func(c context.Context) error {
		...
		manifest := fmt.Sprintf(
			"version: 1\nretracted:\n  - type: bundle\n    name: %q\n    version: \"\"\n    reason: %q\n",
			j3.bundleName, "found to be incorrect guidance; do not use")
		return w.env.AdvanceRemote(j3.bare, map[string]string{".ctxloom/content/manifest.yaml": manifest})
	})
	...
	ctx.Step(`^Alice receives several bundles the company signed with its key$`, func(c context.Context) error {
		w := worldFrom(c)
		j3 := j3Of(w)
		if err := j3EnsureReferenced(w); err != nil { // the background's "secure-coding" bundle
			return err
		}
		for _, extra := range []struct{ name, marker string }{
			{"extra-a", j3ExtraMarkerA},
			{"extra-b", j3ExtraMarkerB},
		} {
			rel := ".ctxloom/content/bundles/" + extra.name + ".yaml"
			url, err := w.env.SeedSignedRemote(map[string]string{rel: j3BundleYAML(extra.marker)}, []string{rel}, j3.signer)
			if err != nil {
			...
			j3.extraMarkers = append(j3.extraMarkers, extra.marker)
		}
		return runOK(w, "remote", "pull")
	})
	...
	ctx.Step(`^her assistant no longer receives any content signed by that key$`, func(c context.Context) error {
		...
		markers := append([]string{j3CompanyMarker}, j3Of(w).extraMarkers...)
	...
	ctx.Step(`^that content is held for her review, as if it had never been signed$`, func(c context.Context) error {
		...
		out := w.env.LastOutput()
		for _, want := range []string{j3Of(w).bundleName, "guidance", "new"} {
	...
	ctx.Step(`^Alice has no signing key available$`, func(c context.Context) error {
		...
		url, err := w.env.SeedRemote(map[string]string{rel: j3BundleYAML(j3ForgeryMarker)})
		if err != nil {
	...
	ctx.Step(`^Alice tries to record a review decision into the team's shared store$`, func(c context.Context) error {
		...
		// path at all, mirroring steps_j1_common.go's driveDiscoverySessionViaMock.
	...
	ctx.Step(`^ctxloom refuses, because a team decision must be signed$`, func(c context.Context) error {
		...
		if !strings.Contains(j3.reviewPTYOutput, "no signing key available") {
