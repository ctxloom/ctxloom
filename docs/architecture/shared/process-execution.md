...
- Exit classification propagates `exitErr.ExitCode()` verbatim, so a signal-killed child yields `-1` — not a valid POSIX status and indistinguishable from a runner-internal error.
