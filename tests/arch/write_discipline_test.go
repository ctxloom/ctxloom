...
var writeDisciplineAllowed = map[string]string{
	...
	"cmd/probe-mcp-server/main.go#server.record":                              "STRUCTURAL, not a deferred migration: this is an APPEND-ONLY evidence log and iox is the wrong tool for it. O_APPEND plus one Write is the correctness mechanism — a client may start the fixture server more than once, and append keeps each record whole across those writers. iox's atomic path is temp+fsync+rename, i.e. a REPLACE, which would discard every earlier record. iox.SyncDir's own doc names this exact case: \"an append-only log ... not an atomic replace, so Durable()'s Option shape does not fit\". Nothing to remove; if this ever needs durability it wants SyncDir directly, not WriteFileAtomic.",
