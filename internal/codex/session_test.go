...
// =============================================================================
// Session registration via hook input
// =============================================================================
...
func TestCodexSessionHistory_TranscriptPathFromHook(t *testing.T) {
	...
	assert.Equal(t, "/tmp/x.jsonl", h.TranscriptPathFromHook("/proj", "session-id", "/tmp/x.jsonl"))
	// Empty when codex reports none (transcript_path may be null).
	assert.Empty(t, h.TranscriptPathFromHook("/proj", "session-id", ""))
}
