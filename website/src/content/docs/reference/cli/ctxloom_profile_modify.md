---
title: "ctxloom profile modify"
---
<!-- GENERATED by `just gen-docs` from the command definitions in internal/cli.
     Do not edit; edit the cobra Short/Long/Example fields instead. -->
:::note
This page is generated from `ctxloom profile modify --help`.
:::

## ctxloom profile modify

Modify a profile's configuration

### Synopsis

Modify an existing profile by adding or removing items.

Examples:
  ctxloom profile modify go-developer --add-parent 'https://github.com/user/ctxloom@bundles/dev#profiles/developer'
  ctxloom profile modify developer --add-bundle https://github.com/user/ctxloom@bundles/go-development
  ctxloom profile modify developer -d "New description"

```
ctxloom profile modify <name> [flags]
```

### Options

```
      --add-bundle strings         Bundle URL(s) to add
      --add-parent strings         Parent profile(s) to add: a local name or <bundle>#profiles/<name> (bundle = canonical URL, remote/bundle alias, or local bundle name)
  -d, --description string         New description for the profile
      --exclude-fragment strings   Fragment name(s) to exclude
      --exclude-mcp strings        MCP server name(s) to exclude
  -h, --help                       help for modify
      --include-fragment strings   Fragment name(s) to stop excluding
      --include-mcp strings        MCP server name(s) to stop excluding
      --llm string                 Set the preferred LLM config label/backend (empty clears it)
      --remove-bundle strings      Bundle URL(s) to remove
      --remove-parent strings      Parent profile(s) to remove
```

### Options inherited from parent commands

```
      --degraded          degrade instead of failing: downgrade fatal startup findings (broken config, unresolvable profiles/bundles, failed hook applies) to warnings and launch anyway
      --format string     Output format: json, yaml, toml, text, or markdown (default "text")
      --no-companions     skip companion loadout discovery: do not execute companion binaries (ltk, taskloom, ...) or contribute their commands, hooks, MCP servers and context
      --set stringArray   override a config value for this invocation: --set <dotted.path>=<value> (repeatable; e.g. --set llm.defaults.primary=big, --set agents.MyCoder.runtime=container)
```

### SEE ALSO

* [ctxloom profile](/reference/cli/ctxloom_profile/)	 - Manage profiles (named fragment collections)

