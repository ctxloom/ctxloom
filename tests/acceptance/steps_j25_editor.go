...
// J25: "Dana in her editor" (j25_editor.feature) — FLOWS-UNIFIED.md's U10.
//
...
// j25State is this journey's fixture state.
...
const (
	// j25Marker is the context Dana's binding composes — the thing that must
	// still reach her when she works through an editor instead of a terminal.
	j25Marker = "J25-EDITOR-BINDING-MARKER"
	// j25Agent is the agent Zed would be pointed at.
	j25Agent = "dev"
)

// j25Setup is the Background: a project with an agent binding carrying real
// composed context, which is what any door into ctxloom is supposed to deliver.
func j25Setup(w *World) error {
	st := j25Of(w)
	if st.ready {
		return nil
	}
	...
	if err := runOK(w, "agent", "create", j25Agent, "--engine", "claude-code", "--profiles", "default"); err != nil {
		return err
	}
	st.ready = true
	return nil
