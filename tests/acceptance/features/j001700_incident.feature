...
Feature: An incident — a bad command ships and must be pulled
  ...
  # NOTE ON SCOPE: J001500 (j001500_corporate_signed.feature) already has LOCKED, green
  ...
  # any of those again here would restate a mechanism J001500 already proves — a
  # second green scenario asserting the same thing reads as coverage while
  # adding none. This journey keeps ONLY the things J001500 cannot express; see
  # each scenario's own comment for exactly what that is and what it deliberately
  # does not repeat.

  # LOCKED — retraction across MORE THAN ONE developer. J001500's retraction
  # scenario (j001500_corporate_signed.feature:69) is single-persona: only Alice
  ...
  # revocation, or rejection-beats-trust — all already LOCKED in J001500.
