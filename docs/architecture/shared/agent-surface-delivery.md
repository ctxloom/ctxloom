...
| `ResolvedSelection.deliverOneShared` | `internal/shared/agent/cells.go:571` | Prefers a backend's `SharedRealization`, else warns loudly and does the well-known write. |
...
- **`deliverOneShared` ignores `resolvedSurface.approach`.** If the backend offers a `SharedRealization` for the kind it wins unconditionally (`cells.go:575`). A caller naming `ContextWriteUnsafeFile` on claude therefore gets the system-prompt scratch write, not CLAUDE.md — the two named approaches are indistinguishable through the shared path. This diverges from the builder's documented claim (`cells.go:324-327`) that callers always name the executing approach.
...
- **`EmptySurfaceSet.SurfaceFor` returns `(nil, nil)`**, contradicting the `SurfaceSet` contract at `cells.go:126-128` ("errors on an unsupported combination"). It is unreached today only because `Build` short-circuits when `len(supported) == 0`.
