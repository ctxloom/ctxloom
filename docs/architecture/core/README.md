...
3. **Sole writers.**
   ...
   | `.ctxloom/lock.yaml` | `LockfileManager.write` (`internal/remote/lockfile.go:111`) — reached from `Save` (`:104`) and from the load-time self-heal in `Load` (`:66`). Callers: `Puller.updateLockfile`/`RecordRetraction` inside `internal/remote`, and `internal/operations/lockfile.go:147` through the `LockfileStore` port |
