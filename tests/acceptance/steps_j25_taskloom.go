...
// J25: "taskloom's tag surface is real across the process boundary"
// (j25_taskloom_tags.feature). taskloom's tag-query grammar, fold, and
...
// j25State is J25's fixture state: the name->harp mapping the tag-matching
...
func j25Of(w *World) *j25State {
	if w.j25 == nil {
		w.j25 = &j25State{harpByName: map[string]string{}, nameByHarp: map[string]string{}}
	}
	return w.j25
}

func registerJ25Steps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^taskloom has tasks:$`, func(c context.Context, table *godog.Table) error {
		...
		for _, row := range table.Rows[1:] { // skip header
			...
			j25.harpByName[name] = harpID
			j25.nameByHarp[harpID] = name
		}
		return nil
	})
	...
	ctx.Step(`^taskloom lists tag query "([^"]*)"$`, func(c context.Context, query string) error {
		...
		j25.lastQueryHarps = harps
		j25.lastFailed = false
		return nil
	})
	...
	ctx.Step(`^taskloom lists tag query "([^"]*)" expecting failure$`, func(c context.Context, query string) error {
		...
		j25.lastFailed = true
		j25.lastExitCode = exitCodeOf(err)
		j25.lastStderr = stderr
		return nil
	})
	...
	ctx.Step(`^the tag query result is exactly "([^"]*)"$`, func(c context.Context, wantCSV string) error {
		...
		got := make([]string, 0, len(j25.lastQueryHarps))
		for _, h := range j25.lastQueryHarps {
			name, ok := j25.nameByHarp[h]
			if !ok {
	...
	ctx.Step(`^the taskloom command's error output mentions "([^"]*)"$`, func(c context.Context, want string) error {
		w := worldFrom(c)
		j25 := j25Of(w)
		if !strings.Contains(j25.lastStderr, want) {
			return fmt.Errorf("taskloom stderr does not contain %q; stderr:\n%s", want, j25.lastStderr)
		}
		return nil
	})
	...
	ctx.Step(`^taskloom sets its status to "([^"]*)"$`, func(c context.Context, status string) error {
		w := worldFrom(c)
		j25 := j25Of(w)
		_, stderr, err := w.env.RunTaskloom(nil, "status", j25.harp, status)
		if err != nil {
	...
	ctx.Step(`^taskloom edits its text to "([^"]*)"$`, func(c context.Context, text string) error {
		w := worldFrom(c)
		j25 := j25Of(w)
		_, stderr, err := w.env.RunTaskloom(nil, "edit", j25.harp, text)
		if err != nil {
	...
	ctx.Step(`^the on-disk task log grew by exactly one line after each operation$`, func(c context.Context) error {
		w := worldFrom(c)
		j25 := j25Of(w)
		if len(j25.snapshots) < 2 {
			return fmt.Errorf("expected at least 2 snapshots to compare, got %d", len(j25.snapshots))
		}
		for i := 1; i < len(j25.snapshots); i++ {
			prev, cur := j25.snapshots[i-1], j25.snapshots[i]
	...
	ctx.Step(`^an independent re-fold reproduces the same task state$`, func(c context.Context) error {
		...
		again, err := j25.foldedTask(w)
		if err != nil {
			return err
		}
		if !reflect.DeepEqual(*j25.lastFold, *again) {
			return fmt.Errorf("independent re-fold differs:\nfirst: %+v\nagain: %+v", *j25.lastFold, *again)
		}
		return nil
	})
	...
	ctx.Step(`^the tool "([^"]*)" input schema mentions "([^"]*)"$`, func(c context.Context, toolName, want string) error {
		w := worldFrom(c)
		detail, err := j25ToolDetail(w, toolName)
		if err != nil {
			return err
		}
...
// trailing line dropped) and appends it to j25's running sequence of
// snapshots. Called once after every taskloom mutation in the append-only
// scenario.
func (j25 *j25State) snapshotTaskLog(w *World) error {
	lines, err := readTaskLogLines(w, false) // pre-mutation baseline: absence is legitimate
	if err != nil {
		return err
	}
	j25.snapshots = append(j25.snapshots, lines)
	return nil
}
...
func (j25 *j25State) foldedTask(w *World) (*tasks.Task, error) {
	...
	for i := range got {
		if got[i].HarpID == j25.harp {
			return &got[i], nil
		}
	}
	return nil, fmt.Errorf("task %s not found in %v", j25.harp, got)
}

// taskEvents reads the current on-disk task log and returns every event
// belonging to j25.harp, in file order — the raw seam the scalar-collapse
...
//
// This used to decode into a local j25RawEvent subset-of-Event
...
func (j25 *j25State) taskEvents(w *World) ([]tasks.Event, error) {
...
// j25ToolDetail fetches tools/list from the (already-connected) taskloom MCP
...
func j25ToolDetail(w *World, toolName string) (*testenv.ToolDetail, error) {
	if w.tlMCP == nil {
