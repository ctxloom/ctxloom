// Package agent is ctxloom's engine-agnostic agent core (in-repo staging for the
// future github.com/ctxloom/agent). It holds the contract and machinery every
// agent adapter shares: the settings-writing reconciler, the owner predicate,
// and the canonical marshaller. The per-engine wire formats live in the agent
// packages (claude/gemini); nothing here is engine-specific.
package agent
