...
func TestWriteManagedCommandFiles_DedupHomeDir(t *testing.T) {
	const manifest = ".ctxloom-manifest"
	...
	t.Run("identical_home_copy_skips_and_omits_from_manifest", func(t *testing.T) {
		...
		require.NoError(t, WriteManagedCommandFiles(fs, proj, manifest, cmds, renderNameContent, WithDedupHomeDir(home)))
		...
		man, err := afero.ReadFile(fs, filepath.Join(proj, manifest))
	...
	t.Run("divergent_home_copy_writes_normally", func(t *testing.T) {
		...
		man, _ := afero.ReadFile(fs, filepath.Join(proj, manifest))
	...
	t.Run("target_equal_to_home_never_self_dedups", func(t *testing.T) {
		...
		require.NoError(t, WriteManagedCommandFiles(fs, dir, manifest, cmds, renderNameContent, WithDedupHomeDir(dir)))

		man, err := afero.ReadFile(fs, filepath.Join(dir, manifest))
		require.NoError(t, err)
