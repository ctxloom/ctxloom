...
**Called in by:** `cmd/ctxloom` (override install, companion switch), `internal/cli` (~35 `Load` sites, `Manager.Update` writes, doctor, startup reporting), `internal/operations` (agent management, context assembly, profiles, trust, init), `internal/lm/backends` (the four `ResolveBundle*` resolvers, `ExecutableTrustGate`, statusline settings), `internal/agentcoord/coord` (trust gate injection before spawning children, `GetAgentTurnCap`), `tests/integration/testenv`.
