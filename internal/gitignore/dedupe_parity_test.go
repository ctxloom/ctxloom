...
// missingPatterns must dedupe within the requested set, not only against the
// FILE: filtering a duplicate-bearing request list against an EMPTY file is
// exactly "dedupe the request list". Without that, a caller list carrying a
// repeated pattern writes the same line to .gitignore twice, and a second
// map-based filter over the same []string is needed to compensate.
func TestMissingPatterns_DedupesWithinTheRequestedSet(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// filtering left is the within-set dedup.
