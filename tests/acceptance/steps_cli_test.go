...
// `ctxloom run --print 'write a file'` must produce ["run", "--print",
...
func TestCtxloomArgs_QuotedArgumentSurvivesAsOneWord(t *testing.T) {
	got, err := ctxloomArgs(`ctxloom run --print 'write a file'`)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	want := []string{"run", "--print", "write a file"}
...
func TestCtxloomArgs_DoubleQuotedArgumentSurvivesAsOneWord(t *testing.T) {
	got, err := ctxloomArgs(`ctxloom run --print "say \"hi\" now"`)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	want := []string{"run", "--print", `say "hi" now`}
	if !reflect.DeepEqual(got, want) {
		t.Fatalf("ctxloomArgs = %#v, want %#v", got, want)
	}
}
