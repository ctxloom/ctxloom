...
// `subagent-setup` skill supplies the setup prompt instead of the built-in —
...
`)
	...
	assert.Equal(t, "BUNDLE-SHIPPED-SETUP-PROMPT", got,
		"a bundle-shipped subagent-setup skill overrides the built-in prompt")
}

// TestResolveSetupPrompt_FallsBackToBuiltin proves the built-in is used when no
// bundle ships subagent-setup, and that a nil config is safe.
...
`)
	...
	assert.Equal(t, "BUILTIN", ResolveSetupPrompt(cfg, "BUILTIN"),
		"no subagent-setup skill installed → the built-in prompt")
