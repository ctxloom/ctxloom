...
func TestForgeAPI_GitHub_NotFoundMapsToSentinel(t *testing.T) {
	...
	stub.AddFile(".ctxloom/content/bundles/present.yaml", []byte("version: 1.0.0\n"))
	f := newGitHubFetcher(t, stub)
	ctx := context.Background()

	t.Run("FetchFile happy path round-trips content", func(t *testing.T) {
		data, err := f.FetchFile(ctx, "owner", "repo", ".ctxloom/content/bundles/present.yaml", "main")
		require.NoError(t, err)
		assert.Equal(t, "version: 1.0.0\n", string(data))
	})

	t.Run("FetchFile 404 -> ErrRemoteContentNotFound", func(t *testing.T) {
		_, err := f.FetchFile(ctx, "owner", "repo", ".ctxloom/content/bundles/missing.yaml", "main")
		require.Error(t, err)
	...
	t.Run("ListDir lists registered directory entries", func(t *testing.T) {
		entries, err := f.ListDir(ctx, "owner", "repo", ".ctxloom/content/bundles", "main")
		require.NoError(t, err)
		var names []string
...
func TestForgeAPI_Unauthorized(t *testing.T) {
	...
	t.Run("github", func(t *testing.T) {
		...
		f := newGitHubFetcher(t, stub)
		_, err := f.FetchFile(ctx, "owner", "repo", ".ctxloom/content/bundles/x.yaml", "main")
		require.Error(t, err)
