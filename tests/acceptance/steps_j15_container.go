...
// J15: "an engine asked for a container actually lands in one"
// (j15_container.feature) — the positive half of the runtime axis.
//
// j9_isolation.feature owns the NEGATIVE half (a requested container that
...
// evidence.
//
// The fixture is J9's, reused rather than copied: the same two agent bindings
// ("mock" on the host, "mock-container" with runtime: container) over the same
// mock LLM label. Only the record file's LOCATION differs and it has to: J9
// writes records beside the project, while a containerized engine can only
// write somewhere the container can see, so J15's records live INSIDE the
...
// j15Record is one run's evidence of WHERE the engine executed, read from the
...
// j15ParseRecord reads the two where-did-this-run lines out of a record body.
...
func j15ParseRecord(body string) j15Record {
	var rec j15Record
...
// j15RecordPath is this run's record file, inside the project workspace so a
...
func j15RecordPath(w *World, runtime string) string {
	return filepath.Join(w.env.ProjectDir, "j15-record-"+runtime+".txt")
}
...
func registerJ15Steps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^Alice runs the mock agent with runtime "([^"]*)"$`, func(c context.Context, runtime string) error {
		...
		if runtime == "container" {
			...
			if rt, _, msg := containercell.Select(c, "J15's containerized-run row"); !rt.Available {
				fmt.Printf("SKIPPED (J15 container row): no container runtime reachable here (%s)\n", msg)
		...
		// J9's fixture verbatim — same labels, same two agent bindings. See
		...
		recPath := j15RecordPath(w, runtime)
		...
		j15.last = j15ParseRecord(string(body))
		w.docStepMaterialized = fmt.Sprintf("hostname=%s container_markers=%s", j15.last.Hostname, j15.last.Markers)
		return nil
	})
