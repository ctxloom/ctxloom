...
// J14: plain `ctxloom session distill <harp>` (j14_session_distill.feature).
//
...
// Its list/show claims are already owned by j23_recall.feature; the one live
...
// command instead.
//
// Fixture shape is deliberately borrowed from steps_j23_recall.go
// (j23WriteCanonicalTranscript / j23AddIndexEntry / j23HarpHome): a session
...
// this scenario exercises the same resolution machinery j23 already proves
...
const (
	// j14Harp is distinct from j23's amber-quiet-heron / brisk-copper-moth so
	// this journey's fixture can never collide with j23's.
	j14Harp = "quiet-ember-forge"

	// j14TranscriptMarker exists ONLY in this journey's seeded transcript, so
	...
	j14TranscriptMarker = "J14-TRANSCRIPT-REACHED-DISTILLER"
)

func registerJ14SessionDistillSteps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^a project whose mock engine is both its primary and its distillation backend$`, func(c context.Context) error {
		// buildJ1Config (steps_j1_common.go) sets llm.defaults.primary AND
		...
		// setup — matching j23Setup's own "mock, mock" call.
	...
	ctx.Step(`^an earlier session "([^"]*)" left a real, non-empty transcript on disk$`, func(c context.Context, harp string) error {
		...
		if err := j23AddIndexEntry(w, harp, "seeded acceptance fixture session", transcriptPath); err != nil {
		...
		if err := j23WriteCanonicalTranscript(w, harp, []string{
			"What should we do about stale cached responses? " + j14TranscriptMarker,
			"Cache by ETag and revalidate on 304. " + j14TranscriptMarker,
		}); err != nil {
	...
	ctx.Step(`^the persisted essence for "([^"]*)" contains "([^"]*)"$`, func(c context.Context, harp, want string) error {
		w := worldFrom(c)
		body, err := w.env.ReadHomeFile(j23HarpHome(harp) + "/essence.md")
		if err != nil {
