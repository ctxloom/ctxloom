...
// reading the source): manage.go:144 (`manage install`) and manage.go:352
...
// a malformed bundle YAML. manage.go:225/388 (uninstall counterparts) and
...
var silentFailureAllowlist = map[string]string{
	"manage.go:144": "runManageInstall (`manage install`): ApplyHooks' per-backend errors are only warned, then the function unconditionally `return nil`s — confirmed by running against a permission-denied backend write (exit 0)",
	"manage.go:225": "runManageUninstall (`manage uninstall`): RemoveHooks' per-backend errors are only warned, then `return nil` — same shape as manage.go:144",
	"manage.go:352": "`manage hooks install` inline RunE: ApplyHooks' per-backend errors are only warned, then `return nil` — confirmed by running against a permission-denied backend write (exit 0)",
	"manage.go:388": "`manage hooks uninstall` inline RunE: RemoveHooks' per-backend errors are only warned, then `return nil` — same shape as manage.go:352",
