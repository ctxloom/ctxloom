...
Feature: A new engineer clones the repo and is already set up
  ...
  # untrusted content onto a fresh machine (see J15 for the trust story itself).
  ...
  # LOCKED — hermetic materialization, over the same four-engine axis J4
  # proved (j4_multi_engine.feature's Outline A: claude-code, kiro,
  # antigravity, codex — reusing that engine-axis machinery, not re-deriving
  # it, see engineContextRelPath in steps_j4.go). Bob is precisely the person
  ...
  # which materialize does populate — the same fix J4's own materialization
  # outline proves.
