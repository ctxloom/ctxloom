...
| `kind` | string | No | The message's kind, from a closed vocabulary. MESSAGE_KIND_MESSAGE (prose), MESSAGE_KIND_RESULT (a finding/verdict/deliverable), MESSAGE_KIND_ERROR (a failure the recipient must act on), MESSAGE_KIND_QUESTION (awaiting an answer). The remaining values are the coordinator's own — sending one is refused. Leaving this unset is refused too: name the kind (One of: `MESSAGE_KIND_UNSPECIFIED`, `MESSAGE_KIND_MESSAGE`, `MESSAGE_KIND_RESULT`, `MESSAGE_KIND_ERROR`, `MESSAGE_KIND_QUESTION`, `MESSAGE_KIND_APPROVAL_REQUEST`, `MESSAGE_KIND_USER_INJECTED`, `MESSAGE_KIND_USER_CONTROL`, `MESSAGE_KIND_EXITED`, `MESSAGE_KIND_STEER`) |
