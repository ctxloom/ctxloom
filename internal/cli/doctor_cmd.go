...
func doctorCheckContentTrust(cfg *config.Config, cfgErr error) doctorCheck {
	...
	pending, err := operations.PendingReview(cfg, operations.PendingReviewRequest{Loader: cfg.BundleLoader()})
	if err != nil {
...
func classifyContentTrust(marker string, pending *operations.PendingReviewResult) doctorCheck {
