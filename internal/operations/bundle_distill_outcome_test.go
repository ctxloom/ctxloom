...
`
	...
	require.NoError(t, store.WriteUnsignedApprove(signing.KindFragments, countersignRef(ref), signing.FormDistilled, []byte("old distilled text")))
	require.NoError(t, store.AppendIndex(countersign.IndexEntry{
		Ref: countersignRef(ref), Kind: string(signing.KindFragments), Form: string(signing.FormDistilled),
