...
// J21: "a delegated child gets ITS OWN permissions and ITS OWN MCP servers —
// not the parent's, not a sibling's — and that grant is journaled, durable,
// and auditable" (j21_delegation.feature).
//
...
// j21AgentSpec is one delegated child's fixture identity: which profile it
...
// j21State is J21's fixture state: the two configured agents (mutable — the
...
type j21State struct {
	...
	specs      map[string]*j21AgentSpec
	beforeEdit map[string]*j21AgentSpec // agent name -> its spec as captured just before an edit
	snapshots  map[string]j21RunFact    // "remembered as" label -> captured journal fact
	harps      map[string]string       // agent name -> its most recently spawned session harp
}

// j21RunFact is runEnqueued's (coord/facts.go) payload, decoded straight off
// runs.jsonl — the exact fields Wave F1 added so a child's resolved grant is
// auditable, not just provable in-process.
type j21RunFact struct {
...
// j21JournalLine is one runs.jsonl line's outer envelope (coord/facts.go's
// journal.Fact shape: {kind, at, data}); only "kind" is inspected here to
// find run.enqueued lines, "data" decodes into j21RunFact.
type j21JournalLine struct {
...
func j21Of(w *World) *j21State {
	if w.j21 == nil {
		w.j21 = &j21State{
			specs:      map[string]*j21AgentSpec{},
			beforeEdit: map[string]*j21AgentSpec{},
			snapshots:  map[string]j21RunFact{},
...
// j21RenderConfig renders config.yaml for the current spec state: one "fast"
...
// iteration order is not stable; j21.order pins it).
func j21RenderConfig(j21 *j21State) string {
	var b strings.Builder
	b.WriteString("version: 4\nllm:\n  configs:\n    fast:\n      type: mock\n  defaults:\n    primary: fast\n    fast: fast\nagents:\n")
	for _, name := range j21.order {
		s := j21.specs[name]
...
// j21BundleYAML renders one agent's backing bundle: a single MCP server whose
// command carries one plausible secret-shaped argument.
func j21BundleYAML(s *j21AgentSpec) string {
	return fmt.Sprintf("version: \"1.0.0\"\nmcp:\n  %s:\n    command: %s\n    args: [%q]\n", s.Server, s.Command, s.SecretArg)
}

// j21ProfileYAML renders one agent's profile: the one bundle that backs it.
func j21ProfileYAML(s *j21AgentSpec) string {
	return fmt.Sprintf("bundles:\n  - ctxloom:local@bundles/%s\n", s.Bundle)
}

// j21JournalRaw reads the coordinator's run-registry journal straight off
...
func j21JournalRaw(w *World) (string, error) {
...
// j21AllFactsForAgent parses every run.enqueued fact for one agent name, in
// journal (append) order.
// j21AllFactsForAgent parses run.enqueued facts for one agent. skipped and
...
func j21AllFactsForAgent(w *World, agent string) (out []j21RunFact, skipped int, firstSkipErr error, err error) {
	raw, err := j21JournalRaw(w)
	if err != nil {
...
// j21LatestFactForAgent returns the MOST RECENT run.enqueued fact for agent —
...
func j21LatestFactForAgent(w *World, agent string) (*j21RunFact, error) {
	facts, skipped, skipErr, err := j21AllFactsForAgent(w, agent)
	if err != nil {
		return nil, err
	}
	if len(facts) == 0 {
...
func registerJ21Steps(ctx *godog.ScenarioContext) {
	ctx.Step(`^Alice's coordinator can delegate to two agents, "([^"]*)" and "([^"]*)", each with its own profile, its own MCP server, and its own permission mode$`,
		func(c context.Context, nameA, nameB string) error {
			...
			if err := w.env.WriteFile(".ctxloom/content/bundles/bundle-review.yaml", j21BundleYAML(j21.specs[nameA])); err != nil {
				return err
			}
			if err := w.env.WriteFile(".ctxloom/content/bundles/bundle-fix.yaml", j21BundleYAML(j21.specs[nameB])); err != nil {
				return err
			}
			if err := w.env.WriteFile(".ctxloom/profiles/review-profile.yaml", j21ProfileYAML(j21.specs[nameA])); err != nil {
				return err
			}
			if err := w.env.WriteFile(".ctxloom/profiles/fix-profile.yaml", j21ProfileYAML(j21.specs[nameB])); err != nil {
				return err
			}
			return w.env.WriteFile(".ctxloom/config.yaml", j21RenderConfig(j21))
		})
	...
	ctx.Step(`^"([^"]*)"'s journaled grant carries its own MCP server and not "([^"]*)"'s$`,
		func(c context.Context, self, other string) error {
			w := worldFrom(c)
			j21 := j21Of(w)
			selfSpec, ok := j21.specs[self]
			if !ok {
				return fmt.Errorf("j21: unknown agent %q", self)
			}
			otherSpec, ok := j21.specs[other]
			if !ok {
				return fmt.Errorf("j21: unknown agent %q", other)
			}
			fact, err := j21LatestFactForAgent(w, self)
			if err != nil {
				return err
			}
	...
	ctx.Step(`^"([^"]*)"'s journaled grant records its own permission mode$`,
		func(c context.Context, name string) error {
			w := worldFrom(c)
			j21 := j21Of(w)
			spec, ok := j21.specs[name]
			if !ok {
				return fmt.Errorf("j21: unknown agent %q", name)
			}
			fact, err := j21LatestFactForAgent(w, name)
			if err != nil {
				return err
			}
	...
	ctx.Step(`^"([^"]*)"'s journaled grant records its own permission mode, different from "([^"]*)"'s$`,
		func(c context.Context, name, other string) error {
			w := worldFrom(c)
			factSelf, err := j21LatestFactForAgent(w, name)
			if err != nil {
				return err
			}
			factOther, err := j21LatestFactForAgent(w, other)
			if err != nil {
				return err
			}
	...
	ctx.Step(`^Alice edits "([^"]*)" to use "([^"]*)"'s profile and permission mode instead$`,
		func(c context.Context, target, donor string) error {
			w := worldFrom(c)
			j21 := j21Of(w)
			donorSpec, ok := j21.specs[donor]
			if !ok {
				return fmt.Errorf("j21: unknown agent %q", donor)
			}
			targetSpec, ok := j21.specs[target]
			if !ok {
				return fmt.Errorf("j21: unknown agent %q", target)
			}
			before := *targetSpec
			j21.beforeEdit[target] = &before
	...
	ctx.Step(`^the grant remembered as "([^"]*)" still shows the original MCP server and permission mode$`,
		func(c context.Context, label string) error {
			w := worldFrom(c)
			j21 := j21Of(w)
			fact, ok := j21.snapshots[label]
			if !ok {
				return fmt.Errorf("j21: no grant was ever remembered as %q", label)
			}
			orig, ok := j21.beforeEdit[fact.Agent]
			if !ok {
				return fmt.Errorf("j21: no pre-edit spec recorded for agent %q", fact.Agent)
			}
	...
	ctx.Step(`^the grant remembered as "([^"]*)" shows the newly edited MCP server and permission mode$`,
		func(c context.Context, label string) error {
			...
			cur, ok := j21.specs[fact.Agent]
			if !ok {
				return fmt.Errorf("j21: unknown agent %q", fact.Agent)
			}
	...
	ctx.Step(`^"([^"]*)"'s spawned session is remembered$`, func(c context.Context, name string) error {
		...
		if !ok || harp == "" {
			return fmt.Errorf("j21: last tool result carries no harp field for %q; result:\n%s", name, w.lastTool.JSON())
		}
		j21.harps[name] = harp
		return nil
	})
	...
	ctx.Step(`^the agent calls tool "agent_stop" for "([^"]*)"'s remembered session$`, func(c context.Context, name string) error {
		w := worldFrom(c)
		j21 := j21Of(w)
		harp, ok := j21.harps[name]
		if !ok || harp == "" {
			return fmt.Errorf("j21: no session harp remembered for %q", name)
		}
	...
	ctx.Step(`^the coordinator's audit trail records the child runner's advertised capabilities$`,
		func(c context.Context) error {
			...
			for {
				caps, err = j21AttachedCapabilities(w)
...
// j21AttachedCapabilities reads every run_channel interaction's advertised
...
func j21AttachedCapabilities(w *World) ([]string, error) {
