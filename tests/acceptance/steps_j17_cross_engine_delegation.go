...
// (j9_isolation.feature's own job), and the fixture's own bundle/profile
...
// scraped. Decoded locally here (mirroring steps_j6_delegation.go's
// j6RunFact/j6JournalLine — a minimal local shadow of the on-disk shape,
// not an import of the internal/transcript package) rather than trusting an
// in-process struct.

// j17TranscriptEntry is one KindEntry line's payload
// (transcript.EntryPayload, field-subset).
type j17TranscriptEntry struct {
	Type    string `json:"type"`
	Content string `json:"content"`
}

// j17TranscriptLine is one transcript.jsonl line's envelope
// (transcript.Record, field-subset): only "kind"/"entry" are needed here.
type j17TranscriptLine struct {
	Kind  string              `json:"kind"`
	Entry *j17TranscriptEntry `json:"entry,omitempty"`
}

// j17TranscriptPath returns harp's canonical transcript path under this
...
// actually wrote under (the identical reasoning steps_j6_delegation.go's
// j6JournalRaw already documents for runs.jsonl).
func j17TranscriptPath(w *World, harp string) string {
...
func j17TranscriptAssistantCount(w *World, harp string, want int) ([]j17TranscriptEntry, error) {
...
func registerJ17Steps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^"([^"]*)"'s reported turn carries its own guidance, not "([^"]*)"'s$`,
		func(c context.Context, self, other string) error {
			...
			assistants, err := j17TranscriptAssistantCount(w, harp, 1)
			if err != nil {
				return err
			}
			body := assistants[0].Content
			w.docStepMaterialized = fmt.Sprintf("%s — %s's first reported turn:\n  %s", j17TranscriptPath(w, harp), self, body)
	...
	ctx.Step(`^"([^"]*)"'s next reported turn carries "([^"]*)"$`,
		func(c context.Context, self, want string) error {
			...
			// step above.
			assistants, err := j17TranscriptAssistantCount(w, harp, 2)
			if err != nil {
				return err
			}
		...
		func(c context.Context, budgetSec int, name string) error {
			...
			for {
				if err := callTool(c, "agent_recv", map[string]any{"wait": 12}); err != nil {
					return fmt.Errorf("j17: agent_recv transport error while waiting for %q: %w", name, err)
				}
				w := worldFrom(c)
				if isErr, msg := w.lastTool.IsError(); isErr {
					lastErr = fmt.Errorf("%s", msg)
					if time.Now().After(deadline) {
						return fmt.Errorf("j17: agent_recv never returned a message from %q within %ds: %w", name, budgetSec, lastErr)
					}
					continue
				}
...
// j17Messages unwraps an agent_recv result's "messages" array (@live only —
