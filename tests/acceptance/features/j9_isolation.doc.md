<!--
J9 narration + outcome-matrix companion.
...
j1_setup.doc.md / j5_multi_engine.doc.md / j6_delegation.doc.md split on, for
the scenarios added by the isolation-matrix task. j9's three PRE-EXISTING
...
named cucumber scenario in this file's feature (steps_j9_isolation_matrix.go)
...
1. **The generic fail-loud/degrade CONTRACT** (any container that cannot
   ...
   engine-agnostically, by j9's own pre-existing "Requesting a container with
...
- **What would go red if the leak closed**: `TestAcceptance/A_worktree_run_for_kiro_isolates_only_session_state_without_an_API_key...`
  (steps_j9_isolation_matrix.go, via j9_isolation.feature) asserts the
...
- **EXECUTED, hermetically, every `just test-acceptance` run** (no live
  ...
  Scenario Outlines/Scenarios in `j9_isolation.feature`, confirmed green
...
- **Live vendor-drift detection** (does the REAL, currently-installed
  ...
  infrastructure (`live_engine_registry.go`, exercised by J1/J5's
