// Package importer defines the shared contract every per-engine vendor
// importer implements: parse ONE vendor-native transcript (codex's rollout
...
// through. This is the point of the whole package: an imported session and a
// tee'd session must land in byte-for-byte the same on-disk schema
// (transcript.acp.jsonl), never a second format a downstream reader would
// need to special-case. See docs/transcript-schema.md §8 for why an importer
// exists at all (reversing S5's scraper deletion for interactive-pty and
// pre-capture sessions, done through the canonical Recorder this time
// instead of a bespoke SessionHistory reader) and
// internal/transcript/importer/codex for the reference implementation every
// other engine's importer copies.
//
...
// but never appeared in a full `go test ./...`). "importer" avoids the
...
// already has lines written to it (e.g. a resumed/retried import): every
...
// already guarantee on the READ side (docs/transcript-schema.md §6.4). An
// importer that aborted on the first bad byte would turn one corrupt line
