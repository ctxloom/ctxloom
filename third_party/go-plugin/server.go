...
func serverListener(unixSocketCfg UnixSocketConfig) (net.Listener, error) {
	...
	// chose unix — unusable across the Docker Desktop VM boundary on macOS/
	// Windows, where a unix socket in a bind-mounted dir is not a live host
	// endpoint. PLUGIN_LISTEN_TCP=1 (set by ctxloom only when the HOST is
	// non-linux) selects the existing TCP listener path, honoring
	// PLUGIN_MIN_PORT/PLUGIN_MAX_PORT exactly as the native-Windows branch does.
