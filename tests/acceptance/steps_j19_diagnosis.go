...
// J19: "the day the assistant goes blind" (j19_diagnosis.feature) — the
// DIAGNOSIS walk. FLOWS-UNIFIED.md's U5, numbered J19 here because that
// document's "proposed J14" slot was taken by bundle distribution before this
// was written.
//
...
// the bug.
//
// SEAM WITH J16/J15. J16 owns the PRODUCTION of signatures and J15 owns the
...
//
// ISOLATION follows steps_j16_signing.go exactly: everything runs through
...
// with steps_j15.go's deliberate blanking of the same key.
package acceptance
...
const (
	// j19DeployMarker IS the deploy guidance: a distinctive string, so every
	// delivery assertion checks for exactly the right bytes rather than for a
	// file's existence or a command's exit code.
	j19DeployMarker = "J19-DEPLOY-PROCESS-MARKER"
	// j19RevisedMarker is Carol's Friday edit — the content that SHOULD be
	// arriving on Monday and is not.
	j19RevisedMarker = "J19-DEPLOY-PROCESS-REVISED-MARKER"

	// j19Bundle is the team's runbook bundle, and j19Fragment the item inside
	// it that carries the deploy process.
	j19Bundle   = "deploy-runbook"
	j19Fragment = "deploy-process"

	// j19KeyComment / j19Principal mirror J16's split: the ssh-agent comment a
	// human recognizes is deliberately NOT the allowed_signers principal, so a
	// fixture can never confuse the two.
	j19KeyComment = "carol@acme.example"
	j19Principal  = "runbooks@acme.example"
	j19PubKeyFile = "acme-runbooks.pub"

	// j19BundlesDir is the authored (committed, publishable) bundle tree.
	j19BundlesDir = ".ctxloom/content/bundles"
)

// j19State is this journey's fixture state.
type j19State struct {
	...
	// one guessed spelling. See j19Probe.
	probes []j19ProbeResult
...
// j19ProbeResult is one candidate inspector invocation and what it produced.
type j19ProbeResult struct {
	args   []string
	exit   int
	output string
}

func j19Of(w *World) *j19State {
	if w.j19 == nil {
		w.j19 = &j19State{}
	}
	return w.j19
...
// j19BundleYAML renders the runbook bundle at a given version carrying content
// as the deploy process. Ordered, literal YAML for the same reason J16 does it:
...
func j19BundleYAML(version, content string) string {
	return fmt.Sprintf("version: %q\nfragments:\n  %s:\n    content: %q\n", version, j19Fragment, content)
}

func j19BundlePath() string { return j19BundlesDir + "/" + j19Bundle + ".yaml" }

// j19Setup is the Background: a hermetic project with one engine, a seed
...
func j19Setup(w *World) error {
	...
	if err := os.MkdirAll(filepath.Join(w.env.ProjectDir, filepath.FromSlash(j19BundlesDir)), 0o755); err != nil {
	...
	st.signer = signer
	if err := w.env.WriteFile(j19PubKeyFile, signer.AuthorizedKey(j19KeyComment)); err != nil {
		return fmt.Errorf("write Carol's public key: %w", err)
	}
	sock, stop, err := testenv.StartSSHAgent(w.env.Root, testenv.SSHAgentIdentity{Signer: signer, Comment: j19KeyComment})
	if err != nil {
		return fmt.Errorf("start hermetic ssh-agent: %w", err)
	}
	st.stopAgent = stop
	w.env.SetEnv("SSH_AUTH_SOCK", sock)
	if err := w.env.GitConfigLocal("user.signingkey", filepath.Join(w.env.ProjectDir, j19PubKeyFile)); err != nil {
		return err
	}
...
// j19WriteAuthored writes the runbook into the authored tree at version
// 1.0.0 — Friday's copy, and the one every locally-authored scenario uses.
func j19WriteAuthored(w *World, content string) error {
	return w.env.WriteFile(j19BundlePath(), j19BundleYAML("1.0.0", content))
}

// j19WriteRevised writes Carol's revision at a HIGHER version, so republishing
// it is a genuine pin advance rather than a byte change the consumer's lock
// never moves to. See j19BundleYAML.
func j19WriteRevised(w *World, content string) error {
	return w.env.WriteFile(j19BundlePath(), j19BundleYAML("1.1.0", content))
}

// j19LockedName is the name the runbook carries in the active lockfile once it
// has been pulled from the team remote — remote-qualified, which is what
// `bundle hold` resolves against.
func j19LockedName() string { return "team/" + j19Bundle }

// j19PublishFromDisk publishes exactly what is on disk — bundle bytes plus
// whatever `.sig` sibling is (or is not) beside them — into the team remote,
// then hands the authoring copy off out of the project.
//
// The hand-off is load-bearing, for the reason steps_j16_signing.go's
// j16SeedFromDisk documents: one hermetic project plays both the publishing
...
// nothing.
func j19PublishFromDisk(w *World) error {
	st := j19Of(w)
	rel := j19BundlePath()
...
// j19Reference wires the team remote into the consuming project the ordinary
...
// publisher advances.
func j19Reference(w *World) error {
	st := j19Of(w)
	if st.referenced {
	...
	if err := runOK(w, "profile", "modify", "default", "--add-bundle", "team/"+j19Bundle); err != nil {
		return err
	}
	st.referenced = true
	return runOK(w, "remote", "pull")
...
// j19TrustCarol writes Carol's key into the committable project trust store,
// so the ATTESTATION boundary is the only thing left to plant a cause at.
func j19TrustCarol(w *World) error {
	return w.env.TrustSigner(j19Of(w).signer, j19Principal, true)
}

// j19Delivered materializes the default profile and returns the assembled
// context — the payload every delivery assertion in this journey reads.
func j19Delivered(w *World) (string, error) {
...
// j19AssertDelivery checks a marker's presence in the assembled context, and
...
func j19AssertDelivery(w *World, marker string, want bool) error {
	body, err := j19Delivered(w)
	if err != nil {
		return err
	}
...
// j19Probe runs one candidate inspector invocation and records it. It never
...
func j19Probe(w *World, args ...string) {
	st := j19Of(w)
	_ = w.env.Run(args...)
	st.probes = append(st.probes, j19ProbeResult{
...
// j19ProbesAnswered reports whether ANY probed invocation produced output
...
func j19ProbesAnswered(w *World, wants ...string) error {
	st := j19Of(w)
	if len(st.probes) == 0 {
...
// j19OutputNamesAll asserts the LAST command's output names every one of
// wants — the shape of "the inspector named the cause".
func j19OutputNamesAll(w *World, what string, wants ...string) error {
	return j19NamesAll(w.env.LastOutput(), what, wants...)
}

// j19NamesAll is j19OutputNamesAll over an already-captured output, for the
// assertions that must read what a specific EARLIER command said rather than
// whatever ran most recently.
func j19NamesAll(out, what string, wants ...string) error {
	var missing []string
	for _, want := range wants {
...
// j19LockedSHA reads the runbook's pinned commit straight out of the project's
// lock.yaml.
//
...
func j19LockedSHA(w *World) (string, error) {
	...
	for key, entry := range lock.Bundles {
		if strings.Contains(key, j19Bundle) {
			if entry.SHA == "" {
...
func registerJ19Steps(ctx *godog.ScenarioContext) {
	...
	ctx.After(func(c context.Context, sc *godog.Scenario, err error) (context.Context, error) {
		w := worldFrom(c)
		if w == nil || w.j19 == nil || w.j19.stopAgent == nil {
			return c, nil
		}
		stop := w.j19.stopAgent
		w.j19.stopAgent = nil
	...
	ctx.Step(`^the search results name no packaged item carrying the deploy process$`, func(c context.Context) error {
		...
		if strings.Contains(out, j19Fragment) || strings.Contains(out, j19Bundle) {
	...
	ctx.Step(`^Carol published the signed runbook, and Alice's assistant receives its deploy guidance$`, func(c context.Context) error {
		...
		if err := j19WriteAuthored(w, j19DeployMarker); err != nil {
			return err
		}
		if err := runOK(w, "bundle", "sign", j19Bundle); err != nil {
			return err
		}
		if err := j19TrustCarol(w); err != nil {
			return err
		}
		if err := j19PublishFromDisk(w); err != nil {
			return err
		}
		if err := j19Reference(w); err != nil {
			return err
		}
		return j19AssertDelivery(w, j19DeployMarker, true)
	})
	...
	ctx.Step(`^Carol edits the runbook on Friday and never re-signs it$`, func(c context.Context) error {
		...
		if err := j19WriteRevised(w, j19RevisedMarker); err != nil {
			return err
		}
		return j19PublishFromDisk(w)
	})
	...
	ctx.Step(`^Alice syncs on Monday$`, func(c context.Context) error {
		...
		pin, err := j19LockedSHA(w)
		if err != nil {
			return err
		}
		j19Of(w).pinBeforeSync = pin
		_ = w.env.Run("remote", "upgrade")
		j19Of(w).syncOutput = w.env.LastOutput()
		j19Of(w).syncExit = w.env.LastExitCode()
		return nil
	})

	ctx.Step(`^her assistant never receives the revised deploy guidance$`, func(c context.Context) error {
		return j19AssertDelivery(worldFrom(c), j19RevisedMarker, false)
	})
	...
	ctx.Step(`^her assistant is still served the content at that pin$`, func(c context.Context) error {
		...
		return j19AssertDelivery(worldFrom(c), j19DeployMarker, true)
	})
	...
	ctx.Step(`^the sync told her the runbook cannot be verified, naming the pin it kept$`, func(c context.Context) error {
		...
		return j19NamesAll(out, "Monday's sync", j19Bundle, "does not verify", kept)
	})
	...
	ctx.Step(`^the pending-review list does not name the runbook at all$`, func(c context.Context) error {
		...
		if strings.Contains(out, j19Bundle) {
	...
	ctx.Step(`^some inspector names the runbook, the signature that does not verify, and the pin she is kept at$`, func(c context.Context) error {
		...
		return j19ProbesAnswered(w, j19Bundle, "does not verify", kept)
	})
	...
	ctx.Step(`^Alice asks ctxloom why the runbook stopped arriving$`, func(c context.Context) error {
		...
		j19Probe(w, "doctor")
		j19Probe(w, "review", "--list")
		j19Probe(w, "bundle", "list")
		j19Probe(w, "bundle", "show", j19LockedName())
		j19Probe(w, "trust", "signer", "list")
		return nil
	})
	...
	ctx.Step(`^Carol publishes a newer signed runbook while Alice's copy is held$`, func(c context.Context) error {
		...
		if err := runOK(w, "bundle", "hold", j19LockedName()); err != nil {
			return err
		}
		if err := j19WriteRevised(w, j19RevisedMarker); err != nil {
			return err
	...
	ctx.Step(`^the installed-bundle listing names the runbook as held$`, func(c context.Context) error {
		return j19OutputNamesAll(worldFrom(c), "the installed-bundle listing", j19Bundle, "held")
	})

	ctx.Step(`^her assistant still receives the older deploy guidance and not the newer$`, func(c context.Context) error {
		w := worldFrom(c)
		if err := j19AssertDelivery(w, j19DeployMarker, true); err != nil {
			return err
		}
		return j19AssertDelivery(w, j19RevisedMarker, false)
	})
	...
	ctx.Step(`^Carol published the signed runbook from a key Alice has never reviewed$`, func(c context.Context) error {
		...
		return j19Reference(w)
	})

	ctx.Step(`^the pending list names the deploy fragment of the runbook$`, func(c context.Context) error {
		return j19OutputNamesAll(worldFrom(c), "the pending-review list", j19Bundle, j19Fragment)
	})
	...
	ctx.Step(`^the pending list says the runbook's signer is one she does not trust, naming the key$`, func(c context.Context) error {
		w := worldFrom(c)
		return j19OutputNamesAll(worldFrom(c), "the pending-review list", "untrusted", j19Of(w).signer.Fingerprint())
	})

	ctx.Step(`^her assistant does not receive the deploy guidance$`, func(c context.Context) error {
		return j19AssertDelivery(worldFrom(c), j19DeployMarker, false)
	})
	...
	ctx.Step(`^the runbook is composed into Alice's profile$`, func(c context.Context) error {
		...
		return runOK(w, "profile", "modify", "default", "--add-bundle", j19Bundle)
	})
	...
	ctx.Step(`^the engine's own surface on disk still holds last week's copy$`, func(c context.Context) error {
		...
		if _, err := j19Delivered(w); err != nil {
			return err
		}
		if err := j19WriteAuthored(w, j19RevisedMarker); err != nil {
			return err
		}
	...
	ctx.Step(`^the wiring report names the materialized surface as stale$`, func(c context.Context) error {
		return j19OutputNamesAll(worldFrom(c), "`manage status`", "stale")
	})
	...
	ctx.Step(`^every earlier inspector is green and the deploy guidance is materialized into the engine's own surface$`, func(c context.Context) error {
		...
		if err := runOK(w, "profile", "modify", "default", "--add-bundle", j19Bundle); err != nil {
			return err
		}
		return j19AssertDelivery(w, j19DeployMarker, true)
	})
	...
	ctx.Step(`^Alice asks ctxloom what her engine actually ingested$`, func(c context.Context) error {
		...
		j19Probe(w, "agent", "show", "default")
		j19Probe(w, "session", "list")
		return nil
	})
	...
	ctx.Step(`^some inspector states plainly that it cannot confirm the engine read what ctxloom delivered$`, func(c context.Context) error {
		return j19ProbesAnswered(worldFrom(c),
	...
	ctx.Step(`^Bob's checkout of the same project delivers the deploy guidance and Alice's does not$`, func(c context.Context) error {
		...
		if err := runOK(w, "profile", "modify", "default", "--remove-bundle", j19Bundle); err != nil {
			return err
		}
		return j19AssertDelivery(w, j19DeployMarker, false)
	})
	...
	ctx.Step(`^Alice asks ctxloom to compare her delivered context with Bob's$`, func(c context.Context) error {
		...
		j19Probe(w, "profile", "show", "default", "--compare", bobFile)
		j19Probe(w, "doctor", "--compare", bobFile)
		j19Probe(w, "profile", "materialize", "default", "--diff", bobFile)
		return nil
	})

	ctx.Step(`^ctxloom reports the deploy guidance as present for Bob and absent for her$`, func(c context.Context) error {
		return j19ProbesAnswered(worldFrom(c), j19DeployMarker)
	})
}
