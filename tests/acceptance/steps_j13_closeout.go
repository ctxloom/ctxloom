...
// J13: "the close-out" (j13_closeout.feature) — FLOWS-UNIFIED.md's U11.
//
...
// j13DeadPid exceeds even a kernel.pid_max=4194304 configuration by a wide
...
const (
	// j13DeadPid names no live process on any Linux configuration — see the
	...
	j13DeadPid = 999999999

	// j13SessionsRel is the harp store, relative to the isolated HOME.
	j13SessionsRel = ".ctxloom/sessions"
	...
	j13BulkMarker     = "J13-MACHINE-WRITTEN-BULK"
	j13EssenceMarker  = "J13-DERIVED-ESSENCE"
	j13AuthoredMarker = "J13-HUMAN-AUTHORED-PLAN"
	j13WIPMarker      = "J13-UNCOMMITTED-WIP"

	// j13LessonsBundle is where the lessons leg lands its fragments.
	j13LessonsBundle = "team-lessons"
	// j13LessonsSkill is the distill skill the lessons leg selects — a ref in
	// the "<bundle>#skills/<name>" grammar FLOWS-UNIFIED §5.3 specifies.
	j13LessonsSkill = "house-style#skills/lessons"
)

// j13Harp records one seeded harp directory and what was planted in it.
type j13Harp struct {
	name      string
	dir       string // absolute
	essence   bool
...
// j13State is this journey's fixture state.
type j13State struct {
	harps   map[string]*j13Harp
...
func j13Of(w *World) *j13State {
	if w.j13 == nil {
		w.j13 = &j13State{harps: map[string]*j13Harp{}, foreign: map[string]string{}}
	}
	return w.j13
}

// j13Git runs a git command in dir under the harness's isolated environment.
func j13Git(w *World, dir string, args ...string) (string, error) {
...
func j13Setup(w *World) error {
	...
	if err := w.env.WriteFile("README.md", "# j13 close-out fixture\n"); err != nil {
		return err
	}
...
// j13HarpDir returns the absolute path of a harp's directory in the isolated
// home — ~/.ctxloom/sessions/<harp>, the layout paths.HarpDir builds.
func j13HarpDir(w *World, harp string) string {
	return filepath.Join(w.env.HomeDir, filepath.FromSlash(j13SessionsRel), harp)
}

// j13SeedHarp plants one harp directory carrying every content class §5.4
...
func j13SeedHarp(w *World, harp string, essence, authored bool) error {
	st := j13Of(w)
	dir := j13HarpDir(w, harp)
	...
	bulk := strings.Repeat(`{"role":"assistant","content":"`+j13BulkMarker+`"}`+"\n", 200)
	...
	st.harps[harp] = &j13Harp{name: harp, dir: dir, essence: essence, authored: authored}
	return j13WriteIndex(w)
}

// j13WriteIndex rewrites the session index over EVERY seeded harp. The shared
...
func j13WriteIndex(w *World) error {
	st := j13Of(w)
	...
	for _, name := range st.order {
		...
		fmt.Fprintf(&b, "    transcript_path: %s\n", filepath.Join(j13HarpDir(w, name), "transcript.jsonl"))
		fmt.Fprintf(&b, "    summary: seeded close-out session %s\n", name)
	}
	return w.env.WriteHomeFile(j13SessionsRel+"/index.yaml", b.String())
}

// j13AddScratchWorktree creates a REAL linked git worktree inside a harp's own
...
func j13AddScratchWorktree(w *World, harp, name string, ownerPid int, dirty bool) (string, error) {
	st := j13Of(w)
	h, ok := st.harps[harp]
	if !ok {
	...
	if _, err := j13Git(w, w.env.ProjectDir, "worktree", "add", "-q", "-b", "wt-"+name, wtDir); err != nil {
		return "", err
	}
	if ownerPid != 0 {
	...
	if dirty {
		...
		if err := os.WriteFile(filepath.Join(wtDir, "in-flight.go"), []byte("// "+j13WIPMarker+"\n"), 0o644); err != nil {
...
// j13AddForeignWorktree creates a long-lived worktree OUTSIDE the sessions
...
// report on it.
func j13AddForeignWorktree(w *World, branch string, dirty bool) (string, error) {
	st := j13Of(w)
	...
	if _, err := j13Git(w, w.env.ProjectDir, "worktree", "add", "-q", "-b", branch, dir); err != nil {
		return "", err
	}
	if dirty {
		if err := os.WriteFile(filepath.Join(dir, "uncommitted.txt"), []byte(j13WIPMarker+"\n"), 0o644); err != nil {
...
// j13Probe records one candidate invocation of a surface that may not exist.
func j13Probe(w *World, args ...string) {
	st := j13Of(w)
	_ = w.env.Run(args...)
...
// j13Answered reports whether the LAST run's output names every want, failing
// with the whole output and the exit code so a red scenario documents what the
// product said instead.
func j13Answered(w *World, what string, wants ...string) error {
...
// j13DirExists is a plain on-disk existence check on an absolute path — the
// only honest way to assert a reap happened or did not.
func j13DirExists(p string) bool {
	info, err := os.Stat(p)
	return err == nil && info.IsDir()
}

// j13RanRealSurface fails unless the last invocation actually reached a
// command that EXISTS.
//
...
func j13RanRealSurface(w *World) error {
	out := w.env.LastOutput()
...
func registerJ13Steps(ctx *godog.ScenarioContext) {
	ctx.After(func(c context.Context, sc *godog.Scenario, err error) (context.Context, error) {
		w := worldFrom(c)
		if w == nil || w.j13 == nil {
			return c, nil
		}
		if w.j13.stopAgent != nil {
			stop := w.j13.stopAgent
			w.j13.stopAgent = nil
	...
	ctx.Step(`^session "([^"]*)" left a scratch worktree nothing can prove the owner of$`, func(c context.Context, harp string) error {
		...
		_, err := j13AddScratchWorktree(worldFrom(c), harp, "unknowable", 0, false)
		return err
	})

	ctx.Step(`^session "([^"]*)" left a scratch worktree whose owning process is still alive$`, func(c context.Context, harp string) error {
		_, err := j13AddScratchWorktree(worldFrom(c), harp, "live", os.Getpid(), false)
		return err
	})
	...
	ctx.Step(`^a long-lived worktree "([^"]*)" of her own, outside the sessions root, with unmerged work$`, func(c context.Context, branch string) error {
		w := worldFrom(c)
		dir, err := j13AddForeignWorktree(w, branch, true)
		if err != nil {
			return err
		}
		...
		if _, err := j13Git(w, dir, "add", "-A"); err != nil {
			return err
		}
		_, err = j13Git(w, dir, "commit", "-m", "unmerged feature work")
		return err
	})
	...
	ctx.Step(`^the checks name the ignore rule and the command that retires it$`, func(c context.Context) error {
		return j13Answered(worldFrom(c), "`ctxloom doctor`", ".ctxloom", "manage gitignore install")
	})
	...
	ctx.Step(`^the checks name the foreign worktree, that it is unmerged and dirty, and the exact commands to remove it$`, func(c context.Context) error {
		w := worldFrom(c)
		return j13Answered(w, "doctor's foreign-worktree report",
	...
	ctx.Step(`^the checks warn that the design notes sit in the harp directory's unclassified top level$`, func(c context.Context) error {
		w := worldFrom(c)
		return j13Answered(w, "doctor's harp-durability check (B13)",
			".plan.md", "persist")
	})
	...
	ctx.Step(`^the report names each scratch worktree with its harp, its owner and its verdict$`, func(c context.Context) error {
		w := worldFrom(c)
		return j13Answered(w, "`ctxloom session worktrees`",
			"ctxloom-wt-clean", "ctxloom-wt-wip", fmt.Sprintf("%d", j13DeadPid))
	})
	...
	ctx.Step(`^the uncommitted work is still there, spared in place$`, func(c context.Context) error {
		...
		for _, h := range j13Of(w).harps {
			for _, wt := range h.worktrees {
				...
				if !strings.Contains(string(body), j13WIPMarker) {
	...
	ctx.Step(`^that bundle is signed in the same breath$`, func(c context.Context) error {
		...
		for _, rel := range []string{
			".ctxloom/content/bundles/" + j13LessonsBundle + ".yaml.sig",
			".ctxloom/content/bundles/" + j13LessonsBundle + "/bundle.yaml.sig",
		} {
		...
		return fmt.Errorf("the lessons bundle was written UNSIGNED. An edited-unsigned bundle is silently withheld "+
			"(boundary B2, and J19's own red scenario), so a lessons flow that ends unsigned manufactures the exact defect "+
	...
	ctx.Step(`^ctxloom refuses rather than falling back to its built-in prompt$`, func(c context.Context) error {
		...
		return j13Answered(w, "the refusal", "withheld", "review")
	})
	...
	ctx.Step(`^the report lists what would be destroyed and what would be kept$`, func(c context.Context) error {
		return j13Answered(worldFrom(c), "`ctxloom session purge`", "transcript", "essence")
	})
	...
	ctx.Step(`^its distilled essence and its index entry survive$`, func(c context.Context) error {
		...
		for _, h := range st.harps {
			...
			if !strings.Contains(string(body), j13EssenceMarker) {
				return fmt.Errorf("%s's essence no longer carries its own bytes; it holds:\n%s", h.name, body)
			}
		}
		idx, err := w.env.ReadHomeFile(j13SessionsRel + "/index.yaml")
		if err != nil {
	...
	ctx.Step(`^her unfiled design notes are still there, and were named in the report$`, func(c context.Context) error {
		...
		for _, h := range j13Of(w).harps {
			...
			if !strings.Contains(string(body), j13AuthoredMarker) {
	...
	ctx.Step(`^ctxloom refuses, naming the session that was never distilled$`, func(c context.Context) error {
		...
		return j13Answered(w, "the refusal", "undistilled")
	})
	...
	ctx.Step(`^the scratch worktrees are still registered with git, untouched by the purge$`, func(c context.Context) error {
		...
		st := j13Of(w)
		out, err := j13Git(w, w.env.ProjectDir, "worktree", "list", "--porcelain")
		if err != nil {
			return err
		}
		for _, h := range st.harps {
