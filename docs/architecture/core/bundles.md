...
flowchart TD
    ...
    SK --> PAY2["ContentPayload → JSON manifest<br/>bundles.go:508"]
...
| `BundleSkill.ContentPayload() ([]byte, error)` | `bundles.go:508` | Canonical JSON `{preimage: "ctxloom-exec/1", manifest: ToManifest()}` — covers `Files` only |
...
6. **The content hash is `sha256` over a per-kind preimage produced in this package, never over the YAML file.** `hashContent` (`bundles.go:349`) is the only hash site. Fragments and commands hash their effective exposed bytes; skills hash `{preimage, manifest}` (`bundles.go:508`); MCP servers hash `{preimage, command, args, env, installation}` (`bundles.go:552`); hooks hash `{preimage, matcher, type, command, prompt, pre_tool_fallback}` (`bundles.go:609`). Field order in those structs is byte order in the preimage, so it is part of the `ctxloom-exec/1` contract.
...
15. **A skill's signed identity is its manifest, not its archive.** `SkillManifest.Serialize()` (`skill.go:112`) over the path/sha256/mode triples is the signature preimage on both the signing side (`operations/skills.go:459`) and the verifying side (`skill_archive.go:702`). The manifest is regenerated only by `ctxloom skill sync` (`internal/cli/skill_cmd.go:204`), so `BundleSkill.Files` is meaningful only after that has run.
...
- `loader_skills.go:87` explains the unverified manifest-less skill path with "that CLI is Part B6"; the CLI ships as `ctxloom skill sync` (`internal/cli/skill_cmd.go:204`).
...
- `hookContentPayload` signs `PreToolFallback` (`bundles.go:615`), but proto `Hook` (`internal/lm/grpc/llm.proto:447`) has no such field and both converters (`internal/lm/grpc/managed.go:90`, `:102`) drop it, so the hook delivered over the wire is not the hook the preimage covered.
...
- `BundleSkill.ContentPayload` covers only `Files` (`bundles.go:508`), so a skill with no `files:` map has a preimage that is constant across all such skills and invariant under changes to the package's bytes; `skillContent` also skips `VerifyExtractedManifest` in that case (`loader_skills.go:105`).
