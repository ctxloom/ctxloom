<!--
J000400 narration companion.

Prose ONLY. It never restates what the Gherkin already says business-readably,
and it carries no assertions of its own — j000400_multi_engine.feature next to it is
the single source of truth for what J000400 promises. What lives here is the
...
engine, or someone hand-translates the standard into three different config
formats and lets them drift the moment anyone forgets to update all three.
...
a bare "the file exists" or "the key name shows up somewhere in it."
...
context is a silent no-op — the only one of the three engines whose
