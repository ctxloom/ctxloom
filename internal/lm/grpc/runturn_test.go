...
func TestRunTurn_DrivesSetupExecuteCleanup(t *testing.T) {
	...
	result, err := RunTurn(context.Background(), fb, req, nil, &out, &out, nil)
...
func TestRunTurn_ExecuteErrorPropagates(t *testing.T) {
	...
	_, err := RunTurn(context.Background(), fb, &RunStart{Options: &RunOptions{}}, nil, &strings.Builder{}, &strings.Builder{}, nil)
	require.Error(t, err)
