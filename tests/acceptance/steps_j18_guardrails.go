...
// J18: "Guardrails — when the assistant does not listen" (j18_guardrails.feature).
//
...
// exactly like steps_j8_onboarding.go already does for reprise, never a real
...
// steps_j8_onboarding.go's own reprise fixture (j8RepriseMarker) already
// established.
//
...
const (
	// j18Target is the materialize destination for Alice's scenarios (1 & 2) —
	// mirrors "Alice starts a session"'s own hardcoded "out" (steps_j2_setup.go).
	j18Target = "out"

	// j18ReuseMarker stands in for reprise's reuse-before-you-write guidance.
	...
	// contribute once it does, exactly like steps_j8_onboarding.go's
	// j8RepriseMarker fixture.
	j18ReuseMarker = "J18-REPRISE-REUSE-BEFORE-YOU-WRITE-MARKER"

	j18LtkPrincipal       = "ltk@example.com"
	j18RepriseCoPrincipal = "reprise@example.com"

	// j18LtkHeading and j18LtkRedirectClaim are exact substrings of ltk's REAL,
	...
	j18LtkHeading       = "# llm-tool-killer (ltk)"
	j18LtkRedirectClaim = "ltk is a cooperative redirect, not a sandbox"
)

// j18LtkLoadoutYAML reads ltk's REAL, committed loadout bundle
...
// ltk binary ships.
func j18LtkLoadoutYAML() (string, error) {
	_, thisFile, _, ok := runtime.Caller(0)
	if !ok {
		return "", fmt.Errorf("resolve this source file's own path")
	}
	// tests/acceptance/steps_j18_guardrails.go -> repo root -> cmd/ltk/loadout.yaml
...
// j18InstallFakeCompanion signs bundleYAML with a fresh throwaway test signer,
...
// mirrors steps_j8_onboarding.go's reprise fixture, generalized to any bin.
func j18InstallFakeCompanion(w *World, bin, principal, bundleYAML, versionJSON string) error {
...
// j18ScrubFromPath removes any PATH entry that would satisfy a lookup for a
// binary literally named bin — shared by J8 (steps_j8_onboarding.go, "the
// reprise companion is not installed") and J18 (below, ltk), consolidated
...
func j18ScrubFromPath(w *World, bin string) error {
...
// j18LtkGuardedTools is the tool classes the scenario sentence "run ltk on
...
// tool — left all three j18 scenarios green. Deriving the expectation from
...
var j18LtkGuardedTools = []string{"Bash", "PowerShell", "Edit", "Write", "MultiEdit", "NotebookEdit"}

// j18LtkShippedPreToolMatcher returns the pre_tool matcher from ltk's real
...
// assertion, never the whole of it (see j18LtkGuardedTools): a matcher that
...
func j18LtkShippedPreToolMatcher() (string, error) {
	raw, err := j18LtkLoadoutYAML()
	if err != nil {
		return "", err
	}
	var doc struct {
...
// j18ClaudeSettings is the minimal shape this journey needs to parse out of
...
func registerJ18Steps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^ltk and reprise are installed on Alice's machine$`, func(c context.Context) error {
		...
		// first (see j18ScrubFromPath's doc) so the fake always wins deterministically.
		if err := j18ScrubFromPath(w, "ltk"); err != nil {
			return err
		}
		if err := j18ScrubFromPath(w, "reprise"); err != nil {
			return err
		}
		ltkYAML, err := j18LtkLoadoutYAML()
		if err != nil {
			return err
		}
		if err := j18InstallFakeCompanion(w, "ltk", j18LtkPrincipal, ltkYAML, `{"name":"ltk","version":"v0.0.0-j18-fake"}`); err != nil {
			return err
		}
		repriseYAML := fmt.Sprintf("version: \"1.0.0\"\nfragments:\n  reuse-discipline:\n    content: %q\n", j18ReuseMarker)
		return j18InstallFakeCompanion(w, "reprise", j18RepriseCoPrincipal, repriseYAML, `{"name":"reprise","version":"v0.0.0-j18-fake"}`)
	})

	// "Alice's project exists" (ensureProjectWithEngine "claude-code") is
	// already registered by steps_j17.go; "Alice starts a session" (materialize
	// "default" into "out") is already registered by steps_j2_setup.go — both
	...
	ctx.Step(`^her assistant receives ltk's task-runner guidance$`, func(c context.Context) error {
		w := worldFrom(c)
		body, err := materializeDefault(w, j18Target)
		if err != nil {
			return err
		}
		w.docStepMaterialized = filepath.Join(j18Target, "CLAUDE.md") + ":\n" + j4Excerpt(body, j18LtkHeading, 4)
		if !strings.Contains(body, j18LtkHeading) {
			return fmt.Errorf("assembled context does not contain ltk's real fragment (looked for %q); content:\n%s", j18LtkHeading, body)
		}
		return nil
	})
	...
	ctx.Step(`^her assistant's engine is configured to run ltk on every shell command and file edit$`, func(c context.Context) error {
		w := worldFrom(c)
		if _, err := materializeDefault(w, j18Target); err != nil {
			return err
		}
		rel := filepath.Join(j18Target, ".claude", "settings.json")
		raw, err := w.env.ReadFile(rel)
		if err != nil {
			return fmt.Errorf("read generated %s: %w", rel, err)
		}
		w.docStepMaterialized = rel + ":\n" + raw
		var settings j18ClaudeSettings
		...
		w.docStepMaterialized = fmt.Sprintf("%s:\nPreToolUse matcher %q -> \"ltk evaluate\"\n(covers %v; identical to ltk's own cmd/ltk/loadout.yaml)", rel, matcher, j18LtkGuardedTools)
		return nil
	})
	...
	ctx.Step(`^the "ltk" companion is not installed on Bob's machine$`, func(c context.Context) error {
		// Only the "not installed" arm is implemented: no J18 scenario needs
		// ltk "installed" on Bob's machine (scenario 1 already proves the
		// present-companion contribution, on Alice) — mirrors J8's own "not
		// installed" behavior for reprise (scrub only, no fake install).
		return j18ScrubFromPath(worldFrom(c), "ltk")
	})
	...
	// J8's own reprise-envelope Given), "Bob starts a session", "his
	...
	// registered by steps_j8_onboarding.go — reused verbatim in
	// j18_guardrails.feature, no new registration here (godog rejects an
