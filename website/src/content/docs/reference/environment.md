...
| `PLUGIN_LISTEN_TCP` | Set to `1` to force the go-plugin transport to TCP-over-loopback instead of the default Linux unix-socket path. Exists for the integration test that exercises the same loopback transport macOS/Windows use (where the container's Linux VM makes a bind-mounted unix socket unreachable from the host kernel) |
