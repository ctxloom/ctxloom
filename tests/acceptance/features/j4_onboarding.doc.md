...
and it carries no assertions of its own — j4_onboarding.feature next to it is
...
J4's claim is that cloning alone should be the *whole* onboarding: `git clone`,
...
unconditionally, exactly as J2 already established for anyone on the team. But
...
reusing the same per-engine proof [J5](/journeys/j5-multi-engine/) already
...
materialized context on this delivery path at all today — a confirmed gap J5
documents on its own — so asserting a codex row here would claim coverage
that does not exist. And like J5, this proves only that Bob's engine-native
...
which one Bob actually uses. J4 builds directly on
[J2](/journeys/j2-team-authoring/)'s first-party authoring,
[J3](/journeys/j3-corporate-signed/)'s trust gate, and
[J5](/journeys/j5-multi-engine/)'s engine-native materialization — onboarding
