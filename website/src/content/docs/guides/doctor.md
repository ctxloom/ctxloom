...
- **`DOCTOR-CHECK-SIGNKEY-k1`** — a signing identity resolves via the exact resolver `ctxloom review`'s approve path and `ctxloom sign`/`--sign` use (explicit `sign.key`, then `git config user.signingkey`, then ssh-agent's sole identity)
...
- **`DOCTOR-CHECK-HOOKS-TRUST-d4`** — hooks and MCP registration per configured backend (the same read `ctxloom manage status` exposes), plus how many active signers the trust store carries
