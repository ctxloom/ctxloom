...
| Transcript | **None; scraper deleted outright.** `internal/kiro/session.go` was **deleted** in commit `6683bc4c` — not demoted to an importer — because it parsed kiro's private v1 `sessions/cli/*.jsonl` store while a real `kiro-cli chat --no-interactive` oneshot (the exact mode `Execute` uses) persists to a structurally different **v2 SQLite blob** at `$XDG_DATA_HOME/kiro-cli/data.sqlite3` (tables `auth_kv`, `state`, `conversations_v2`), so it returned empty forever without erroring (`backend.go:18-26`). A `nil` history now fails loudly at both consumers |
