...
func TestNewRetractChecker(t *testing.T) {
	mf := newMockFetcher()

	checker := NewRetractChecker(mf, "owner", "repo", "v1")

	assert.NotNil(t, checker)
	assert.Equal(t, "owner", checker.owner)
	assert.Equal(t, "repo", checker.repo)
	assert.Equal(t, "v1", checker.version)
}

func TestRetractChecker_IsRetracted(t *testing.T) {
	ctx := context.Background()

	t.Run("delegates to CheckRetracted", func(t *testing.T) {
		mf := newMockFetcher()
		mf.defaultBranch = "main"
		mf.files["ctxloom/v1/manifest.yaml"] = []byte(`
version: 1
...
`)
		...
		retracted, reason, err := checker.IsRetracted(ctx, ref, ItemTypeBundle)

		require.NoError(t, err)
		assert.True(t, retracted)
		assert.Equal(t, "Vulnerability", reason)
	})
}
