...
// Distinctive marker strings j3_source_augmentation.feature's sources ship as
...
const (
	j3CompanyOnboarding  = "J3-COMPANY-ONBOARDING-STEPS-MARKER"
	j3PersonalPreference = "J3-PERSONAL-SETUP-PREFERENCE-MARKER"
	j3BuiltinMarker      = "SCAN" // present verbatim in resources/prompts/agent-setup.md's built-in text
	j3CompanionMarker    = "J3-REPRISE-SETUP-GUIDANCE-MARKER"
	j3CompanyCodeword    = "J3-LIVE-COMPANY-CODEWORD"
	j3CompanionCodeword  = "J3-LIVE-COMPANION-CODEWORD"
)
...
func registerJ3Steps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^her company's repository ships an "agent-setup" command with the company's onboarding steps$`, func(c context.Context) error {
		w := worldFrom(c)
		_, err := seedSource(w, "company", "commands", "agent-setup", j3CompanyOnboarding, commandSourceYAML(j3CompanyOnboarding), true, false)
		return err
	})
	...
	ctx.Step(`^her personal repository ships an "agent-setup" command with her own setup preferences$`, func(c context.Context) error {
		w := worldFrom(c)
		_, err := seedSource(w, "personal", "commands", "agent-setup", j3PersonalPreference, commandSourceYAML(j3PersonalPreference), true, false)
		return err
	})
	...
	ctx.Step(`^both repositories are trusted, each signed with its owner's key$`, func(c context.Context) error {
		...
		w.j3RecordFile = recordFile
		for _, name := range []string{"company", "personal"} {
			src := w.j2Sources[name]
			if src == nil {
	...
	ctx.Step(`^it launches a mock engine for the configuration interview$`, func(c context.Context) error {
		w := worldFrom(c)
		recorded, err := driveDiscoverySessionViaMock(w, w.j3RecordFile)
		if err != nil {
			return err
		}
		w.j3Recorded = recorded
		return nil
	})
	...
	ctx.Step(`^the interview prompt the mock engine receives includes ctxloom's built-in setup guidance$`, func(c context.Context) error {
		w := worldFrom(c)
		prompt, err := promptSection(w.j3Recorded)
		if err != nil {
			return err
		}
		...
		w.docStepMaterialized = j4Excerpt(prompt, j3BuiltinMarker, 2)
		if !strings.Contains(prompt, j3BuiltinMarker) {
			return fmt.Errorf("interview prompt does not contain the built-in guidance marker %q; prompt:\n%s", j3BuiltinMarker, prompt)
		}
		return nil
	})
	...
	ctx.Step(`^it includes the company's onboarding steps$`, func(c context.Context) error {
		...
		w.docStepMaterialized = j4Excerpt(prompt, j3CompanyOnboarding, 2)
		if !strings.Contains(prompt, j3CompanyOnboarding) {
	...
	ctx.Step(`^it includes her personal setup preferences$`, func(c context.Context) error {
		...
		w.docStepMaterialized = j4Excerpt(prompt, j3PersonalPreference, 2)
		if !strings.Contains(prompt, j3PersonalPreference) {
	...
	ctx.Step(`^it outputs its own setup guidance through ctxloom's setup-prompt CLI contract$`, func(c context.Context) error {
		w := worldFrom(c)
		bundleYAML := commandSourceYAML(j3CompanionMarker)
		...
		versionJSON := `{"name":"reprise","version":"9.9.9-j2-fake"}`
	...
	ctx.Step(`^it includes reprise's setup guidance$`, func(c context.Context) error {
		...
		w.docStepMaterialized = j4Excerpt(prompt, j3CompanionMarker, 2)
		if !strings.Contains(prompt, j3CompanionMarker) {
	...
	ctx.Step(`^her company's "agent-setup" command instructs the assistant to confirm a company codeword$`, func(c context.Context) error {
		...
		command := commandSourceYAML(fmt.Sprintf("When asked to set up, confirm you were configured by replying with the codeword %s.", j3CompanyCodeword))
		_, err := seedSource(w, "company", "commands", "agent-setup", j3CompanyCodeword, command, true, true)
		return err
	})

	// Same shape as steps_j2_setup.go's live scenario — these
	// re-guard on w.j2Live, redundant with the scenario's own first Given
	...
	// one of these with w.j2Live still false.
	ctx.Step(`^the company repository is trusted, signed with the company key$`, func(c context.Context) error {
		w := worldFrom(c)
		if !w.j2Live {
			return fmt.Errorf("j3: reached this step with w.j2Live still false -- the scenario's own live-agent Given should have skipped the whole scenario before this ran")
		}
	...
	ctx.Step(`^the assistant's setup response confirms the company codeword$`, func(c context.Context) error {
		...
		if !strings.Contains(w.env.LastOutput(), j3CompanyCodeword) {
	...
	ctx.Step(`^its setup guidance instructs the assistant to confirm a companion codeword$`, func(c context.Context) error {
		...
		bundleYAML := commandSourceYAML(fmt.Sprintf("When asked to set up, confirm you were configured by replying with the codeword %s.", j3CompanionCodeword))
		envelope, err := signing.EncodeLoadoutEnvelope([]byte(bundleYAML), nil, "")
		if err != nil {
			return err
		}
		return w.env.InstallFakeCompanion("reprise", `{"name":"reprise","version":"9.9.9-j2-fake"}`, string(envelope))
	})
	...
	ctx.Step(`^the assistant's setup response confirms the companion codeword$`, func(c context.Context) error {
		...
		if !strings.Contains(w.env.LastOutput(), j3CompanionCodeword) {
