...
// J5: "Dana in her editor" (j5_editor.feature) — FLOWS-UNIFIED.md's U10.
//
...
// j5State is this journey's fixture state.
type j5State struct {
	ready bool
}
...
const (
	// j5Marker is the context Dana's binding composes — the thing that must
	// still reach her when she works through an editor instead of a terminal.
	j5Marker = "J5-EDITOR-BINDING-MARKER"
	// j5Agent is the agent Zed would be pointed at.
	j5Agent = "dev"
)

// j5Setup is the Background: a project with an agent binding carrying real
// composed context, which is what any door into ctxloom is supposed to deliver.
func j5Setup(w *World) error {
	st := j5Of(w)
	if st.ready {
		return nil
	}
	...
	if err := runOK(w, "agent", "create", j5Agent, "--engine", "claude-code", "--profiles", "default"); err != nil {
		return err
	}
	st.ready = true
	return nil
