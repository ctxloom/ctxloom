...
// J16: "a signature somebody can check" (j16_signing.feature) — the PRODUCTION
// half of ctxloom's signing story, which no acceptance scenario had ever
// driven. J15 proves CONSUMPTION (tamper, retraction, key revocation, rejection
...
//
// SEAM WITH J15 (docs/journey-coverage-gaps.md §J16, "one thing to decide"):
...
// step here sets HOME, USERPROFILE or XDG_* itself; the one variable J16 does
...
// environment.
//
// SSH_AUTH_SOCK AND J15. steps_j15.go:473-478 deliberately BLANKS SSH_AUTH_SOCK
// so an ambient developer agent cannot satisfy key discovery and turn its
// "no signing key anywhere" scenario into a host-dependent coin flip. J16 does
...
// leave a value behind for the other: J15 still gets its guaranteed-empty
// agent, J16 still gets its guaranteed-present one.
package acceptance
...
const (
	// j16KeyComment is the ssh-agent COMMENT Trent's key is loaded under —
	// the identity string a human recognizes, and the one
	// agentkey.resolveByComment matches for `--key <name>`.
	j16KeyComment = "trent@acme.example"
	// j16Principal is the allowed_signers principal Trent's team trusts.
	...
	j16Principal = "context@acme.example"
	// j16PubKeyFile is where Trent keeps the public half, inside his
	// project — the path both `git config user.signingkey` and
	// `trust signer add --key` are pointed at.
	j16PubKeyFile = "acme-publish.pub"
	...
	j16TDDMarker     = "J16-TDD-GUIDANCE-MARKER"
	j16TDDRevised    = "J16-TDD-REVISED-MARKER"
	j16CurlMarker    = "J16-CURL-PIPE-SH-MARKER"
	j16PublishedName = "secure-coding"
)

// j16BundlesDir is the authored (committed, publishable) bundle tree — the set
// `bundle sign --all` signs and `bundle move` relocates out of.
const j16BundlesDir = ".ctxloom/content/bundles"

// j16State is this journey's fixture state.
type j16State struct {
	...
	// off. See j16SeedFromDisk for why the authoring copy cannot stay.
	publishedDir string
...
// j16Fragment is one authored fragment: name plus the marker string that IS
// its content.
type j16Fragment struct{ name, content string }

// j16BundleYAML renders an authored bundle manifest carrying frags in the
...
func j16BundleYAML(frags ...j16Fragment) string {
...
// j16BundlePath is the authored bundle file's project-relative path.
func j16BundlePath(name string) string { return j16BundlesDir + "/" + name + ".yaml" }

// j16VerifyDetachedSignature is THE assertion this journey exists for: read
...
func j16VerifyDetachedSignature(w *World, bundleName string) error {
	rel := j16BundlePath(bundleName)
	...
	if err := signing.Verify([]byte(body), []byte(sig), j16Of(w).signer.Public, signing.NamespacePublish); err != nil {
		return fmt.Errorf("%s does not verify against the %d bytes of %s read fresh off disk, under Trent's key %s: %w",
			sigPath, len(body), rel, j16Of(w).signer.Fingerprint(), err)
	}
	return nil
}

// j16DeclaredSignersPath is where a repository declares WHO may publish it:
...
const j16DeclaredSignersPath = ".github/allowed_signers"

// j16DeclarePublisher writes that declaration: principal, the publish
// namespace, and pub's key line — byte-for-byte the shape of
// ctxloom-default's own .github/allowed_signers.
func j16DeclarePublisher(w *World, principal string, key *testenv.TestSigner) error {
	st := j16Of(w)
	st.declared = key
	...
	return w.env.WriteFile(j16DeclaredSignersPath, line+"\n")
}

// j16DirBundleDir / j16DirBundleManifest name a DIRECTORY-form bundle's tree
// and its manifest — the shape whose detached signature was never refreshed.
func j16DirBundleDir(name string) string      { return j16BundlesDir + "/" + name }
func j16DirBundleManifest(name string) string { return j16DirBundleDir(name) + "/bundle.yaml" }

// j16VerifyDirectorySignature is defect B's assertion: the detached signature
...
func j16VerifyDirectorySignature(w *World, name string) error {
	rel := j16DirBundleManifest(name)
	body, err := w.env.ReadFile(rel)
	if err != nil {
	...
	if err := signing.Verify([]byte(body), []byte(sig), j16Of(w).signer.Public, signing.NamespacePublish); err != nil {
		return fmt.Errorf("%s does not cover the %d bytes of %s read fresh off disk under Trent's key %s — the signature "+
			"is STALE, which reads downstream as tampering: %w", sigPath, len(body), rel, j16Of(w).signer.Fingerprint(), err)
	}
	return nil
}

// j16VerifyTreeAttestation runs the CONSUMER's verifier over a directory
...
func j16VerifyTreeAttestation(w *World, name string) error {
	dir := filepath.Join(w.env.ProjectDir, filepath.FromSlash(j16DirBundleDir(name)))
	...
	root := allowedsigners.NewStore(allowedsigners.Entry{
		Principals: []string{j16Principal},
		Namespaces: []string{signing.NamespacePublish},
		PublicKey:  j16Of(w).signer.Public,
		KeyType:    j16Of(w).signer.Public.Type(),
	})
...
// j16ListSignatures returns every `.sig` file under the authored bundle tree,
// project-relative and sorted, so a scenario can assert a COUNT.
func j16ListSignatures(w *World) ([]string, error) {
	dir := filepath.Join(w.env.ProjectDir, filepath.FromSlash(j16BundlesDir))
	entries, err := os.ReadDir(dir)
	if err != nil {
		if os.IsNotExist(err) {
			return nil, nil
		}
		return nil, fmt.Errorf("read %s: %w", j16BundlesDir, err)
	}
	var out []string
...
// j16ListBundles returns every authored bundle file name (without the .yaml).
func j16ListBundles(w *World) ([]string, error) {
	dir := filepath.Join(w.env.ProjectDir, filepath.FromSlash(j16BundlesDir))
	entries, err := os.ReadDir(dir)
	if err != nil {
		if os.IsNotExist(err) {
...
// j16Setup is the Background: a hermetic publisher's machine. Deliberately
// scaffolds config.yaml WITHOUT steps_j2_common.go's seed bundle+profile —
...
func j16Setup(w *World) error {
	...
	if err := w.env.WriteFile(".ctxloom/config.yaml", buildJ2Config("claude-code", "claude-code")); err != nil {
		return err
	}
	...
	if err := os.MkdirAll(filepath.Join(w.env.ProjectDir, filepath.FromSlash(j16BundlesDir)), 0o755); err != nil {
	...
	st.signer = signer
	if err := w.env.WriteFile(j16PubKeyFile, signer.AuthorizedKey(j16KeyComment)); err != nil {
		return fmt.Errorf("write Trent's public key: %w", err)
	}

	sock, stop, err := testenv.StartSSHAgent(w.env.Root, testenv.SSHAgentIdentity{Signer: signer, Comment: j16KeyComment})
	if err != nil {
	...
	// this coexists with steps_j15.go's deliberate blanking of the same key.
	...
	if err := w.env.GitConfigLocal("user.signingkey", filepath.Join(w.env.ProjectDir, j16PubKeyFile)); err != nil {
		return err
	}
...
// j16PublishedBundle writes Trent's flagship bundle: two fragments, each
...
func j16PublishedBundle(tddContent string) string {
	return j16BundleYAML(
		j16Fragment{name: "tdd", content: tddContent},
		j16Fragment{name: "curl-pipe-sh", content: j16CurlMarker},
	)
}

// j16SeedFromDisk publishes what is ON DISK — the authored bundle bytes and
...
func j16SeedFromDisk(w *World, name string) error {
	st := j16Of(w)
	rel := j16BundlePath(name)
	body, err := w.env.ReadFile(rel)
	if err != nil {
...
// j16VerifyPublished verifies the archived PUBLISHED pair — the exact bytes
// and the exact CLI-produced signature that went to the remote — read fresh
// off disk, under Trent's key.
func j16VerifyPublished(w *World, name string) error {
	st := j16Of(w)
	if st.publishedDir == "" {
...
// j16Reference wires the company remote into the consuming project the way a
// developer does — remote add (an address; never trust), reference it from the
// composed profile, then pull.
func j16Reference(w *World) error {
	st := j16Of(w)
	if st.referenced {
	...
	if err := runOK(w, "bundle", "create", "seed", "-d", "J16 seed bundle"); err != nil {
		return err
	}
	if err := runOK(w, "profile", "create", "default", "-b", "seed", "-d", "J16 default profile"); err != nil {
		return err
	}
	if err := runOK(w, "remote", "create", "company", st.url, "--forge", "git"); err != nil {
		return err
	}
	if err := runOK(w, "profile", "modify", "default", "--add-bundle", "company/"+j16PublishedName); err != nil {
		return err
	}
	st.referenced = true
	return runOK(w, "remote", "pull")
...
// j16ItemRef builds the canonical item ref for a fragment of the PUBLISHED
// bundle — the same grammar `trust accept`, `trust reject` and `bundle sign`
// all share.
func j16ItemRef(w *World, fragment string) string {
	return "file://" + j16Of(w).bare + "@bundles/" + j16PublishedName + "#fragments/" + fragment
}

// j16Delivered materializes the default profile and returns the assembled
// context — the surface a scenario asserts a marker reached, or did not.
func j16Delivered(w *World) (string, error) {
...
// j16AssertDelivery checks a marker's presence in the assembled context.
func j16AssertDelivery(w *World, marker string, want bool) error {
	body, err := j16Delivered(w)
	if err != nil {
		return err
	}
...
// j16AssertReviewState reads the review-state LABEL `ctxloom fragment list
...
func j16AssertReviewState(w *World, fragment, want string) error {
	...
	for _, row := range rows {
		...
		if b, _ := row["bundle_label"].(string); b != j16PublishedName {
			continue
		}
	...
	return fmt.Errorf("no %q fragment of bundle %q in `fragment list --format json`:\n%s", fragment, j16PublishedName, out)
}

// j16EmbeddedPrincipals returns the principals ctxloom's compiled-in trust
...
func j16EmbeddedPrincipals() []string {
	var out []string
...
// j16StoreEntry is one parsed allowed_signers line.
type j16StoreEntry struct {
	principal  string
	namespaces string
	pub        ssh.PublicKey
}

// j16ParseStore parses an allowed_signers file into entries, so an assertion
...
func j16ParseStore(body string) ([]j16StoreEntry, error) {
	var out []j16StoreEntry
	for _, line := range strings.Split(body, "\n") {
		...
		entry := j16StoreEntry{principal: fields[0]}
...
// j16AssertTrustRoot asserts that body (an allowed_signers file's contents)
...
func j16AssertTrustRoot(w *World, label, body, principal string, wantNamespaces ...string) error {
	if len(wantNamespaces) == 0 {
		wantNamespaces = []string{signing.NamespacePublish}
	}
	entries, err := j16ParseStore(body)
	if err != nil {
		return fmt.Errorf("%s: %w", label, err)
	}
	want := j16Of(w).signer.Fingerprint()
	for _, e := range entries {
...
func registerJ16Steps(ctx *godog.ScenarioContext) {
	...
	ctx.After(func(c context.Context, sc *godog.Scenario, err error) (context.Context, error) {
		w := worldFrom(c)
		if w == nil || w.j16 == nil || w.j16.stopAgent == nil {
			return c, nil
		}
		stop := w.j16.stopAgent
		w.j16.stopAgent = nil
	...
	ctx.Step(`^Trent's project publishes a bundle "([^"]*)" carrying the fragment "([^"]*)"$`, func(c context.Context, name, frag string) error {
		w := worldFrom(c)
		return w.env.WriteFile(j16BundlePath(name), j16BundleYAML(j16Fragment{name: frag, content: j16TDDMarker}))
	})

	ctx.Step(`^Trent's project publishes the "([^"]*)" bundle his team depends on$`, func(c context.Context, name string) error {
		w := worldFrom(c)
		return w.env.WriteFile(j16BundlePath(name), j16PublishedBundle(j16TDDMarker))
	})
	...
	ctx.Step(`^Trent's project publishes (\d+) bundles$`, func(c context.Context, n int) error {
		...
		for i := 0; i < n; i++ {
			...
			body := j16BundleYAML(j16Fragment{name: "guidance", content: fmt.Sprintf("%s-%d", j16TDDMarker, i)})
			if err := w.env.WriteFile(j16BundlePath(name), body); err != nil {
				return err
			}
		}
		return nil
	...
	ctx.Step(`^Trent's project publishes the directory bundle "([^"]*)" carrying the fragment "([^"]*)"$`, func(c context.Context, name, frag string) error {
		w := worldFrom(c)
		return w.env.WriteFile(j16DirBundleManifest(name), j16BundleYAML(j16Fragment{name: frag, content: j16TDDMarker}))
	})
	...
	ctx.Step(`^Trent's repo declares "([^"]*)" as the only identity allowed to publish, and it is Trent's own key$`, func(c context.Context, principal string) error {
		w := worldFrom(c)
		return j16DeclarePublisher(w, principal, j16Of(w).signer)
	})
	...
	ctx.Step(`^the refusal names the key it would have signed with and the principal the repo requires$`, func(c context.Context) error {
		...
		for _, want := range []struct{ what, text string }{
			...
			{"where that requirement is declared", j16DeclaredSignersPath},
		} {
		...
		for _, name := range bundles {
			...
			if verr := signing.Verify([]byte(body), []byte(sig), st.declared.Public, signing.NamespacePublish); verr != nil {
				return fmt.Errorf("%s.sig does not verify under %s, the key %s declares for %s — this is exactly what a "+
					...
					rel, ssh.FingerprintSHA256(st.declared.Public), j16DeclaredSignersPath, st.declaredPrincipal, verr)
			}
		}
		return nil
	})
	...
	ctx.Step(`^the signature beside the directory bundle "([^"]*)" verifies against its bundle\.yaml on disk$`, func(c context.Context, name string) error {
		return j16VerifyDirectorySignature(worldFrom(c), name)
	})
	...
	ctx.Step(`^the directory bundle "([^"]*)" still verifies as a whole tree, with nothing unclaimed$`, func(c context.Context, name string) error {
		return j16VerifyTreeAttestation(worldFrom(c), name)
	})

	ctx.Step(`^Trent edits the bundle "([^"]*)" after signing it$`, func(c context.Context, name string) error {
		w := worldFrom(c)
		body, err := w.env.ReadFile(j16BundlePath(name))
		if err != nil {
			return err
		}
		return w.env.WriteFile(j16BundlePath(name), body+"  late-addition:\n    content: \"added after the signature\"\n")
	})
	...
	ctx.Step(`^the signature beside bundle "([^"]*)" is non-empty and verifies against the bundle's bytes on disk$`, func(c context.Context, name string) error {
		return j16VerifyDetachedSignature(worldFrom(c), name)
	})
	...
	ctx.Step(`^every published bundle carries a signature that verifies, and there are exactly (\d+)$`, func(c context.Context, want int) error {
		...
		for _, name := range bundles {
			if err := j16VerifyDetachedSignature(w, name); err != nil {
				return err
			}
		}
		return nil
	...
	ctx.Step(`^Trent's key is trusted in the committable project store as "([^"]*)"$`, func(c context.Context, principal string) error {
		return runOK(worldFrom(c), "trust", "signer", "create", principal, "--key", j16PubKeyFile, "--project", "--yes")
	})

	ctx.Step(`^Trent's key is also trusted in his personal user store as "([^"]*)"$`, func(c context.Context, principal string) error {
		return runOK(worldFrom(c), "trust", "signer", "create", principal, "--key", j16PubKeyFile, "--yes")
	})

	// A review decision recorded by someone holding a signing key is SIGNED
	// (J15's forgery scenario proves the converse: a team decision refuses to be
	...
	// j16_signing.feature: WITHOUT this grant, `ctxloom trust accept` prints
	...
	ctx.Step(`^Alice's own review key is trusted for approve and reject as "([^"]*)"$`, func(c context.Context, principal string) error {
		return runOK(worldFrom(c), "trust", "signer", "create", principal, "--key", j16PubKeyFile, "--namespace", "approve,reject", "--yes")
	})
	...
	ctx.Step(`^the project store "([^"]*)" trusts "([^"]*)" for publishing, with Trent's own key$`, func(c context.Context, rel, principal string) error {
		...
		return j16AssertTrustRoot(w, "the project store "+rel, body, principal)
	})
	...
	ctx.Step(`^the user store "([^"]*)" was never written$`, func(c context.Context, rel string) error {
		...
		entries, perr := j16ParseStore(body)
		if perr != nil {
	...
	ctx.Step(`^Trent's team trusts "([^"]*)" in the committable project store$`, func(c context.Context, principals string) error {
		...
		for _, p := range strings.Split(principals, ",") {
			if err := runOK(w, "trust", "signer", "create", strings.TrimSpace(p), "--key", j16PubKeyFile, "--namespace", "publish", "--project", "--yes"); err != nil {
				return err
			}
		}
		return nil
	...
	ctx.Step(`^the project store "([^"]*)" holds exactly the entries for "([^"]*)"$`, func(c context.Context, rel, principals string) error {
		...
		for _, p := range want {
			if aerr := j16AssertTrustRoot(w, "the project store "+rel, body, p); aerr != nil {
				return aerr
			}
		}
	...
	ctx.Step(`^the project store "([^"]*)" trusts "([^"]*)" for exactly the namespaces "([^"]*)"$`, func(c context.Context, rel, principal, namespaces string) error {
		...
		return j16AssertTrustRoot(w, "the project store "+rel, body, principal, strings.Split(namespaces, ",")...)
	})
	...
	ctx.Step(`^the project store "([^"]*)" holds one entry with no trailing newline$`, func(c context.Context, rel string) error {
		...
		line := fmt.Sprintf("handwritten@acme.example namespaces=%q %s",
			signing.NamespacePublish, strings.TrimSpace(j16Of(w).signer.AuthorizedKey(j16KeyComment)))
	...
	ctx.Step(`^ctxloom's own compiled-in publishing key is trusted$`, func(c context.Context) error {
		w := worldFrom(c)
		if len(j16EmbeddedPrincipals()) == 0 {
	...
	ctx.Step(`^Trent publishes the signed bundle to his company repo, and Alice references it$`, func(c context.Context) error {
		w := worldFrom(c)
		if err := j16SeedFromDisk(w, j16PublishedName); err != nil {
			return err
		}
		return j16Reference(w)
	})
	...
	ctx.Step(`^her assistant no longer receives the "([^"]*)" guidance either$`, func(c context.Context, which string) error {
		marker, err := j16MarkerFor(which)
		if err != nil {
			return err
		}
		return j16AssertDelivery(worldFrom(c), marker, false)
	})

	ctx.Step(`^the published "([^"]*)" fragment's review state is "([^"]*)"$`, func(c context.Context, frag, want string) error {
		return j16AssertReviewState(worldFrom(c), frag, want)
	})
	...
	ctx.Step(`^Trent revises the "([^"]*)" fragment, re-signs it, and publishes again$`, func(c context.Context, frag string) error {
		...
		if err := w.env.WriteFile(j16BundlePath(j16PublishedName), j16PublishedBundle(j16TDDRevised)); err != nil {
			return err
		}
		if err := runOK(w, "bundle", "sign", j16PublishedName); err != nil {
			return err
		}
		return j16SeedFromDisk(w, j16PublishedName)
	})

	ctx.Step(`^I run "ctxloom trust accept" on the published "([^"]*)" fragment$`, func(c context.Context, frag string) error {
		return runOK(worldFrom(c), "trust", "accept", j16ItemRef(worldFrom(c), frag))
	})

	ctx.Step(`^I run "ctxloom trust reject" on the published "([^"]*)" fragment$`, func(c context.Context, frag string) error {
		return runOK(worldFrom(c), "trust", "reject", j16ItemRef(worldFrom(c), frag))
	})
	...
	ctx.Step(`^I try to run "ctxloom trust accept" on the published "([^"]*)" fragment$`, func(c context.Context, frag string) error {
		w := worldFrom(c)
		_ = w.env.Run("trust", "accept", j16ItemRef(w, frag))
		return nil
	})
	...
	ctx.Step(`^her assistant receives the "([^"]*)" guidance$`, func(c context.Context, which string) error {
		...
		return j16AssertDelivery(worldFrom(c), marker, true)
	})
	...
	ctx.Step(`^the content Trent signed still verifies against the bytes he published$`, func(c context.Context) error {
		return j16VerifyPublished(worldFrom(c), j16PublishedName)
	})
	...
	ctx.Step(`^the relocated "([^"]*)" is byte-identical to what was signed, and its signature still verifies$`, func(c context.Context, name string) error {
		...
		if err := signing.Verify(body, sig, j16Of(w).signer.Public, signing.NamespacePublish); err != nil {
	...
	ctx.Step(`^the source bundle "([^"]*)" and its signature are gone$`, func(c context.Context, name string) error {
		w := worldFrom(c)
		for _, rel := range []string{j16BundlePath(name), j16BundlePath(name) + ".sig"} {
			if w.env.FileExists(rel) {
	...
	ctx.Step(`^the shared standards directory is still empty$`, func(c context.Context) error {
		w := worldFrom(c)
		entries, err := os.ReadDir(j16Of(w).sharedDir)
		if err != nil {
...
// j16MarkerFor maps a fragment's human name to the marker string its content
...
func j16MarkerFor(which string) (string, error) {
	switch which {
	case "tdd":
		return j16TDDMarker, nil
	case "revised tdd":
		return j16TDDRevised, nil
	case "curl-pipe-sh":
		return j16CurlMarker, nil
	default:
		return "", fmt.Errorf("no J16 marker is defined for %q", which)
	}
}
