...
ctxloom profile create testing -b ctxloom-main/testing
ctxloom remote pull
```
...
- `/recover` - Recover context from a previous session after `/clear`
...
<CardGrid>
  ...
  <Card title="llm-tool-killer (ltk)" icon="approve-check">
    Companion tool — [guide the commands your agent runs](/ecosystem/llm-tool-killer/), not just its context.
  </Card>
</CardGrid>
...
ctxloom is primarily tested with **Claude Code** (Anthropic's CLI). Secondary testing is performed with **Gemini CLI**, which is the next LLM to receive comprehensive testing coverage. Other LLM backends may work but are not actively tested.
:::
