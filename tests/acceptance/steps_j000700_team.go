...
// J000700: "a team lead shares a command with the team" journey
// (j000700_team_authoring.feature). Carol and Bob are two team members with
...
// steps_j000200_common.go's fragmentSourceYAML/commandSourceYAML) rather than
...
const (
	j000700Bundle  = "team-standards"
	j000700Profile = "default"

	// j000700CommandMarkerV1/V2 are the "before"/"after" content of the
	...
	j000700CommandMarkerV1 = "J000700-CONVENTIONAL-COMMITS-V1-USE-IMPERATIVE-MOOD-IN-THE-SUBJECT-LINE"
	j000700CommandMarkerV2 = "J000700-CONVENTIONAL-COMMITS-V2-INCLUDE-A-SCOPE-IN-THE-SUBJECT-LINE"

	// j000700FragVerboseMarker/DistilledMarker are scenario 2's original/compact
	...
	j000700FragVerboseMarker   = "J000700-VERBOSE-ORIGINAL-FRAGMENT-GUIDANCE-KEEP-THIS-LONG-FORM-TEXT-INTACT-FOR-THE-TEST"
	j000700FragDistilledMarker = "J000700-DISTILLED-COMPACT-FRAGMENT-GUIDANCE"
)
...
// j000700 returns (lazily allocating) this scenario's J000700 state.
func (w *World) j000700() *j000700State {
	if w.j000700s == nil {
		w.j000700s = &j000700State{}
	}
	return w.j000700s
}

func registerJ000700Steps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^Carol is working in the team's project$`, func(c context.Context) error {
		return j000700SetupTeamProject(worldFrom(c))
	})

	ctx.Step(`^Carol authors a "([^"]*)" command$`, func(c context.Context, name string) error {
		return j000700AuthorCommand(worldFrom(c), name, j000700CommandMarkerV1)
	})

	ctx.Step(`^she commits it to the project$`, func(c context.Context) error {
		w := worldFrom(c)
		return j000700CommitAndPush(w, "author "+w.j000700().commandName+" command")
	})
	...
	ctx.Step(`^Bob's assistant can invoke the conventional-commits command$`, func(c context.Context) error {
		w := worldFrom(c)
		if err := runBob(w, "profile", "materialize", j000700Profile, "--target", "out"); err != nil {
			return err
		}
		body, err := findBobCommandFile(w, w.j000700().commandNameOrDefault())
		if err != nil {
			return err
		}
	...
	ctx.Step(`^Carol distills the fragment$`, func(c context.Context) error {
		return runOK(worldFrom(c), "fragment", "distill", j000700Bundle+"#fragments/guidance", "-f")
	})

	ctx.Step(`^she commits both forms$`, func(c context.Context) error {
		return j000700CommitAndPush(worldFrom(c), "distill guidance fragment")
	})
	...
	ctx.Step(`^Bob's assistant receives the distilled guidance$`, func(c context.Context) error {
		...
		if !strings.Contains(body, j000700FragDistilledMarker) {
	...
	ctx.Step(`^it does not receive the verbose original$`, func(c context.Context) error {
		...
		if strings.Contains(body, j000700FragVerboseMarker) {
	...
	ctx.Step(`^Bob's assistant already has Carol's conventional-commits command$`, func(c context.Context) error {
		...
		if err := j000700AuthorCommand(w, "conventional-commits", j000700CommandMarkerV1); err != nil {
			return err
		}
		if err := j000700CommitAndPush(w, "author conventional-commits command"); err != nil {
			return err
		}
		if err := j000700BobPull(w); err != nil {
			return err
		}
		return runBob(w, "profile", "materialize", j000700Profile, "--target", "out")
	})

	ctx.Step(`^Carol edits the command$`, func(c context.Context) error {
		w := worldFrom(c)
		return j000700AuthorCommand(w, w.j000700().commandNameOrDefault(), j000700CommandMarkerV2)
	})

	ctx.Step(`^she commits the change$`, func(c context.Context) error {
		return j000700CommitAndPush(worldFrom(c), "edit conventional-commits command")
	})
	...
	ctx.Step(`^Bob's assistant has the updated command$`, func(c context.Context) error {
		...
		if !strings.Contains(body, j000700CommandMarkerV2) {
...
func (s *j000700State) commandNameOrDefault() string {
	if s.commandName == "" {
...
// j000700SetupDistillProject: git-init Carol's checkout on a deterministic "main"
...
func j000700SetupProject(w *World, configYAML string) error {
	...
	_, err = j000700Git(w.env.ProjectDir, "push", "-u", "origin", "main")
	return err
}

// j000700AuthorCommand overwrites the team bundle's YAML file so it carries exactly
...
// to be meaningful). Records the name on World's J000700 state for later steps.
...
func j000700FragmentBundleYAML(content string) string {
	return fmt.Sprintf("version: \"1.0.0\"\nfragments:\n  guidance:\n    content: %q\n", content)
}

// j000700CommitAndPush commits every change in Carol's checkout and pushes it to
// the team origin — "she commits it to the project" / "she commits both
// forms" / "she commits the change".
func j000700CommitAndPush(w *World, message string) error {
	if err := w.env.GitCommit(message); err != nil {
		return err
	}
	_, err := j000700Git(w.env.ProjectDir, "push", "origin", "main")
	return err
}

// j000700BobPull is "Bob pulls the project" / "...again" / "...with distilled
...
func j000700BobPull(w *World) error {
	j000700 := w.j000700()
	if j000700.bareOrigin == "" {
	...
	_, err := j000700Git(j000700.bobDir, "pull", "origin", "main")
	return err
}
...
// working directory. Fails loudly on a non-zero exit (mirroring steps_j000200_
...
func readBobFile(w *World, rel string) (string, error) {
	data, err := os.ReadFile(filepath.Join(w.j000700().bobDir, rel))
	if err != nil {
		return "", err
	}
...
func findBobCommandFile(w *World, commandName string) (string, error) {
	...
	if len(matches) == 0 {
		return "", fmt.Errorf("no materialized command file matches %q under %s", glob, w.j000700().bobDir)
	}
	...
	// sidecar (set-and-consume; a no-op when capture is off). J000700's teammate-side
...
// j000700GitEnv is the hermetic git invocation environment for J000700's own origin/
...
// j000700NewBareOrigin creates a fresh bare git repo (branch "main") under the
...
func j000700NewBareOrigin(w *World) (string, error) {
	root, err := os.MkdirTemp(w.env.Root, "j000700-origin-*")
	if err != nil {
		return "", err
	}
