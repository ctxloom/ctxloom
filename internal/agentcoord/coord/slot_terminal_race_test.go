...
// waitForSlotWaiters blocks until turnSlots has exactly n goroutines parked in
// acquire. This is the test's SYNCHRONISATION POINT, not a hope: a waiter
// channel is appended to s.waiters inside acquire and nowhere else, so
// observing the append proves the acquirer is genuinely parked on the cap —
// and, because the claim precedes the acquire in acquireRunSlot, that its
// claim (if the guard exists at all) is already recorded. Nothing below races
// a sleep.
func waitForSlotWaiters(t *testing.T, s *turnSlots, n int) {
	t.Helper()
	require.Eventually(t, func() bool {
		s.mu.Lock()
		defer s.mu.Unlock()
		return len(s.waiters) == n
	}, conformanceWait, time.Millisecond,
...
func TestRunChild_TerminateWhileParkedOnCap_DoesNotStrandTheSlot(t *testing.T) {
	...
	require.True(t, c.slots.tryAcquire(), "precondition: the single slot starts free")
	...
	waitForSlotWaiters(t, c.slots, 1)
	...
	require.Eventually(t, func() bool {
		...
		return c.slots.free == 1 && len(c.slots.waiters) == 0
	}, conformanceWait, time.Millisecond,
...
func TestAcquireRunSlot_Outcomes(t *testing.T) {
	t.Run("landed", func(t *testing.T) {
		...
		c.slots.mu.Lock()
		defer c.slots.mu.Unlock()
		assert.Equal(t, 0, c.slots.free, "the slot must actually have been taken from the pool")
	})
	...
	t.Run("cancelled by the run terminal mid-wait", func(t *testing.T) {
		...
		require.True(t, c.slots.tryAcquire())
		...
		c.slots.release() // the peer finishes; the parked acquire lands
	...
	t.Run("already accounted for", func(t *testing.T) {
		...
		c.slots.mu.Lock()
		defer c.slots.mu.Unlock()
		assert.Equal(t, 2, c.slots.free, "no second slot may be taken for an rt that already holds one")
	})
}
