...
Rules:
- Approve a fragment only if its premise describes THIS moment. Not a topic the
  situation mentions, not something you might do later.
- Loading costs context. Failing to load something you needed means you never
  learn it existed. Both are real errors; do not resolve that by approving
  everything.
- Selecting NOTHING is valid and is sometimes correct.
...
- config-hierarchy: You are adding or changing a flag, an environment variable, or a config key; working out which layer won when a setting resolved unexpectedly; or reading configuration to decide how something is declared. For anyone touching how this family of binaries is configured, however the moment is worded.
...
- coordination-tools: You are calling one of the agent coordination tools and need to know its semantics -- what agent_run returns, how agent_recv delivers, how a child addresses its parent, how an artifact is fetched. For anyone who has already decided to delegate and now needs the mechanics. Skip this when the question is WHETHER to delegate or HOW to write the brief.
- delegation: You are about to read several files, run a broad search, fetch a page, or take on a self-contained investigation yourself; or you are deciding who should do a piece of work. For anyone weighing doing it against handing it to a sub-agent.
...
- fail-loud-launch: You are writing a check that can find something wrong; deciding what a command should do when its input is broken, missing, or untrusted; or choosing between refusing and carrying on with less. For anyone deciding how loudly a failure should surface.
...
- isolation-axes: You are creating, configuring, or delegating to an agent; choosing a runtime or a workspace for one; or relying on isolation you did not explicitly specify. For anyone whose next action puts work in someone else's hands.
- ltk: A command you ran was blocked, refused, or redirected with a suggested replacement; or you are about to invoke a build, test, lint, or release tool directly rather than through the project's task runner. For anyone deciding how to run a command here.
...
- preexisting-ownership: A test, gate, or check is failing and you are deciding whose problem it is, or about to attribute it to something other than your own change. For anyone about to write or think 'pre-existing', 'unrelated', 'flaky', or 'not mine'.
- problem-solving: Something is failing and you are considering a workaround, a retry, a sleep, a threshold change, a disabled check, or a fallback; or you are deciding whether to chase the root cause or patch the symptom. For anyone about to route around a defect rather than fix it.
- prompt-authoring: You are about to write the brief, instructions, or task description a sub-agent will work from, or deciding what a child agent must return. For anyone composing instructions for an agent that will not see this conversation.
- prototype: You are about to preserve an old code path, add a shim, fallback, version check, or deprecation; hesitating to break existing callers; or choosing between a compatible fix and a correct one. For anyone weighing what already exists against doing it properly.
...
- sequential-thinking-usage: You are facing a multi-step plan, a decomposition, a dependency chain, or a design decision with more than one moving part. For anyone whose next answer is not obvious and where a wrong turn is expensive.
...
- turn-gates: You are choosing which build, test, lint, or mutation command to run; interpreting an exit code or a suite's output; or deciding whether the gate you ran actually covers the claim you are making. For anyone deciding which command proves a change is done. This is about WHICH gate and how to read it, not about whether a passing test means anything.
- unchecked-bindings: You are about to write prose that names something specific -- a list, a count, a file, a symbol, a supported set -- in a comment, doc, README, commit message, or config annotation; or you are relying on such a statement someone else wrote. For anyone about to state in words something the code can later contradict silently.
- worktree-isolation: You are dispatching an agent that will work in its own worktree or cell; you are that agent and about to write something someone else must read; or you are about to trust a report that work was written, archived, or cleaned up. For anyone whose output has to survive leaving a sandbox.
