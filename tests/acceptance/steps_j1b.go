...
const (
	j1bCompanyOnboarding  = "J1B-COMPANY-ONBOARDING-STEPS-MARKER"
	j1bPersonalPreference = "J1B-PERSONAL-SETUP-PREFERENCE-MARKER"
	j1bBuiltinMarker      = "SCAN" // present verbatim in resources/prompts/agent-setup.md's built-in text
	j1bCompanionMarker    = "J1B-REPRISE-SETUP-GUIDANCE-MARKER"
	j1bCompanyCodeword    = "J1B-LIVE-COMPANY-CODEWORD"
	j1bCompanionCodeword  = "J1B-LIVE-COMPANION-CODEWORD"
)
...
func registerJ1bSteps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^it launches a mock engine for the configuration interview$`, func(c context.Context) error {
		w := worldFrom(c)
		recorded, err := driveDiscoverySessionViaMock(w, w.j1bRecordFile)
		if err != nil {
			return err
		}
		w.j1bRecorded = recorded
		return nil
	})
	...
	ctx.Step(`^the interview prompt the mock engine receives includes ctxloom's built-in setup guidance$`, func(c context.Context) error {
		w := worldFrom(c)
		prompt, err := promptSection(w.j1bRecorded)
		if err != nil {
			return err
		}
	...
	ctx.Step(`^it outputs its own setup guidance through ctxloom's setup-prompt CLI contract$`, func(c context.Context) error {
		...
		versionJSON := `{"name":"reprise","version":"9.9.9-j1-fake"}`
	...
	ctx.Step(`^its setup guidance instructs the assistant to confirm a companion codeword$`, func(c context.Context) error {
		...
		return w.env.InstallFakeCompanion("reprise", `{"name":"reprise","version":"9.9.9-j1-fake"}`, string(envelope))
	})
