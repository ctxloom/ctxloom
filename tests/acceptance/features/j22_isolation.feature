...
Feature: Bounding what the agent can reach, even with permissions bypassed
  ...
  # WHAT THIS JOURNEY CAN AND CANNOT SEE (honesty, mirroring j21_delegation's
  ...
  # all. See j22_isolation.doc.md for the rendered matrix this proves and does
  # not prove, and steps_j22_isolation_matrix.go's own package doc for why
  ...
  # j22_isolation.doc.md for the rendered outcome matrix (isolated / LEAKS /
  ...
  # file-level note in steps_j22_isolation_matrix.go), so the payload it can
