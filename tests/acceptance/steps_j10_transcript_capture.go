...
// Package acceptance: J10, cross-engine transcript capture
// (j10_transcript_capture.feature). Every hermetic scenario is a pure
...
// j10State accumulates this journey's fixture state across a scenario's
...
func j10From(w *World) *j10State {
	if w.j10 == nil {
		w.j10 = &j10State{saved: map[string]string{}}
	}
	return w.j10
}

// j10FixtureFile maps the feature file's <engine> token to the REAL, shipped
...
var j10FixtureFile = map[string]string{
...
// j10RepoRoot resolves the repo root relative to THIS source file via
// runtime.Caller — steps_j18_guardrails.go's j18LtkLoadoutYAML precedent —
...
// j10SeededEngineVersion is the engine version a seeded session claims to have
...
func j10SeededEngineVersion(backend string) string {
	switch backend {
	case "claude-code":
		return "2.1.214"
	case "codex":
...
// j10AddIndexEntry appends one session to the home index.yaml, preserving
// every entry a prior call in this scenario already wrote (see j10State's
...
func j10AddIndexEntry(w *World, harp, backend, transcriptPath string) error {
	st := j10From(w)
	entry := fmt.Sprintf("  - harp_name: %s\n"+
		...
		harp, harp, backend, w.env.ProjectDir, transcriptPath, j10SeededEngineVersion(backend))
...
// j10CanonicalTranscriptRelPath resolves harp's captured canonical
...
func j10CanonicalTranscriptRelPath(w *World, harp string) (string, error) {
...
func j10ReadCanonicalTranscript(w *World, harp string) (string, error) {
	rel, err := j10CanonicalTranscriptRelPath(w, harp)
	if err != nil {
		return "", err
	}
...
// j10ReadCanonicalRecords parses harp's canonical transcript into the real
...
func j10ReadCanonicalRecords(w *World, harp string) ([]transcript.Record, error) {
	body, err := j10ReadCanonicalTranscript(w, harp)
	if err != nil {
		return nil, err
	}
...
// j10FindEntry returns the index and payload of the first KindEntry record
// matching pred, in file order.
func j10FindEntry(recs []transcript.Record, pred func(transcript.EntryPayload) bool) (int, *transcript.EntryPayload, bool) {
	for i, r := range recs {
...
// j10EngineTurnCheck is one real, grounded assertion about a shipped
...
type j10EngineTurnCheck struct {
	label string
	match func(transcript.EntryPayload) bool
}

// j10EngineTurnChecks pins the Scenario Outline's per-<engine> real-turn
...
var j10EngineTurnChecks = map[string][]j10EngineTurnCheck{
	"codex": {
...
// — so j10RemovedPrompt below must never appear anywhere in the converted
...
// j10QueueInterleavedFixture reproduces for the delivered second prompt.
const (
	j10SecondPrompt  = "Also, what's the capital of France?"
	j10RemovedPrompt = "nevermind, skip that, I changed my mind"
	j10FirstAnswer   = "6 times 7 is 42."
)

// j10QueueInterleavedFixture builds a claude-shaped vendor transcript where:
...
func j10QueueInterleavedFixture(sessionID string) string {
	lines := []string{
		...
		fmt.Sprintf(`{"type":"queue-operation","operation":"enqueue","timestamp":"2026-07-25T13:00:01.000Z","sessionId":%q,"content":%q}`, sessionID, j10SecondPrompt),
		fmt.Sprintf(`{"type":"queue-operation","operation":"enqueue","timestamp":"2026-07-25T13:00:02.000Z","sessionId":%q,"content":%q}`, sessionID, j10RemovedPrompt),
		fmt.Sprintf(`{"type":"queue-operation","operation":"remove","timestamp":"2026-07-25T13:00:03.000Z","sessionId":%q,"content":%q}`, sessionID, j10RemovedPrompt),
		fmt.Sprintf(`{"type":"queue-operation","operation":"dequeue","timestamp":"2026-07-25T13:00:04.000Z","sessionId":%q}`, sessionID),
		fmt.Sprintf(`{"type":"user","sessionId":%q,"message":{"role":"user","content":%q}}`, sessionID, j10SecondPrompt),
		fmt.Sprintf(`{"type":"assistant","sessionId":%q,"message":{"model":"claude-haiku-4-5-20251001","id":"msg_first","type":"message","role":"assistant","content":[{"type":"text","text":%q}],"stop_reason":"end_turn","usage":{"input_tokens":5,"output_tokens":6}}}`, sessionID, j10FirstAnswer),
...
func registerJ10Steps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^a recorded "([^"]*)" session "([^"]*)" bound to the shipped (\S+) vendor-transcript fixture$`,
		func(c context.Context, backend, harp, engineKey string) error {
			w := worldFrom(c)
			path, err := j10FixturePath(engineKey)
			if err != nil {
				return err
			}
			return j10AddIndexEntry(w, harp, backend, path)
		})

	ctx.Step(`^a recorded "([^"]*)" session "([^"]*)"$`, func(c context.Context, backend, harp string) error {
		return j10AddIndexEntry(worldFrom(c), harp, backend, "")
	})
	...
	ctx.Step(`^a recorded "([^"]*)" session "([^"]*)" that already has a canonical transcript and a shipped (\S+) vendor fixture$`,
		func(c context.Context, backend, harp, engineKey string) error {
			...
			if err := j10AddIndexEntry(w, harp, backend, path); err != nil {
				return err
			}
			original := j10SeedCanonicalTranscript(w, harp)
			j10From(w).saved["original:"+harp] = original
			return nil
		})
	...
	ctx.Step(`^a recorded "([^"]*)" session "([^"]*)" bound to a vendor log where a second prompt is delivered mid-response and a third is queued then removed$`,
		func(c context.Context, backend, harp string) error {
			w := worldFrom(c)
			rel := filepath.Join("j10-fixtures", harp+".jsonl")
			if err := w.env.WriteFile(rel, j10QueueInterleavedFixture(harp)); err != nil {
				return err
			}
			abs := filepath.Join(w.env.ProjectDir, rel)
			return j10AddIndexEntry(w, harp, backend, abs)
		})
	...
	ctx.Step(`^the canonical transcript for "([^"]*)" replays the fixture's real turns in order: the user goal, the assistant reply, and the Glob and Bash tool calls with their real inputs$`,
		func(c context.Context, harp string) error {
			w := worldFrom(c)
			recs, err := j10ReadCanonicalRecords(w, harp)
			if err != nil {
				return err
			}
			iUser, _, ok := j10FindEntry(recs, func(e transcript.EntryPayload) bool {
			...
			iAsst, _, ok := j10FindEntry(recs, func(e transcript.EntryPayload) bool {
			...
			iGlob, globEntry, ok := j10FindEntry(recs, func(e transcript.EntryPayload) bool {
			...
			iBash, bashEntry, ok := j10FindEntry(recs, func(e transcript.EntryPayload) bool {
	...
	ctx.Step(`^the canonical transcript for "([^"]*)" preserves the tool_result marked is_error from the rejected call$`,
		func(c context.Context, harp string) error {
			...
			_, result, ok := j10FindEntry(recs, func(e transcript.EntryPayload) bool {
	...
	ctx.Step(`^the canonical transcript for "([^"]*)" replays the (\S+) fixture's real conversation turns in seq order$`,
		func(c context.Context, harp, engine string) error {
			...
			checks, ok := j10EngineTurnChecks[engine]
			if !ok {
				return fmt.Errorf("no known real-turn assertions for engine %q", engine)
			}
			last := -1
			for _, chk := range checks {
				idx, _, found := j10FindEntry(recs, chk.match)
				if !found {
	...
	ctx.Step(`^the canonical transcript for "([^"]*)" still holds its original captured content$`, func(c context.Context, harp string) error {
		w := worldFrom(c)
		want, ok := j10From(w).saved["original:"+harp]
		if !ok {
	...
	ctx.Step(`^the canonical transcript for "([^"]*)" places the real answer to the first prompt after the interleaved second prompt, exactly as the vendor log ordered them$`,
		func(c context.Context, harp string) error {
			...
			iSecondPrompt, _, ok := j10FindEntry(recs, func(e transcript.EntryPayload) bool {
				return e.Type == "user" && strings.Contains(e.Content, j10SecondPrompt)
			})
			if !ok {
				return fmt.Errorf("canonical transcript for %q: the interleaved second prompt never arrived", harp)
			}
			iAnswer, _, ok := j10FindEntry(recs, func(e transcript.EntryPayload) bool {
				return e.Type == "assistant" && strings.Contains(e.Content, j10FirstAnswer)
			})
			if !ok {
	...
	ctx.Step(`^the canonical transcript for "([^"]*)" contains no trace of the removed, never-delivered prompt$`,
		func(c context.Context, harp string) error {
			...
			if strings.Contains(body, j10RemovedPrompt) {
				return fmt.Errorf("canonical transcript for %q leaked the removed, never-delivered prompt %q into a captured turn", harp, j10RemovedPrompt)
			}
			return nil
		})
}

// j10SeedCanonicalTranscript writes a minimal, realistic already-captured
...
// later.
func j10SeedCanonicalTranscript(w *World, harp string) string {
	...
	if err != nil {
		...
		panic(fmt.Sprintf("j10SeedCanonicalTranscript: marshal record: %v", err))
	}
	line := string(data) + "\n"
	rel := ".ctxloom/sessions/" + harp + "/persist/transcript.jsonl"
	if err := w.env.WriteHomeFile(rel, line); err != nil {
		panic(fmt.Sprintf("j10SeedCanonicalTranscript: write %s: %v", rel, err))
	}
	return line
}
