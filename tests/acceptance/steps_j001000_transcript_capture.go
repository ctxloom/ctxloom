...
func j001000From(w *World) *j001000State {
	if w.j001000 == nil {
		w.j001000 = &j001000State{saved: map[string]string{}}
	}
	return w.j001000
}
...
func j001000AddIndexEntry(w *World, harp, backend, transcriptPath string) error {
	...
	entry := fmt.Sprintf("  - harp_name: %s\n"+
		...
		harp, harp, backend, w.env.ProjectDir, transcriptPath, j001000SeededEngineVersion(backend))
