# Structured Patch (SP) — file format specification, v0 draft

**Status: DRAFT for human review (P1 gate).** Nothing here is implemented. No parser,
no backend, no CLI exists. This document plus `tests/spcorpus/` is the deliverable.

**Bound by the P0 rulings** (`ugly-icy-squid/structured-patch-standard.plan.md`): the name is
Structured Patch, the binary is `sp`, the extension is `.sp`. Formats implemented: JSON,
JSONC, YAML, TOML, Markdown, HCL. Formats documented only: INI-family, dotenv, XML (§12).

**Notation (human, 2026-08-14, superseding the earlier hybrid ruling): SP v0 is one grammar.**
...
**Architecture (human, 2026-08-14): four components around one IR.** Parser (`.sp` → IR) and
renderer (IR → `.sp`) are notation-side inverses; differ (two sources → IR) and applier
...
1. [Why SP exists, and the one property everything else serves](#1-why-sp-exists)
...
SP is that shape-mirroring-with-margins notation, generalized off C and onto config trees,
...
- **No fuzz factor.** SP has no analogue of `patch --fuzz`. Context either matches or the
  apply fails.
...
- **Already-applied is a failure, not a success** — by default. A `-` line whose node is
  absent is `SP010 stale-target` even if the corresponding `+` line's node is already
...
patch. A reader of a `.sp` file never has to learn a second grammar, and a reviewer never has
...
An SP file is UTF-8 text, LF-terminated lines. A trailing newline on the last line is
...
directive   := key ":" SP value LF
filesection := targetline ( hunk | comment | blank )+
targetline  := "--- " path [ SP attrlist ] LF
hunk        := hunkheader bodyline*
hunkheader  := "@@ " address " @@" [ SP attrlist ] LF
bodyline    := margin SP text LF | comment | blank
comment     := "#" [ SP text ] LF
attrlist    := attr ( SP attr )*
attr        := key "=" value
```
...
- `sp:` — **required**, must be the first non-comment, non-blank line. Value is the format
  version integer. This document specifies `1`. A reader seeing an unknown version MUST fail
  with `SP002` rather than attempt a best-effort parse.
- `format:` — optional default format for every file section that does not declare its own.

No other preamble keys are defined in v0. An unknown preamble key is `SP001` (fail loud;
...
- The path is relative to the apply root (the CLI's `--root`, default: the process working
  directory). Absolute paths are legal; `..` traversal above the root is `SP003`.
- `format=` is optional. Resolution order: `format=` attr → preamble `format:` → extension
  inference (§8.0) → `SP021 unsupported-format`.
...
A hunk header names the **anchor**: the SP path (§4) of the node the body mirrors. `@@ / @@`
anchors at the document root.

The anchor node must exist and must resolve to exactly one node, or the hunk fails
(`SP013 no-match` / `SP012 ambiguous-match`) — with two exceptions: a trailing `?` on the last
...
| `#` | **comment** | An SP comment. Ignored entirely. Not part of either projection. |
...
A line whose column 1 is none of the six margin characters is `SP001 parse-error`. There is
no "loose" mode.
...
The SP margin `#` and the target's `#` never collide because the target's lives at column 3.

---

## 4. SP paths

An SP path addresses a node. It is **RFC 6901 JSON Pointer plus four extensions**, chosen so
...
Escapes: `~0` = `~`, `~1` = `/`, **and SP adds `~2` = `=`** so that an object key containing
...
- **Exactly one element must match**, or `SP012 ambiguous-match` / `SP013 no-match`. This is
  the loud-staleness rule applied to addressing: an array that grew a duplicate key is a
  drifted array and SP says so by name.
...
Same uniqueness rule: zero matches is `SP013`, more than one is `SP012` — a duplicated scalar
in a list is drift, and SP names it instead of picking the first. This is the address the
...
**A repeated `(type, labels)` tuple is `SP012 ambiguous-match` unless the hunk carries an
...
effect: `SP013 no-match` at that segment becomes a creation instead of an error. Creation
...
same parent are `SP012 ambiguous-match`.
...
This is the normative core of SP, and it is what makes the notation implementable once per
...
1. **Match.** The before-image must match the target subtree at the anchor, under the
   matching rules of §6.1. Failure is `SP010 stale-target`, naming the first mismatching SP
...
The survey's canonical four-operation change-set, as one SP file:
...
That is where loud staleness lives in the lowered form, and it is why an SP patch is
...
and it is the single largest readability win SP has over RFC 7386 Merge Patch, which must
...
asserting — the case Merge Patch handles by accident and expensively, and SP handles on
purpose and cheaply.
...
A line-scoped annotation not followed by a body line, and not first in the hunk, is `SP001`.

### 7.1 `?` assertions

| Directive | Meaning | Failure |
|---|---|---|
| `? expect <sppath> = <value>` | The node exists and equals the value. Does not modify. | `SP011` |
| `? absent <sppath>` | The node does not exist. | `SP011` |
| `? exhaustive` | The listed children are the *complete* child set of the governed container. | `SP011` |
| `? count <sppath> = <n>` | The container has exactly `n` children. | `SP011` |
| `? kind <sppath> = <k>` | The node's kind is `k` ∈ `map\|seq\|scalar\|block\|section`. | `SP011` |
...
`/provider/"aws"` names two nodes, so it is `SP012` on its own. The ordinal is written as an
...
- `label=[…]` is **optional and redundant by design**: when present it is checked against the
  selected block's actual labels and a mismatch is `SP011`. It exists because a bare `ord=1`
...
its assertions do not hold. This is how SP expresses "check that the world is what I think it
...
is still `SP010 stale-target`, which is exactly the dangerous case a naive "just merge it"
tool papers over.
...
provide. A conformant linter should warn on every use, and the SP comment above it should say
why. (OP-06.)
...
|---|---|---|
| *(none)* | `SP014 already-exists` — the strict default | OP-02 |
...
is `SP021 unsupported-format` — SP never sniffs content to guess.
...
- `SP040 anchor-ambiguity` by default. SP will not guess whether you meant to change every
...
present at that site**. Removing it with `-` is `SP013 no-match`, not a silent success — you
...
1. An edit to a path that **already exists** is applied at whichever surface form the target
   actually uses. SP never adds a second surface for an existing path.
2. If a path is defined at **two** surfaces in the same document (which TOML forbids but real
   files occasionally contain), that is `SP041 surface-ambiguity`.
...
- **Expressions are compared as source text**, normalized for whitespace only. SP does not
...
- Alignment: `hclwrite` re-aligns `=` within a body. SP adopts the backend's alignment on any
  body it modified, and leaves untouched bodies byte-identical. The corpus pins this.
- **Repeated `(type, labels)` tuples**: `SP012 ambiguous-match` unless a `! match ord=`
...
Markdown is the one format where SP is not addressing a keyed tree, and it gets a dialect
...
- **A body line's margin applies to its whole line, but matching and ops are per block.**
  ...
  margins is `SP001`. To change one line of a paragraph, remove the paragraph and add the
...
begin marker is `SP002 target-parse-error` — refuse, do not repair. This is the direct SP
...
**Architecture ruling (human, 2026-08-14).** SP is four components around one intermediate
...
```
        NOTATION SIDE                    IR                    FORMAT SIDE
                                 ┌─────────────────┐
   .sp text ──[ parser ]───────► │                 │ ──[ applier ]──► patched bytes
                                 │  transform list │       (per format)
   .sp text ◄──[ renderer ]───── │   addresses     │ ◄──[ differ ]──── (old, new) bytes
                                 │   ops (6902)    │       (per format)
                                 │   assertions    │
                                 └─────────────────┘
```

- **Parser** (`.sp` → IR). Owns the notation. **Never touches a target file, never opens
...
- **Renderer** (IR → `.sp`). Notation side, format-agnostic like the parser. Writes the mirror
...
the `.sp` mirror grammar remains the only authoring surface designed to be read in a pull
request.
...
1. **Writing more context makes a patch stricter, not merely more readable.** `-U3` in
   unified diff buys reviewability; context radius in SP buys reviewability *and* drift
...
2. For each context and `-` line, in body order, emit a `test` transform at that node's SP
...
3. For each `? ` assertion, emit the corresponding transform: `expect` → `test`; `absent` →
   ...
   `test` with the corresponding qualifier. (These qualifiers are SP extensions to 6902's
...
|---|---|---|---|---|
| **Abstract** | SP paths, key-match and selectors intact | relative (`after:`/`before:`/`end`) | parser (§9.1), differ (§9.4) | applier, renderer |
| **Resolved** | RFC 6901 pointers, indices concrete | numeric indices | `Lower(ir, doc)` against a specific target | interop consumers, `sp apply --ops`, corpus `expected-ops.json` |
...
`sp apply --ops` prints it; `sp` cannot read it back, and v0 defines no way to author one.
...
An applier **must not**: see the `.sp` text, know what a margin is, apply a fuzz factor,
reorder transforms, skip a `test` it does not understand, or emit a partial document. An
applier that encounters a transform qualifier it does not implement must fail `SP020`, never
...
`.sp` file, on every run, in every implementation. This is what makes a diff case pinnable in
...
- `--context=N` sets it; `--context=0` emits margins only; `--context=all` emits every sibling
  of every touched container (which, combined with `? exhaustive`, is the strictest patch SP
  can express).
...
and emits an SP comment saying so. See [O18](#open-questions-for-ratification) — a hardcoded
...
**R6 — Inexpressible changes.** If the structural diff contains a change SP v0 cannot express
(Appendix C), the differ fails with `SP020` naming the change. It MUST NOT silently degrade —
...
Canonical invocation: `sp diff HEAD:config.yaml config.yaml` — "what have I changed since the
...
### 9.6 Canonical serialization of the transform list — `.spt`

Normative. The IR's serialized form is a **YAML document** (and therefore also readable as
JSON, since JSON is a YAML subset — one reader serves both). Extension `.spt`, media type
`application/vnd.ctxloom.sp-transforms+yaml`. Name and extension flagged as
...
|---|---|---|
| `sp-transforms` | yes | Version integer. `1` here. Must be the first key. |
| `target` | yes | Target path, same semantics as a `--- ` line (§2.2). |
| `format` | no | Same resolution order as §2.2. |
| `transforms` | yes | Ordered sequence. Empty is `SP001` (an empty patch is refused, §10.2). |
...
lint for it. It is not a *lossless* record of a `.sp` file either: SP comments, hunk
...
Every failure is one of these codes. Every error message carries: the code, the SP path in the
target, the patch file line number, and — where applicable — the expected and actual values.

| Code | Name | Raised when |
|---|---|---|
| `SP001` | `parse-error` | The `.sp` file is malformed: bad margin, bad path, unknown directive, mixed margins in a Markdown block, `! match` on an unambiguous path, an unattached line-scoped annotation. |
| `SP002` | `target-parse-error` | The target file will not parse in its declared format, or an unclosed managed marker, or an unknown `sp:` version. **Nothing is written.** (`agent.RefuseCorrupt`'s stance, as a spec rule.) |
| `SP003` | `target-path-error` | The target path escapes the apply root, or is not a regular file. |
| `SP010` | `stale-target` | A context or `-` line's node is absent or unequal. The characteristic drift error. |
| `SP011` | `assertion-failed` | A `? expect` / `? absent` / `? exhaustive` / `? count` / `? kind`, or a `! match label=` cross-check, did not hold. |
| `SP012` | `ambiguous-match` | A key-match, HCL label tuple, or Markdown heading selected more than one node and no ordinal annotation resolved it. |
| `SP013` | `no-match` | A required node does not exist: a `-` line's node, an anchor without `?`, an `ord=` beyond the sibling count, a merge-key-inherited key. |
| `SP014` | `already-exists` | A `+` line's node already exists and the hunk is not `! idempotent`. |
| `SP020` | `inexpressible` | The requested edit cannot be represented in SP v0 or in the target format — a node move or copy (Appendix C), a comment node in JSON, a sub-block Markdown edit, a TOML surface migration. The message names Appendix C's condition for a spec revision. |
| `SP021` | `unsupported-format` | No binding for the declared or inferred format. |
| `SP030` | `conflict` | Two hunks in one apply touch overlapping nodes with incompatible ops. |
| `SP040` | `anchor-ambiguity` | A YAML path resolves through or at an alias with no `! anchor` directive. |
| `SP041` | `surface-ambiguity` | A TOML path is defined at two surfaces. |
...
- An already-applied patch (`SP010`), absent `! idempotent`.
- An empty patch file, or a file section with no hunks: `SP001`. (`cli.runConfigWrite`'s
...
```
sp: config.yaml:/server/timeout: SP010 stale-target
  patch.sp:9: expected 30
...
can find it here and determine whether SP v0 does it, how it is written, or why it does not.
...
- **Mirror** — the `.sp` rendering, or `IR-only` when the mirror grammar cannot express it.
...
`✗` explicitly unsupported (raises `SP020`).
...
| **CUE** | unification; **no delete** | Not an op vocabulary. Its missing delete is why SP is not a unification language. |
...
| **ctxloom M7 `config.yaml`** | node-tree patch preserving unchanged sections, drop removed keys, append new keys sorted | OP-01, OP-05, OP-02 — and it is the closest existing thing to an SP applier. |
...
withdrawn. SP's position is that a merge is a *set of explicit ops*, and the differ (§9.4) is
...
**Formats** json ✓ · jsonc ✓ · yaml ✓ · toml ✓ · hcl ✓ · markdown — · **Errors** `SP012` `SP013`
...
implies. Without `?`: absent → `SP013`. Two elements with the same key → `SP012`.
**Mirror**
```
@@ /mcpServers @@
  - name: github
...
Comments are nodes in JSONC, YAML, TOML and HCL, and SP treats them as such. **Decision (this
...
**Formats** markdown ✓ only · **Errors** `SP010` `SP012` `SP013`
...
mechanisms SP is meant to serve unaccounted for.

| # | Operation | Source | Status | Where it lives instead |
|---|---|---|---|---|
| OP-48 | `create-file-if-absent` | M2 (never creates), M4 (does create) | **deferred** | The CLI's `--create` would own it. The IR is per-target and assumes the target exists. Named because M2's "an absent file is never created" is a *deliberate* behavior SP must be able to express. |
...
| OP-51 | `ledger-recorded-withdrawal` | M1 (`.ctxloom-managed`) | **out of scope** | Ownership records are not a patch operation. But see [O14](#open-questions-for-ratification): an applied `.sp` file is itself a candidate ownership record, which would make `sp revert` the withdrawal story. |
...
| systemd-style | `[Service]` with repeatable keys | `/Service/ExecStart` addresses a *sequence* when the key repeats; a repeated key that the patch addresses as a scalar is `SP012`. |
...
Node kinds: element, attribute, text, comment, processing instruction. Addressing sketch: SP
...
`tests/spcorpus/` is the normative artifact. The Go implementation and the Rust port run the
...
|---|---|---|
| **`parse`** | `patch.sp` → transform list | The notation. Format-agnostic; needs no target file and no backend. A Rust port can pass every `parse` case before a single backend exists. |
| **`apply-ir`** | `transforms.spt` + `target.*` → `expected.*` | The applier. Format mechanics and byte preservation, with the notation entirely out of the picture. |
| **`e2e`** | `patch.sp` + `target.*` → `expected.*` | The composition, which is what a user experiences. |

A case directory carrying `patch.sp`, `transforms.spt`, `target.*` and `expected.*` pins all
three from one set of fixtures, and the runner asserts each independently. This is why
`transforms.spt` (§9.6) is a corpus file and not an implementation detail — and why
`sp apply --transforms` (Appendix B) gets conformance coverage for free: it is the
...
| Seam | Input → output | What it isolates |
|---|---|---|
| **`render`** | transform list → `.sp` → transform list | The renderer, via IR identity (§13.5). |
| **`diff`** | `old.*` + `new.*` → `expected.sp` | The differ's determinism and context radius (§9.4). |
...
| `patch.sp` | every seam except `apply-ir` and `diff` | The patch. |
| `transforms.spt` | `parse`, `apply-ir`, `render` seams | The canonical transform list (§9.6) — expected output of the parser *and* input to the applier. |
...
| `expected.sp` | `diff` seam | The differ+renderer's expected output, byte-exact (§9.4-R1). |
...
`SP001` that surfaces at the `e2e` seam instead of the `parse` seam means the parser is
...
seams: [cli]
kind: cli
argv: ["apply", "--in-place", "patch.sp"]
exit: 1
stderr_contains: ["SP010"]
...
an identity on the text, and the corpus must not assert that it is: SP comments, hunk
...
`old.*`, `new.*`, `expected.sp`, and `case.yaml` with `seams: [diff, render, parse, apply-ir, e2e]`.
...
**Package path:** `github.com/ctxloom/ctxloom/internal/sp`, with format bindings at
`internal/sp/spjson`, `spjsonc`, `spyaml`, `sptoml`, `sphcl`, `spmarkdown`, and the CLI at
`cmd/sp`. The core package imports nothing from ctxloom (architecture principle 6: it is
extraction-ready for the standalone repo and the Rust port). Filesystem, git and ledger
integration live in `internal/sp/spfs` and `cmd/sp`, never in the core.
...
|---|---|---|
| Parser | `sp.Parse([]byte) (*Patch, error)` + `(*Patch).Transforms()` | notation, format-agnostic |
| Renderer | `sp.Render(TransformList, RenderOptions) ([]byte, error)` | notation, format-agnostic |
| Applier | `sp.Applier` interface, one impl per format | format |
| Differ | `sp.Differ` interface, one impl per format | format |
...
// Version is the .sp notation version; TransformsVersion is the IR serialization version.
const (
    Version           = 1
    TransformsVersion = 1
)
...
// MarshalTransforms writes the canonical .spt serialization (spec §9.6).
func MarshalTransforms(tl TransformList) ([]byte, error)

// UnmarshalTransforms reads a hand-authored or generated .spt document. This is the
// input path behind `sp apply --transforms`, and the reason moves and copies need no
// notation surface.
...
```go
// Parse reads an SP patch document. It performs NO I/O, opens no target, and knows
...
```go
// Render writes a transform list back out as .sp mirror-grammar text.
...
type RenderOptions struct {
    ...
    Preamble  bool // emit "sp: 1"; false when appending to an existing document
    Comment   string
}
...
var ErrInexpressible = errors.New("sp: transform not expressible in mirror grammar")
```

### A.4 Applier — IR + target bytes → patched bytes

```go
// Applier is a format binding's apply half. It never sees .sp text, a margin, a
...
type Applier interface {
    ...
    // qualifiers. An unsupported qualifier must surface as SP020, never be ignored.
...
```go
package spsource // cmd/sp's helper; NOT imported by internal/sp
...
```go
package spfs // may import ctxloom
...
func ApplyFile(fsys afero.Fs, root string, p *sp.Patch, opt sp.Options) ([]FileResult, error)

// ApplyTransforms is the same path for a hand-authored or generated .spt document —
// the `sp apply --transforms` entry point, and the seam the corpus pins as `apply-ir`.
func ApplyTransforms(fsys afero.Fs, root string, tls []sp.TransformList, opt sp.Options) ([]FileResult, error)
...
const (
    ...
    CodeParse             Code = "SP001"
    CodeUnsupportedFormat Code = "SP021"

    // Resolver / IO layer — raised by spfs and spsource.
    CodeTargetParse Code = "SP002"
    CodeTargetPath  Code = "SP003"

    // Applier layer — raised while evaluating a transform list against a document.
    CodeStaleTarget      Code = "SP010"
    CodeAssertionFailed  Code = "SP011"
    CodeAmbiguousMatch   Code = "SP012"
    CodeNoMatch          Code = "SP013"
    CodeAlreadyExists    Code = "SP014"
    CodeInexpressible    Code = "SP020"
    CodeConflict         Code = "SP030"
    CodeAnchorAmbiguity  Code = "SP040"
    CodeSurfaceAmbiguity Code = "SP041"
)
...
// Error is the one error type SP returns. Every field is part of the contract the
// corpus asserts on.
type Error struct {
    Code       Code
    Layer      Layer
...
```go
package spcorpus // tests/spcorpus is data; this is the Go runner

type Seam string

const (
    SeamParse   Seam = "parse"    // patch.sp        -> transforms.spt
    SeamApplyIR Seam = "apply-ir" // transforms.spt + target -> expected
    SeamE2E     Seam = "e2e"      // patch.sp + target       -> expected
    SeamRender  Seam = "render"   // transforms.spt -> .sp -> transforms.spt  (RT2)
    SeamDiff    Seam = "diff"     // old + new      -> expected.sp
    SeamCLI     Seam = "cli"
)
...
**Shape ruling still open** (P0 left it so): standalone companion binary `sp` (family
...
```
sp apply [flags] <patch.sp>...
sp apply -                                    read the patch from stdin
sp apply --transforms <file.spt>...           apply a transform list directly
```
...
| `--transforms FILE` | Read a canonical transform list (§9.6) instead of `.sp` notation. **This is the authoring path for moves and copies** (Appendix C) and the only way to reach OP-21/OP-22/OP-07/OP-36. Mutually exclusive with positional `.sp` arguments. Flag name at [O21](#open-questions-for-ratification). |
...
```
sp diff [flags] <old> <new>
sp diff HEAD:config.yaml config.yaml          the canonical invocation
sp diff HEAD~3:.mcp.json .mcp.json
sp diff old.toml -                            new side from stdin
```
...
| `--transforms-out FILE` | Emit the transform list instead of `.sp` notation. |
...
| `1` | The patch did not apply: `SP010`–`SP041`. **No file was modified** (§10.5). |
| `2` | Trouble: usage error, unreadable patch (`SP001`), unreadable/unparseable target (`SP002`/`SP003`), unresolvable descriptor, `git` absent for a git anchor, unsupported format (`SP021`), I/O failure. |
...
treats nonzero as "unknown state" is being unnecessarily defensive; SP's contract is that
nonzero means unchanged.

Stdout carries results (`--ops`, `--transforms-out`, `-o -`, `--format-out json`, `sp diff`).
...
SP v0 is one human-authoring notation, and one notation cannot express everything. This
...
`sp apply --transforms move.spt`. The applier preserves the subtree's source bytes and its
...
are lost, byte-exact formatting is lost, and a large subtree must be restated in full. SP does
...
Same reasoning, same remedy: `{op: copy, from: …, path: …}` in a `.spt`. The mirror-grammar
...
The IR cannot express it either. `SP020`.

### C.4 TOML surface migration (OP-39)

Rewriting `[a.b]` into `a.b = {…}` or vice versa (§8.4 rule 4). Deferred, and **not** reachable
via the IR — SP edits values at whichever surface exists; restructuring surfaces is a
formatter's job. `SP020`, and see [O10](#open-questions-for-ratification).
...
|---|---|---|---|
| **O1** | Standalone `sp` binary vs `ctxloom patch` subcommand. | Appendix B specifies standalone. | P0 left it explicitly open. Standalone means a fourth companion binary to build, sign, and install; the ctxloom subcommand is free but ties a general-purpose standard to one host. |
| **O2** | Doc filename convention. | `docs/design/sp-spec.md`, per the brief. | The directory's existing convention is `<name>.design.md`; `docs/` also has `signature-envelope.spec.md`. Once ratified this is a *spec*, not a design, so `docs/sp.spec.md` may be the right home. |
| **O3** | Is re-applying a patch an error? | Yes by default (`SP010`); `! idempotent` (§7.5) opts out per hunk. | Every ctxloom managed-file writer is idempotent by construction, so `! idempotent` may end up on ~every hunk ctxloom itself writes — an argument for inverting the default, or for a file-level `idempotent: true` preamble key. |
...
| **O5** | Extending RFC 6901's escape set with `~2` for `=`. | Adopted (§4.1). | It makes SP paths not-quite-JSON-Pointers even when they look like one. Alternative: require quoting the whole segment (`/"a=b"`), reusing the label-segment syntax. |
...
| **O8** | `! match ord=` on an unambiguous path. | `SP001 parse-error` (§7.2). | Strict, and it means a patch breaks when the file *stops* being ambiguous (someone deletes the duplicate block). The alternative — tolerate a redundant ordinal — means an ordinal can silently become meaningless. |
| **O9** | Unknown preamble keys and unknown `!` directives. | Hard failure `SP001`. | Fail-loud is this project's discipline, but it means SP v1 readers reject every v1.1 patch, so the version field carries the whole forward-compat burden. |
| **O10** | TOML surface migration (`[a.b]` → `a.b = {…}`). | Not a patch operation (`SP020`, §8.4 rule 4, Appendix C.4). | It is a real thing people want to do, and refusing it pushes them to a text editor — where they will drift. |
...
| **O14** | Where does the ledger meet SP? | Nowhere in v0: the spec has no ownership-record concept. | `config-write`'s missing ownership record (review B2, `distinct-bullpen`) is the mechanism SP is meant to serve. An SP patch is a natural *record* of what ctxloom added — an `sp revert` from the same file could be the withdrawal story the ledger currently provides. That may deserve a spec section rather than an adapter. |
| **O15** | Corpus location. | `tests/spcorpus/`, sibling to `tests/acceptance` etc. | The corpus is meant to be consumed by a *different repository* (the Rust port). A top-level `spcorpus/` or a dedicated repo may be right; `tests/` implies Go-test-only ownership. |
| **O16** | **Is delete-and-add acceptable semantics for a move?** | Yes — the spec does not detect the pattern, does not error on it, and Appendix C.1 records it as the substitute. | The alternative was to detect "a `-` and a `+` of an equal subtree at different paths" and raise `SP020 inexpressible` pointing at Appendix C. That was rejected because the detection is a heuristic (how equal is equal? does a one-key change break it?), because a false positive would block a legitimate delete-and-add, and because SP has no way to know whether the author *meant* a move. **The cost is real and named:** comments attached to the removed node are lost, byte-exact formatting of the moved subtree is lost, and a large subtree must be restated in full. If a reviewer wants moves detected rather than silently degraded, that is a deliberate reversal and Appendix C.1 changes with it. |
...
| **O20** | Git anchors via subprocess, not a linked library. | §9.5, Appendix A.7. | Ruled by the human. Recorded here because it has a real cost: `sp` in a container without `git` silently loses `REV:path` support, and the error is a usage error at the CLI rather than a capability probe. |
| **O21** | Name and extension of the serialized IR: `.spt`, `sp apply --transforms`. | §9.6, Appendix B.1. | Alternatives: `.sp.yaml` (signals "this is YAML"), `.spops`, or `sp apply --ir`. Also: should `sp apply` sniff the input and accept either form on the positional argument, rather than requiring a flag? |
| **O22** | File-level effects derived from node-level results (OP-48, OP-49). | Deferred; not in the IR. | ctxloom's M2 genuinely does this: it *removes the file* when nothing user-authored remains, and never creates an absent one. If SP is to replace M2, either the IR grows file-level effects or the adapter keeps owning them — and the second answer means SP does not actually replace M2. |
...
- **The diff algorithm itself.** `sp diff` is P4. §9.4 specifies its *requirements*
...
- **Merge / conflict resolution** between two SP patches.
- **Signing.** SP files are content like any other; whether they ride ctxloom's signature
  envelope is a P5 question.
- **Encoding other than UTF-8**, BOM handling, and CRLF. (CRLF targets: a binding must
  preserve the target's line ending; the `.sp` file itself is LF. Not corpus-covered in v0.)
...
- **A second human-authoring notation.** The v0 ruling is one grammar. The transform list
  (§9.6) is an accepted *input*, but it is machine-first by design and a `.spt` file where a
  `.sp` file would do is a review-quality regression.
- **Reverting an applied patch** (`sp revert`). Sketched only in [O14](#open-questions-for-ratification).
